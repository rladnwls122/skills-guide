...
대회(전국기능경기대회 클라우드컴퓨팅)의 모든 과제는 Terraform 으로 시작한다. 이 문서는 set-02 task-1 실측 코드를 근거로, "채점 스크립트가 무엇을 보는가" 관점으로 문법과 VPC 를 정리한다.
...
**② 왜 필요한가 (채점 관점)** — 대회 규칙상 SW 버전은 최신 안정 버전으로 **고정**해야 한다(latest 금지). 또 리전이 틀리면 채점 스크립트가 리소스를 아예 못 찾아 전 항목 0점이 된다. 리전은 반드시 변수로 주입한다.
