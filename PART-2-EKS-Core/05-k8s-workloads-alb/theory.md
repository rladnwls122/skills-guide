...
graph TD
    ...
    D --> P6["IAM: serviceAccountName<br/>wskorea26-book-sa (IRSA, eksctl이 생성)"]
```
...
| resources | requests+limits 명시 | QoS 확보. requests=limits로 맞추면 Guaranteed 클래스 |
| preStop + grace | `sleep 15` / 45s | 아래 graceful shutdown 참조 |

**graceful shutdown 흐름**: Pod 삭제 시작 → (동시에) 엔드포인트 제거 전파 & preStop 실행. `sleep 15` 동안 ALB/TGB가 타깃 등록 해제를 끝내므로 in-flight 요청 유실이 없다. 이후 SIGTERM, 최대 45초(terminationGracePeriodSeconds) 내 종료.
...
**① 한 줄 정의** — LBC는 k8s 리소스를 보고 ELB API를 호출하는 컨트롤러이며, **TargetGroupBinding(TGB)** 은 "이미 존재하는 TG"에 Service 뒤 Pod IP를 등록/해제하는 CRD다.
...
```mermaid
graph LR
    TF["Terraform<br/>aws_lb + aws_lb_target_group<br/>(이름 고정, target_type=ip)"] -->|TG ARN output| TGB["TargetGroupBinding CRD<br/>serviceRef: wskorea26-book-svc:8080<br/>targetGroupARN, targetType: ip"]
    TGB --> LBC["LBC (kube-system,<br/>nodeSelector node-type=addon)"]
    LBC -->|RegisterTargets| TG["wskorea26-book-tg"]
...
**② 왜 (채점 관점)** — mark 7-2가 `describe-rules`의 `HttpHeaderConfig.Values[]` 출력으로 **`wskorea26-cf` 정확히 2줄** + `curl http://<ALB>/book` = **403**을 검사한다. 규칙을 추가/삭제하면 줄 수가 달라져 즉시 감점 — **규칙 개수는 정확히 2개**.

**③ 원리 (실측 alb.tf)**

```mermaid
graph TD
    L["Listener :80"] --> R1["규칙 priority 10<br/>조건: X-Origin-Verify=wskorea26-cf<br/>AND method=POST"] --> TG1["wskorea26-book-tg (Pod)"]
    L --> R2["규칙 priority 20<br/>조건: X-Origin-Verify=wskorea26-cf"] --> TG2["wskorea26-lambda-tg (Lambda)"]
...
1. readinessProbe와 livenessProbe의 실패 시 동작 차이는?
2. topologySpreadConstraints에서 `whenUnsatisfiable: DoNotSchedule`과 `ScheduleAnyway`의 차이는?
3. Ingress 대신 Terraform ALB + TGB 패턴을 쓰는 결정적 이유는?
4. mark 7-2가 기대하는 출력은 무엇이고, 규칙을 3개로 늘리면 어떻게 되는가?
5. preStop `sleep 15`는 무엇을 기다리는 시간인가?

<details><summary>정답</summary>

1. readiness 실패 → Service 엔드포인트(및 TG 타깃)에서 제외될 뿐 컨테이너는 유지. liveness 실패 → 컨테이너 재시작.
2. DoNotSchedule은 제약을 못 지키면 **스케줄 자체를 거부**(하드), ScheduleAnyway는 최선 노력(소프트). AZ 분산을 강제하려면 DoNotSchedule.
3. Ingress가 만드는 ALB는 **이름을 지정할 수 없는데** 채점이 ALB/TG 이름을 정확 비교하기 때문. Terraform으로 이름 고정 생성 후 TGB로 Pod IP만 등록한다.
4. `wskorea26-cf` 2줄 + 403. 헤더 조건 규칙이 3개면 Values[] 가 3줄이 되어 정확 일치 실패 → 1.5점 감점.
5. 엔드포인트/타깃 등록 해제가 ALB까지 전파되는 시간. 그동안 컨테이너가 살아서 in-flight 요청을 마저 처리한다.
