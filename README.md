...
> AWS 기초만 아는 상태에서 14일 만에 **수상 과제 스택을 블라인드 재현**하고,
> 대회장의 **30% 변동·트러블슈팅**에 대응하는 것이 목표.
>
...
1. 1과제 공통 스택(CloudFront→ALB→EKS/Lambda→DynamoDB + KMS + 관측성)을
   **문서 없이 4시간 내 배포 + mark.sh 자동 채점 90%+**.
2. 과제지 30% 변동을 **30분 내 반영** (grep/sed 드릴).
3. 대표 장애 12종을 **확인 명령→원인→조치** 순서로 자력 해결.
