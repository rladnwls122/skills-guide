...
- 한글 서체: 제목 조선일보명조 · 본문 Noto Serif KR · UI Pretendard — `src/styles/korean-fonts.css`
...
따로 싣는다. 원본 합계 36.5MB 를 문서에 실제로 쓰인 글자만 남겨 568KB 로 굽고,
결과물은 레포에 커밋한다 — 빌드에는 Python 이 필요 없다.

| 역할 | 서체 | 서브셋 |
| --- | --- | --- |
| 제목·register | 조선일보명조 ChosunSm | 97KB |
| 본문 reading | Noto Serif KR | 284KB |
...
`FONTS` 에서 경로 변경). Noto Serif KR 은 없으면 자동으로 내려받고, Pretendard 는
`npm install` 로 받아진다.
