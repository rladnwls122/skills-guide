...
| **WAF** | ✗ | **○ (1.5/30)** | **○ (2.5/30)** | 모듈 02 에 신설 |
| EC2 | ✗ | ○ | ○ | — |
| **관측성**(Prometheus·Grafana·Fluent Bit) | **○ (3.5/30)** | **○ (5.2/30)** | **○ (5.0/30)** | PART-3 모듈 07(D8) |

**관측성이 후보 세 편 모두 1과제 안에 있다.** 초판은 "set-07 에만 있다"고 적었는데 틀렸다.
`task-1/task.md` 에서 Prometheus·Grafana·Fluent Bit 언급 횟수가 set-02 4회, set-03 5회, set-07 11회고,
각 `mark.md` 에 배점이 잡혀 있다. set-02 만 도구 선택이 자유롭고 수동 채점이며, set-03·set-07 은
스택이 고정되고 실시간 채점이다.

**배점으로 보면 관측성이 WAF 보다 크다**(3.5~5.2 대 1.5~2.5). 그런데 사이트는 관측성을 PART-3
모듈 07(D8)에 두고 1과제 완주를 D4~D7 에 둔다 — **어느 후보가 나와도 순서가 맞지 않는다.**
1과제 완주 모듈에서 Fluent Bit·kube-prometheus-stack 설치를 최소한 다루고 모듈 07 로 링크해야 한다.
장기적으로는 관측성 일부를 PART-2 로 당기는 재배치가 필요하다.

WAF 는 후보 셋 중 둘(set-03·set-07)의 채점 대상이라 모듈 02 에 절을 신설했다.
...
| 7 | 1과제 후보 차이 — WAF 신설 + 후보 3편 비교표 | 확정 | 완료 |
| 8 | Flink · MSK 실습 보강 (현재 함정 암기만) | 확정 | 완료 |
| 9 | 3과제 승격 — 부하·SLO·비용 최적화 신규 | 확정 | 완료 |
| 10 | **관측성을 PART-2 로 당기는 재배치** — 후보 3편 모두 1과제 안에서 요구한다 | 확정 | 대기 |
| 11 | PART 재배치와 로드맵·인덱스 갱신 | 10까지 끝난 뒤 | 대기 |

7~9번을 실행하면서 나온 것 — **관측성이 후보 3편 모두 1과제 안에 있다는 사실**을 확인했다(위
발견 2 참조). 초판이 set-07 만이라고 본 것이 틀렸다. 배점도 WAF 보다 크다. 지금은 모듈 06 에
경고와 링크를 넣는 선에서 막았지만, 근본적으로는 관측성 일부를 PART-2 로 당기는 재배치가
필요하다 — 10번으로 남긴다.
