...
모듈 07(D8)에 두고 1과제 완주를 D4~D7 에 뒀었다 — **어느 후보가 나와도 순서가 맞지 않았다.**
실행 단계 10 에서 관측성 모듈을 PART-2 로 옮기고 완주 **앞**(모듈 06, D6)에 놓아 해소했다.
...
아래가 **실행 완료된 최종 배치**다(실행 단계 10·11).

| PART | 모듈 | 일차 | 내용 | 근거 |
|---|---|---|---|---|
| 선수 지식 | — | D0 | 자가진단 + 기초 문서 | 현행 유지 |
| PART-1 공통 기반 | 01~03 | D1~D3 | VPC · IAM · KMS · S3 · CloudFront · DynamoDB · ECR · Terraform | 1·2·3과제 공통 부품 |
| PART-2 1과제 · EKS·관측성 | 04~07 | D4~D8 | eksctl · k8s 워크로드 · LBC · **관측성** → 1과제 완주 | 후보 3편 모두 EKS + 관측성이 1과제 안 |
| PART-3 Hard Mode | 08 | D9 | fully-private EKS · IAM 심화 · CoreDNS | 후보 중 어려운 편 대비 |
| PART-4 2과제 모듈 | 09~13 | D10~D12 | 후보 12개 모듈. **DocumentDB · VPC Lattice · CDN Function 포함** | 후보 세 세트 실측 |
| PART-5 3과제 | 14 | D13~D14 | 부하 운영 · SLO 튜닝 · RDS Proxy·인덱스 · 비용 최적화 | **배점 40점. 2주 안으로 승격** |
| PART-6 Battle Drills | 15~17 | **일차 없음** | 30분 치환 드릴 · 12종 자력 복구 · 4시간 모의 대회 | 배점 없는 훈련 — 여유 시간에 |

합계 14일로 "2주 완성"과 맞는다. 모듈 번호가 PART 순서와 일치한다 — 신설 3종이 09·10 뒤에
15~17 로 붙어 어긋났던 것을 11~13 으로 당기고, 드릴 3종을 15~17 로 밀었다.

**관측성이 PART-2 로 들어오면서 하루가 필요해졌고, 그 하루는 모의 대회에서 뺐다.** 개편안이
모의 대회를 이미 "위 일정 안에서"로 규정하고 있었으므로 고정 일차를 없애는 쪽이 가장 싸다.
대안이던 "완주 2일 → 1일 압축"은 중심축 모듈을 깎는 것이고, "Hard Mode 08 일차 제거"는
선수 학습 문서 여러 곳이 모듈 08 을 후속 학습으로 지목하고 있어 둘 다 부작용이 더 컸다.

**3과제는 D13~D14 로 2일이다** — 개편안 초안의 3일에서 하루 줄었다. 모듈 14 자체가 8~10h × 2일로
잡혀 있어 3일째에 넣을 내용이 없다. 대신 "2주 밖 예비"에서 일정 안으로 들어왔다.

관측성 모듈이 완주 **앞**에 온다. 완주 런북 10단계에 모니터링·로깅 단계가 들어 있어 순서가
반대면 배우지 않은 것을 실행하게 된다.
...
**결과** — 드릴 두 편이 고정 일차를 잃었다. 이후 실행 단계 11 에서 세 편(변동 드릴·파괴/복구·
모의 대회)이 함께 PART-6 으로 묶이면서 모의 대회의 D14 도 없어졌다 — 셋 다 "일정 안에서
끼워 넣는" 훈련이 됐다. 경로는 `part-6/15~17` 이다.
...
| 10 | **관측성을 PART-2 로 당기는 재배치** — 후보 3편 모두 1과제 안에서 요구한다 | 확정 | 완료 |
| 11 | PART 재배치와 로드맵·인덱스 갱신 | 10까지 끝난 뒤 | 완료 |
| 12 | `reference/mark-script-guide` 를 후보 세트 `mark.sh` 실측으로 재작성 | 확정 | 완료 |

7~9번을 실행하면서 나온 것 — **관측성이 후보 3편 모두 1과제 안에 있다는 사실**을 확인했다(위
발견 2 참조). 초판이 set-07 만이라고 본 것이 틀렸다. 배점도 WAF 보다 크다. 당시에는 완주 모듈에
경고와 링크를 넣는 선에서 막았고, 10·11번에서 모듈 자체를 옮겨 해소했다.

**10·11번은 한 번에 실행했다.** 관측성을 옮기면 PART-2 가 하루 늘고, 그 여파로 뒤 PART 의 일차와
모듈 번호가 전부 밀린다 — 나눠서 하면 중간 상태가 어차피 어긋난 채로 남는다. 실제로 옮긴 것:
`07-observability` → `part-2/06`, 완주 → `part-2/07`, 신설 3종 → `part-4/11~13`,
3과제 → `part-5/14`, 드릴 3종 → `part-6/15~17`. 사이드바·루트 로드맵 표·카드·`reference/links`
모듈 표·산문 참조를 함께 고쳤고 `starlight-links-validator` 로 링크 74페이지를 검증했다.
...
**12번에서 나온 것** — 채점 스크립트가 두 종류다. set-02·set-07 은 값만 찍고 채점자가 채점지와
대조하는 **출력형**, set-03 은 스크립트가 `PASS`/`FAIL` 을 스스로 내는 **판정형**이다. 판정형이
더 무섭다 — `if` 조건 하나가 그 항목의 전부다. set-03 의 `check_kms()` 는 키 정책에 `"kms:*"`
또는 `:root"` 가 있으면 `grep` 한 줄로 FAIL 을 낸다. `AdministratorAccess` 부착·정책 액션 `*`·
클러스터 SG 의 전체 개방도 같은 방식으로 그 항목을 0점 처리한다. **편의로 넓게 준 권한이 감점이
아니라 항목 전체를 날린다.**

두 가지가 더 나왔다. (1) set-03 9-2 는 CloudFront 캐시 정책을 **관리형 ID 로 하드코딩**해
비교한다 — 동작이 같은 커스텀 정책을 만들면 FAIL 이다. (2) set-03 11-1 은 채점 스크립트가
`kubectl run` 으로 이상 파드 6개를 심고 180초 뒤 Grafana 알림 5개가 Firing 인지 본다. 관측성은
"패널이 보이면 통과"가 아니라 **채점자가 주입한 이상을 잡아내야** 하는 항목이다. PART-2 모듈 06
이 이미 알람 6종·mark 11-4 Firing 검사·`log_to_metrics` 파이프라인을 다루고 있어 보강할 것은
없었다 — 채점 스크립트를 읽고 나서야 그 모듈의 서술이 맞았다는 것을 확인했다.

**전 단계 완료.** 남은 것은 콘텐츠 보강이지 배치 변경이 아니다 — 3과제 공식 자료가 나오면
"미확인" 절 전체를 재검증하고, 3과제 후보 목록을 받으면 PART-5 의 근거를 다시 잡는다.
