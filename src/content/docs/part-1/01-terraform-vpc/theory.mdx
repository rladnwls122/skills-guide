...
provider 블록이 리전을 갖는 이유는 AWS API 자체가 리전별로 나뉘어 있기 때문이다. VPC·S3 버킷·KMS 키 같은 리소스는 만들어진 리전 안에만 존재하고, 호출 주소(엔드포인트)도 `ec2.ap-northeast-2.amazonaws.com` 처럼 리전이 박혀 있다. provider 의 `region` 이 그 주소를 정하므로, 이 값이 그 provider 로 만드는 모든 리소스의 위치가 된다. 다른 리전에도 리소스를 만들어야 하면 provider 를 하나 더 선언해 별칭(alias)을 붙인다.
...
**④ 세트별 차이** — set-03 은 같은 `aws_caller_identity` 의 계정 ID 를 KMS 키 정책의 `kms:CallerAccount` 조건에 넣어 root ARN 없이 계정 위임을 표현한다(root-less 키 정책). Day 2 에서 다룬다.
