...
4. **(심화) root-less 키 정책으로 재작성**
   ...
   - `kms:*` 없이 admin 액션을 개별 나열한 `kms_admin` 문서 작성 (`kms:Create*`, `kms:Put*` 포함 필수)
...
6. **(보너스) WAF Web ACL 부착 — CLOUDFRONT scope**
   ...
   **오탐 대응 훈련(Count → 검증 → Block 복구)**: SQLi 규칙의 `action`을 `block{}`에서 `count{}`로 바꿔 `apply`한 뒤, 같은 SQLi 페이로드 요청이 200으로 통과하는지 확인한다(차단은 안 되지만 CloudWatch 메트릭·sampled requests엔 여전히 잡힌다). 확인했으면 `action`을 다시 `block{}`으로 되돌려 `apply`한다 — 관리형 규칙 그룹(set-07 방식)이었다면 이 자리에 `rule_action_override`가 들어간다.
