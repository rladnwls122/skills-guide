...
root-less 키 정책이 통과하는 조건을 판정 순서로 그렸다. 첫 관문은 채점 쪽이다 — root principal 이나 `kms:*` 가 정책에 있으면 그 자리에서 실격이다. 둘째 관문은 KMS 쪽이다 — 정책을 넣는 신원에게 `kms:PutKeyPolicy` 가 없으면 키를 아무도 못 다루게 되는 상태가 되므로 apply 를 거부한다. 둘 다 피하는 형태가 관리 권한은 `kms:CallerAccount` 조건으로 계정 전체에 위임하고, 서비스에는 사용 권한만 따로 주는 구성이다.
