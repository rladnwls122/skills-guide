...
**② 왜 필요한가 (채점 관점)** — 후보 세트 중 set-03·set-07 은 WAF를 채점하고 set-02 는 아예 없다. 그런데 확인 방식이 다르다. set-03 mark 10-1 은 `aws wafv2 list-web-acls`로 리소스 자체를 조회한 뒤 SQLi/XSS payload(`booking_id=1' OR 1=1--`, `<script>alert(1)</script>`)를 실제로 던져 403을 확인하고, rate 규칙의 `Limit` 값을 API로 읽어 200 이하인지 본다. set-07 은 `aws wafv2` 호출이 전혀 없이 순수 curl 블랙박스 테스트(XSS 프로브 상태 코드, 연속 요청 후 상태 확인)로만 판정한다. 이름만 맞고 실제로 안 막으면 두 세트 모두 감점이라는 뜻이다.
...
관리형 규칙 그룹이 정상 요청을 오차단하면: ① CloudWatch sampled requests 로 원인 규칙을 특정 → ② 그 규칙만 RuleActionOverrides 로 Count 전환 → ③ 정상 요청이 통과하는지 검증 → ④ 문제가 해결됐으면 다시 Block. 명령어 수준 절차는 [트러블슈팅](/reference/troubleshooting/)의 "정상 요청이 403" 항목에 있다.
:::
...
**로깅**: set-07 은 `aws_wafv2_web_acl_logging_configuration`으로 평가 로그를 CloudWatch Logs(이름은 반드시 `aws-waf-logs-`로 시작, us-east-1, Platform CMK 암호화)에 남긴다. 로그 배달 주체(`delivery.logs.amazonaws.com`)가 로그 그룹에 쓸 수 있도록 리소스 정책을 별도로 붙여야 로깅 설정 자체가 성공한다. set-03 은 과제지에 로깅 요구가 없어 로깅 설정을 만들지 않는다 — **과제지에 없으면 안 만드는 원칙(4절)은 여기도 동일**하다.
...
- [x] `kms:*` 를 못 쓰므로 `kms:Create*`/`kms:Put*` 등 관리 액션을 개별 나열해야 KMS lockout safety check(관리 불능 정책 거부)를 통과한다
...
관리형 규칙 그룹은 세 층으로 오버라이드한다. 그룹 전체를 켜고 끄는 `override_action`(그룹 단위), 그리고 그 안의 이름 있는 규칙 하나만 바꾸는 `rule_action_override`(규칙 단위, API 이름은 `RuleActionOverrides`). 오탐이 난 규칙 하나만 Count로 돌리고 나머지는 그대로 Block을 유지하는 것이 표준 절차다 — 문제 규칙을 CloudWatch sampled requests로 먼저 특정하고, Count 전환 후 정상 요청이 통과하는지 검증한 뒤 다시 Block으로 되돌린다. set-03처럼 직접 짠 리프 규칙(`sqli_match_statement` 등)은 애초에 규칙을 자기가 소유하므로 `action`을 바로 `count`로 바꾸면 된다 — 이 계층 구분이 필요 없다.
</Quiz>
