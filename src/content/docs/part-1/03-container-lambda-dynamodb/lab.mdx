...
4. **push & 스캔 확인 (CloudShell)**
   ...
   이미지를 올리고 스캔 결과까지 확인한다. `${ECR%%/*}` 는 리포지토리 경로를 잘라 레지스트리 주소만 남기는 셸 문법이라 로그인 대상이 된다. `wait image-scan-complete` 로 스캔이 끝나기를 기다린 뒤 심각도 집계를 보고, 마지막 `describe-images` 로 태그가 `stable` 하나뿐인지 확인한다 — `latest` 가 함께 붙으면 감점이다.
