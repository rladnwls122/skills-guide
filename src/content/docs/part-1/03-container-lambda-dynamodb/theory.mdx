...
:::caution[빌드 환경]
대회 PC(Windows, Docker/WSL 불가)에서는 빌드 불가 → **기본 CloudShell** 에서 빌드한다. CloudShell 은 amd64 네이티브라 단일 아키텍처 이미지가 되고 스캔과 호환된다. (VPC CloudShell 은 인터넷이 없어 alpine 을 못 받는다 — 기본 CloudShell 사용.)
:::
...
graph LR
    ...
    subgraph GSI["<span class='icon--mdi icon--mdi--layers-outline'></span> GSI concert_name-created_at-index<br/>(HASH concert_name + RANGE created_at)"]
        G1["A: [t1, t3] 정렬 저장"]
        G2["B: [t2]"]
    end
    TABLE -- "자동 복제" --> GSI
...
DynamoDB 숫자에 Python float 를 넣으면 **boto3 가 저장 자체를 거부**한다. 또 `483/5` 를 float 로 계산하면 `96.60000000000001` 이 되어 채점 기대값 `96.6` 과 불일치.
:::
...
- [x] ECR Basic 스캔(Clair)이 OS 패키지 DB 가 없어 아예 스캔하지 못한다 — **스캔 불가는 스캔 통과가 아니다**
...
ECR Basic 스캔(Clair)이 scratch 를 스캔하지 못해 스캔 결과 검증이 불가하고, CA 번들이 없어 DynamoDB TLS 연결이 실패하며, zoneinfo 부재로 created_at 이 UTC 로 저장된다.
...
빌드 위치도 함께 기억할 것: 대회 PC(Windows)는 Docker/WSL 불가, VPC CloudShell 은 인터넷이 없어 alpine 을 못 받는다 → **기본 CloudShell**(amd64 네이티브).
</Quiz>
...
응답 봉투도 ALB 규격을 정확히 지켜야 한다 — `statusCode` / `statusDescription`(`"200 OK"`) / `isBase64Encoded` / `headers` / `body`. API Gateway 통합이면 `statusDescription` 이 필요 없다는 점이 세트별 차이다.
</Quiz>

<Quiz title="boto3 Decimal 함정">
DynamoDB 숫자 필드에 Python `float` 를 넣으면 boto3 가 저장 자체를 거부한다. 게다가 `483 / 5` 를 float 로 계산하면 `96.60000000000001` 이 되어 채점 기대값과 어긋난다. `Decimal(str(483)) / Decimal("5")` 로 계산했을 때 나오는 정확한 값은 [[96.6]] 이다.
