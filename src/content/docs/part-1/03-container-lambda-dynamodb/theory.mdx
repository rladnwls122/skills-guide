...
- ECR Basic 스캔(Clair)이 scratch 를 스캔하지 못해 스캔 결과 검증이 불가하다.
...
빌드 위치도 함께 기억한다. 대회 PC(Windows)는 Docker/WSL 이 안 되고, VPC CloudShell 은 인터넷이 없어 alpine 을 못 받는다. 남는 선택지는 **기본 CloudShell**(amd64 네이티브)뿐이다.
</Quiz>
