...
**① 한 줄 정의** — data source 는 만들지 않고 조회만 하는 블록이다. `aws_iam_policy_document` 는 IAM·KMS·버킷 정책 JSON 을 HCL 로 조립하는 data source 다.
...
**① 한 줄 정의** — `wskorea26-vpc`(172.16.0.0/16)는 AZ c/d 에 public 2 + private 2 서브넷을 둔다. public 은 IGW 를 쓰는 공용 RTB 1개로, private 은 AZ별 NAT 와 AZ별 RTB 2개로 라우팅한다.
...
**② 왜 필요한가 (채점 관점)** — mark 1-1 은 CIDR 을 검사한다. **mark 1-2 는 RTB↔서브넷 연결과 기본 라우트의 GatewayId(IGW)·NatGatewayId(NAT)를 그대로 본다.** 서브넷이 엉뚱한 RTB 에 붙어 있으면 인터넷이 되더라도 감점이다.
...
**⑤ 세트별 차이** — CIDR(172.16 vs 10.x), AZ 쌍(a/b vs c/d), 서브넷 수, NAT 개수가 바뀐다. NAT 는 공용 1개인 세트와 AZ별로 두는 세트가 갈린다. 이 실측 코드는 전부 변수·locals 파생이라 tfvars 교체로 흡수된다.
...
**① 한 줄 정의** — `output` 은 apply 결과값(ID·ARN·DNS)을 외부로 내보내는 통로다. 대회에서는 이 값을 `.env` 파일로 박아 터미널이 끊겨도 재사용한다.
