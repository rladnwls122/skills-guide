...
선행 지식과 학습 경로는 [모듈 개요](/part-1/07-kms-s3-cloudfront/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

ARN(Amazon Resource Name)은 AWS 리소스마다 붙는 전역 고유 이름이다. 생김새와 자리별 뜻은 [iam-basics](/start/iam-basics/)에 있다.

암호화(KMS)와 정적 웹 서빙(S3+CloudFront)은 거의 모든 세트에 나온다. 채점은 기계적 검사다 — alias 이름 역조회, 정책 텍스트 grep, 퍼블릭 차단 상태를 본다. 그래서 정확한 구조를 암기 수준으로 익힌다.
...
**① 한 줄 정의** — envelope encryption 은 데이터를 데이터 키(DEK)로 암호화한다. 그 DEK 를 다시 KMS 의 CMK 로 암호화해 보관하는 2단 구조다. 봉투 암호화·CMK·키 정책이라는 말 자체가 처음이면 [kms-basics](/start/kms-basics/)를 먼저 읽는다.
...
**④ 세트별 차이** — CMK 개수가 갈린다. set-02 는 3개(s3·dynamodb·eks), set-03 은 5개(db·ecr·eks·bucket·function)다. 원칙은 하나다 — 과제지가 이름을 준 서비스에만 CMK 를 만든다.
...
**① 한 줄 정의** — 키 정책은 CMK 에 직접 붙는 리소스 정책이다. 키를 관리하는 statement(admin)와 키를 사용하는 statement(use)를 분리해 쓴다.
...
키 정책을 두 조각으로 나눠 합친다. 위쪽은 계정 root 에 전권을 주는 기본 구문이고, 아래쪽이 CloudFront 서비스에 복호화만 허용하는 구문이다. `source_policy_documents` 가 둘을 한 문서로 합성한다. 키 정책의 `resources` 가 항상 `"*"` 인 것은 그 정책이 붙은 키 자신을 뜻하기 때문이다. [kms-basics 의 실물 JSON 절](/start/kms-basics/#1-root-less-키-정책-실물--적용된-json)이 이 표기를 IAM 정책의 `"Resource": "*"` 와 나란히 놓고 푼다. 그리고 `aws:SourceAccount` 조건이 다른 계정의 CloudFront 가 이 키를 쓰지 못하게 막는다.
...
**① 한 줄 정의** — 유의사항이 키 정책에 root principal 과 `kms:*` 액션을 금지할 때가 있다. 그러면 관리자 principal 을 `"*"` 로 두고 `kms:CallerAccount` 조건으로 계정 하나에 가둔다. 그리고 허용할 액션을 전부 나열한다.

**② 왜 필요한가 (채점 관점)** — set-03 mark 의 check_kms 가 **정책 텍스트를 grep** 한다. `:root` 나 `kms:*` 가 있으면 실격이다. 동시에 KMS 의 lockout safety check 를 통과해야 한다. 이 검사가 보는 것은 하나다 — **호출한 신원 본인에게 `kms:PutKeyPolicy` 가 허용되는가**. 그래서 관리 statement 에 `kms:Put*` 를 반드시 포함시키고, 나머지 관리·운영 액션도 개별 나열한다.

principal 을 배포자 한 사람의 ARN 으로 좁히지 않는 이유는 채점 쪽에 있다. 과제지가 "미리 IAM 사용자를 만들어 그 신원으로 진행하라"고 말하지 않으면 채점자는 지급받은 root 로 접속한다. KMS 는 IAM 정책이 아니라 키 정책이 1차 관문이다. 관리자를 배포자 신원 하나로 잠그면 채점 스크립트 자신이 `describe-key`·`get-key-policy` 조차 못 해 KMS 항목이 통째로 0점이 된다. `"*"` + `kms:CallerAccount` 는 계정 밖 principal 을 막으면서 계정 안 신원 판단은 각자의 IAM 정책에 맡기므로, root 로 채점해도 통과한다.
...
:::danger[치명 함정]
관리 statement 하나로 끝나지 않는다. AWS 서비스가 **자기 이름으로**(서비스 principal 로) 키를 부르는 경로는 `kms:CallerAccount` 조건으로 덮이지 않는다. 그래서 그 소비자마다 **별도 statement 를 직접** 써야 한다. set-03 은 키마다 소비자를 따로 적는다. bucket 키에 `cloudfront.amazonaws.com`(`aws:SourceAccount` 조건), function 키에 `lambda.amazonaws.com`(`kms:EncryptionContext` 조건)을 명시한다. db 키에도 사용 주체인 `book_pod`·`book_function` 역할을 최소 액션으로 적어 둔다.
...
**① 한 줄 정의** — 정적 웹 버킷은 퍼블릭 액세스 4종을 전부 차단한다. 암호화는 CMK 를 쓰는 SSE-KMS 이고, BucketKey 로 KMS 호출을 줄인다.

퍼블릭 액세스 차단이 접근 통제 4층 중 어디에 해당하는지, SSE-KMS 가 다른 암호화 방식과 무엇이 다른지는 [s3-basics](/start/s3-basics/)에 있다.

**② 왜 필요한가 (채점 관점)** — mark 2-2 가 퍼블릭 차단 상태와 암호화 키를 검사한다. `get-bucket-policy-status` 가 `IsPublic: false` 를 반환하려면 OAC 버킷 정책이 있어야 한다. 버킷 이름은 `wskorea26-concert-bucket-<비번호>` — 비번호 변수화 필수.
...
**① 한 줄 정의** — OAC(Origin Access Control)는 CloudFront 가 S3 에 SigV4 서명으로 접근하게 하는 장치다. 버킷 정책은 `AWS:SourceArn = 배포 ARN` 조건으로 그 배포에게만 GetObject 를 허용한다. OAC 가 원본을 비공개로 유지하는 구조 자체는 [s3-basics 의 OAC 절](/start/s3-basics/#cloudfront-oac--원본을-비공개로-유지하는-구조)에 있다.
...
요청 한 건이 통과하는 관문 둘이다. CloudFront 가 OAC 로 SigV4 서명을 붙여 S3 를 부르면 버킷 정책이 서비스 주체와 `AWS:SourceArn`(배포 ARN)을 확인한다. 객체가 SSE-KMS 라면 S3 가 이어서 KMS 에 복호화를 요청하고, 이번에는 키 정책이 `aws:SourceAccount` 를 확인한다. 둘 중 하나만 막혀도 사용자는 403 을 본다 — 버킷 정책만 고쳐 두고 키 정책을 빠뜨리는 실수가 여기서 드러난다.
...
**① 한 줄 정의** — WAF Web ACL은 priority(우선순위)가 매겨진 규칙(rule) 목록이다. 각 규칙은 매치 조건(statement)과 처리(action)를 갖는다. scope 는 붙이는 대상이 정한다 — CloudFront 에 붙이면 `CLOUDFRONT`(항상 us-east-1), ALB 에 직접 붙이면 `REGIONAL`(보호 대상과 같은 리전)이다.

**② 왜 필요한가 (채점 관점)** — 후보 세트 중 set-03·set-07 은 WAF를 채점하고 set-02 는 아예 없다. 그런데 확인 방식이 다르다.

set-03 mark 10-1 은 세 가지를 본다. 먼저 `aws wafv2 list-web-acls` 로 리소스 자체를 조회한다. 다음으로 SQLi·XSS payload(`booking_id=1' OR 1=1--`, `<script>alert(1)</script>`)를 실제로 던져 403 을 확인한다. 마지막으로 rate 규칙의 `Limit` 값을 API 로 읽어 200 이하인지 본다.
...
`visibility_config` 는 이 규칙의 관측 설정이고 세 필드가 필수다. `cloudwatch_metrics_enabled` 는 규칙별 매치 수를 CloudWatch 메트릭으로 내보낼지를 정한다. `metric_name` 은 그 메트릭에 붙일 이름이고, `sampled_requests_enabled` 는 매치된 요청 표본을 콘솔에서 볼 수 있게 남길지 정한다. 오탐 대응이 이 표본에서 시작하므로 두 값을 `true` 로 켜 둔다.
