...
베이스 이미지 선택이 만드는 결과를 갈라 놓았다. `scratch` 는 가볍지만 대가가 셋이다. ECR Basic 스캔이 안 되고, CA 번들이 없어 DynamoDB TLS 연결이 실패하며, zoneinfo 가 없어 시각이 UTC 로 저장된다 — 세 가지가 모두 채점 항목과 직접 부딪힌다. 고정 버전 alpine 은 셋을 각각 해결한다.
...
위 표의 "키 속성 이름" 줄은 한 칸짜리로 적혀 있지만 실제로는 이 모듈에서 가장 비싼 변경이다. [set-07 `dynamodb.tf`](https://github.com/ishs-cloud-computing/skills-2026/blob/main/set-07/task-1/terraform/dynamodb.tf)와 나란히 놓으면 이렇다.
...
**파티션 키는 만든 뒤에 못 바꾼다.** 테이블을 지우고 다시 만들어야 하고, 삭제 방지가 켜져 있으므로 **그것을 먼저 끄지 않으면 지워지지도 않는다.** 이 모듈에서 [06 의 VPC CIDR](/part-1/06-terraform-vpc/lab/)에 해당하는 값이 바로 이것이다.
...
`mark 4-1-A` 는 테이블 하나를 조회해 키·인덱스·암호화·삭제 방지를 한 번에 뽑는다. 그 조회를 그대로 해 본다. **읽기만 하므로 테이블을 만들거나 고치지 않는다.**
