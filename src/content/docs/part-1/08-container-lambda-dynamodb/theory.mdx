---
title: 08. 컨테이너 + Lambda + DynamoDB — 이론
...
선행 지식과 학습 경로는 [모듈 개요](/part-1/08-container-lambda-dynamodb/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

SSE(Server-Side Encryption)는 저장할 때 서버가 알아서 암호화하는 방식이다. 종류와 키 선택은 [s3-basics](/start/s3-basics/)에 있다.

이 모듈의 채점은 "실제 출력값 비교"가 많다(created_at 시각, JSON 키 순서, 비ASCII 문자, 평균값 자릿수). 인프라가 떠 있어도 출력이 1글자 다르면 0점 — 함정을 사실 단위로 암기한다.

:::tip[읽으면서 확인한다]
2절과 3절 끝에 **지금 확인하기** 블록이 접혀 있다. 조회와 로컬 검사뿐이라 **과금이 0원**이고 아무것도 만들지 않는다. 블록을 열지 않고 읽어도 문서는 그대로 이어진다.
:::
...
**① 한 줄 정의** — 베이스 이미지는 컨테이너의 OS 레이어다. 선택에 따라 취약점 스캔 가능 여부·TLS 신뢰 저장소·타임존 데이터가 결정된다. 이미지·레이어·`FROM` 같은 Dockerfile 기본기는 [docker-basics](/start/docker-basics/)에 있다.
...
베이스 이미지 선택이 만드는 결과를 갈라 놓았다. `scratch` 는 가볍지만 대가가 셋이다. ECR Basic 스캔이 안 되고, CA 번들이 없어 DynamoDB TLS 연결이 실패한다. zoneinfo 가 없어 시각도 UTC 로 저장된다. 셋 다 채점 항목과 직접 부딪힌다. 고정 버전 alpine 은 셋을 각각 해결한다.
...
| 바뀌는 값 | 고칠 곳 | 확인 |
|---|---|---|
| 앱 바이너리 이름 | `Dockerfile` 의 `COPY` · `ENTRYPOINT` | 두 줄이 같은 경로를 가리킨다 |
| 앱 포트 | `Dockerfile` 의 `EXPOSE` | 앱이 여는 포트와 같은 값 |
| 앱 언어·베이스 이미지 | `FROM alpine:3.24.1` 한 줄 | latest 금지 — 고정 태그 유지 |
| 스캔에 High 발생 | `RUN apk upgrade --no-cache` 추가 후 재빌드 | `describe-image-scan-findings` 에 CRITICAL/HIGH 키 없음 |
| KST 시각 요구 | `apk add tzdata` + `ENV TZ` 짝 | mark 9-2 의 created_at 비교 |

골격은 세트가 바뀌어도 같다 — 고정 버전 alpine, `tzdata` 와 `TZ` 짝, 비루트 실행, 기본 CloudShell(amd64) 빌드.
...
**④ 변형 포인트** — 이름과 태그는 과제지를 그대로 따라간다.

| 바뀌는 값 | 고칠 곳 | 확인 |
|---|---|---|
| 리포지토리 이름 | `ecr.tf` 의 `name` | push 주소와 `--repository-name` 이 같은 이름 |
| 이미지 태그 (`stable` / `v1.0.0`) | `docker build` · `docker push` 태그와 `--image-id imageTag=` | 요구된 태그만 push — `latest` 추가 금지 |
| ECR 전용 CMK 요구 | `encryption_configuration` 의 `kms_key` | 미지정이면 관리형 `aws/ecr` |

판단 기준은 과제지에 ECR 용 CMK 이름이 명시됐는지다. 이름이 있으면 CMK 를 만들어 `kms_key` 로 지정하고(set-03 `wsc2026-ecr-kms`), 없으면 `encryption_type = "KMS"` 만 두어 관리형 `aws/ecr` 를 쓴다(set-02).

### set-07 은 태그 정책을 요구한다 — 도구 버전이 걸린다 :badge[실측]{variant=note}

[set-07 `ecr.tf`](https://github.com/ishs-cloud-computing/skills-2026/blob/main/set-07/task-1/terraform/ecr.tf)는 위 셋에 하나를 더 건다. **"`latest` 를 제외한 태그는 덮어쓸 수 없다."**

태그 덮어쓰기 금지 자체는 `image_tag_mutability = "IMMUTABLE"` 한 줄이다. 문제는 **예외를 두는 것**이다. `latest` 는 원래 계속 옮겨 다니는 태그라 완전 금지로 두면 push 가 막힌다. 그래서 값이 `IMMUTABLE_WITH_EXCLUSION` 이 되고, 무엇을 예외로 할지 적는 블록이 따로 붙는다.

```hcl title="ecr.tf (set-07)"
...
**여기서 지금까지 없던 종류의 함정이 나온다.** 이 값과 블록은 비교적 최근에 추가된 것이라, 프로바이더가 낡으면 **문법이 맞는데도** `Unsupported argument` 나 `Blocks of type ... are not expected here` 로 막힌다. 과제지 요구가 코드가 아니라 **도구 버전을 올리게 만드는** 사례다.

<details class="build-step">
<summary>지금 확인하기 — 내 프로바이더가 이 블록을 아는가</summary>

프로바이더는 자기가 아는 인자 목록을 갖고 있고 그것을 물어볼 수 있다. **로컬에서만 돌고 AWS 를 호출하지 않는다.** `terraform init` 을 마친 디렉터리에서 실행한다.
...
기대: 네 줄이 나온다 — `encryption_configuration` · `image_scanning_configuration` · **`image_tag_mutability_exclusion_filter`** · `timeouts`.

세 번째가 없으면 프로바이더가 낡은 것이다. `terraform init -upgrade` 로 올린 뒤 다시 확인한다. **코드를 고칠 일이 아니다.**

이 명령이 `block_types` 를 묻는다는 점도 함께 본다. `image_tag_mutability` 는 값 하나짜리라 `attributes` 쪽에 있고, 예외 필터는 중첩 블록이라 `block_types` 쪽에 있다. **찾는 자리를 틀리면 "없다"는 잘못된 결론이 나온다.**

`jq: error` 나 `속성을 찾을 수 없습니다` 가 나오면 아직 `terraform init` 을 안 한 디렉터리다.
...
**④ 변형 포인트** — 조회 패턴이 바뀌면 GSI 축이 따라 바뀐다.

| 바뀌는 값 | 고칠 곳 | 확인 |
|---|---|---|
| 테이블 이름 | `dynamodb.tf` 의 `name` | Lambda 는 `aws_dynamodb_table.data.name` 참조라 무수정 |
| 키 속성 이름 | `hash_key` + 같은 이름의 `attribute` 블록 | 키로 쓰는 속성만 선언 — 비키 속성은 validate 에러 |
| 요구된 조회 패턴 | `global_secondary_index` 의 `key_schema` HASH·RANGE | 정렬 축이 되는 속성이 RANGE 키 |
| 인덱스 이름 | `global_secondary_index` 의 `name` | Lambda `INDEX_NAME` 환경 변수와 같은 값 |

"요구된 조회 패턴 → GSI HASH/RANGE 결정" 사고 과정은 세트가 바뀌어도 같다. 삭제방지·SSE·PITR 은 각각 별도 채점 항목이라 이름이 바뀌어도 함께 챙긴다.

### set-07 은 파티션 키가 다르다 :badge[실측]{variant=note}

"키 속성 이름" 줄은 표에서 한 칸으로 적혀 있지만 실제로는 이 모듈에서 가장 비싼 변경이다. [set-07 `dynamodb.tf`](https://github.com/ishs-cloud-computing/skills-2026/blob/main/set-07/task-1/terraform/dynamodb.tf)와 나란히 놓으면 이렇다.

| 항목 | set-02 | set-07 |
|---|---|---|
| 테이블 이름 | `wskorea26-data-table` | `unicorn-concert-db` |
| **파티션 키** | `client_id` | **`booking_id`** |
| GSI 이름 | `concert_name-created_at-index` | `client-id-created-at-index` |
| GSI HASH / RANGE | `concert_name` / `created_at` | `client_id` / `created_at` |
| PITR | 없음 | 있음 |
| 암호화 키 | `wskorea26-dynamodb-key` | app 용도 키 |

**조회 패턴이 뒤집혔다.** set-02 는 "예약 하나를 고객으로 찾고, 콘서트별 최신순은 인덱스로" 였다. set-07 은 "예약 하나를 예약 번호로 찾고, 고객별 최신순은 인덱스로" 다. 그래서 `client_id` 가 테이블의 파티션 키 자리에서 GSI 의 파티션 키 자리로 옮겨 간다.

**파티션 키는 만든 뒤에 못 바꾼다.** 테이블을 지우고 다시 만들어야 하고, 삭제방지가 켜져 있으므로 **그것을 먼저 끄지 않으면 지워지지도 않는다.** 이 모듈에서 [06 의 VPC CIDR](/part-1/06-terraform-vpc/lab/)에 해당하는 값이 바로 이것이다.

GSI 를 적는 문법도 두 가지가 함께 쓰인다.

```hcl
# set-02 — key_schema 블록을 둘 쓴다
global_secondary_index {
  name = "concert_name-created_at-index"
...
# set-07 — hash_key / range_key 로 줄여 쓴다
global_secondary_index {
...
둘은 같은 것을 만든다. 정답지가 어느 쪽을 쓰든 놀라지 않도록 양쪽을 읽을 수 있어야 한다.

<details class="build-step">
<summary>지금 확인하기 — 채점이 테이블에서 무엇을 뽑아 가나</summary>

`mark 4-1-A` 는 테이블 하나를 조회해 키·인덱스·암호화·삭제방지를 한 번에 뽑는다. 그 조회를 그대로 해 본다. **읽기만 하므로 테이블을 만들거나 고치지 않는다.**

테이블이 아직 없어도 된다 — 없을 때 나오는 에러를 보는 것도 이 블록의 일부다.
...
기대: 테이블이 있으면 `PK` · `GSI` · `Delete` 세 필드가 든 JSON 이 나온다. `PK` 가 곧 파티션 키 이름이고, 채점은 이 글자를 비교한다.

테이블이 없으면 `ResourceNotFoundException: Requested resource not found` 다. **`AccessDenied` 가 아니라는 점이 중요하다** — 권한은 있고 대상이 없다는 뜻이라, 진단할 때 이 둘을 구분하면 방향이 갈린다.

</details>

### 3-1. 접근 패턴에서 키 설계로 — 절차 :badge[실기출]{variant=note}

여기까지는 주어진 키를 그대로 만들었다. 실기출은 **키를 직접 설계하라**고 한다.

> order 애플리케이션 동작 설명을 기반으로 테이블을 설계하고 생성해야 합니다. (2024 1과제)

> 어플리케이션 동작과 트래픽 패턴에 맞게 PK, SK, Index 의 설계가 필요할 수 있습니다. (2025 3과제)

관계형 데이터베이스와 순서가 정반대다.

| | 관계형 | DynamoDB |
|---|---|---|
| 먼저 정하는 것 | 데이터 구조 | **접근 패턴** |
| 나중에 붙이는 것 | 질의 | 키 |
| 새 질의가 생기면 | 그냥 쓴다 | **인덱스가 필요할 수 있다** |

**질의를 나중에 자유롭게 쓸 수 없기 때문이다.** DynamoDB 는 키로만 효율적으로 찾는다. 그래서 무엇을 물어볼지부터 정하고 키를 거기에 맞춘다.

절차는 넷이다.

**1단계 — 앱이 무엇을 묻는지 적는다.** API 표와 바이너리에서 뽑는다([Go·Gin 앱 읽기](/start/go-app-basics/#3-2-라우트를-뽑는다)).

```
GET  /v1/product?id=xxx        -> id 로 하나 찾기
POST /v1/product               -> 하나 넣기
```
...
**3단계 — 가장 잦은 질의를 기본 키로 만든다.**

찾는 축이 `id` 하나이고 결과가 1건이면 **파티션 키 `id` 하나로 끝난다.** 정렬 키가 필요 없다.

**4단계 — 남은 질의를 GSI 로 만든다.**

기본 키로 답할 수 없는 질의가 있으면 그것이 GSI 다.

:::note[Scan 이 보이면 설계가 틀린 것이다]
운영 중 DynamoDB 지표에서 `Scan` 이 보이면 앱이 테이블 전체를 훑고 있다는 뜻이다. 항목이 늘수록 느려지고 비싸진다. 그 축으로 GSI 를 만들어야 한다.
:::
...
PK 는 정해 주었지만 나머지 둘은 성격을 알려 주지 않았다. `order` 앱이 "어느 고객의 주문 목록"을 물으면 `customerid` GSI 가, "어느 제품의 주문"을 물으면 `productid` GSI 가 필요하다. **앱 동작을 확인해야 안다.**

**2025 3과제 `product` 테이블**

문제지에는 테이블을 하나만 만들라는 말과 데이터 예시뿐이다. 그런데 바이너리를 열면 환경변수가 둘이다.
...
`TABLE_INDEX` 는 **문제지 어디에도 없다.** 앱이 GSI 이름을 받는다는 뜻이고, 곧 기본 키가 아닌 축으로도 조회한다는 뜻이다. 이 정보는 바이너리에서만 나온다.

### 3-3. 핫 파티션

2025 3과제 문제지가 성능에 관한 힌트를 준다.

> product API 는 제품 생성(POST 요청) 이후 해당 제품에 대한 정보는 변경되지 않으며 **동일한 id 에 대한 요청이 빈번하게 발생될 것으로 예상됩니다.**

두 가지를 말한다.

**첫째, 불변이다.** 갱신이 없으니 캐싱해도 낡은 값을 줄 위험이 없다.

**둘째, 같은 키가 반복된다.** DynamoDB 는 파티션 키의 해시로 데이터를 나눠 저장한다. 특정 키에 요청이 몰리면 그 파티션만 뜨거워진다.

| 대책 | 효과 | 대가 |
|---|---|---|
| DAX | 마이크로초 단위 캐시 | 클러스터 비용 |
| 앱 내 캐시 | 무료 | 인스턴스마다 따로 |
| **CDN 캐싱** | **DynamoDB 까지 안 온다** | 설정만 |

세 번째가 가장 크다. 불변 데이터라 안전하고, 응답 시간과 비용이 함께 좋아진다.

:::danger[POST 는 캐싱하지 않는다]
GET 만 캐싱한다. POST 를 캐싱하면 새로 만든 제품이 조회되지 않는다. 경로와 메서드로 캐시 정책을 나눈다([CloudFront 오리진 분기](/part-1/07-kms-s3-cloudfront/theory/#6-3-경로로-오리진을-가른다)).
:::

### 3-4. 용량 모드 고르기

| | 온디맨드(`PAY_PER_REQUEST`) | 프로비저닝 |
|---|---|---|
| 과금 | 요청 건수당 | 확보한 처리량당(시간) |
| 트래픽 예측 | 필요 없다 | 필요하다 |
| 급증 대응 | 자동 | 오토스케일링 설정 필요 |
| 유리한 때 | 트래픽을 모를 때, 간헐적일 때 | 꾸준하고 예측 가능할 때 |

**대회에서는 온디맨드가 기본 선택이다.** 트래픽 패턴을 모르고, 프로비저닝을 낮게 잡으면 스로틀이 나 요청이 실패한다.

2025 3과제 문제지도 "DynamoDB 의 스케일(Capacity)은 별도 제한이 없으나 너무 과도한 비용이 나오지 않도록" 이라고만 했다. 온디맨드로 두고 넘어가는 편이 안전하다.

:::caution[스로틀은 조용히 점수를 깎는다]
프로비저닝 모드에서 확보량을 넘기면 `ProvisionedThroughputExceededException` 이 나고 앱이 500 을 낸다. 가용성 점수가 그대로 떨어진다.
...
aws cloudwatch get-metric-statistics --namespace AWS/DynamoDB \
  ...
  --dimensions Name=TableName,Value=<테이블> \
...
**이름.** 앱이 환경변수로 받는 그 이름이어야 한다. 관례는 `<파티션키>-<정렬키>-index` 다.

**키.** 조회 축이 파티션 키, 정렬·범위 축이 정렬 키다.

**프로젝션.** 인덱스에 어느 속성을 복사해 둘지다.

| 값 | 담는 것 | 언제 |
|---|---|---|
| `KEYS_ONLY` | 키만 | 저장이 가장 적다. 본문이 필요하면 다시 조회 |
| `INCLUDE` | 키 + 지정한 속성 | 필요한 것만 |
| `ALL` | 전부 | **인덱스만으로 답이 끝난다** |

:::danger[프로젝션이 부족하면 조회가 두 번 간다]
`KEYS_ONLY` 인덱스로 찾으면 키만 나오고, 나머지 속성을 얻으려면 기본 테이블을 다시 읽어야 한다. 응답 시간이 두 배가 된다.

응답 시간이 채점 항목이면 **`ALL` 로 두는 편이 안전하다.** 저장 비용이 늘지만 실습·대회 규모에서는 무시할 만하다.
:::
...
**④ 변형 포인트** — 통합 대상이 바뀌면 응답 봉투가 통째로 바뀐다.

| 바뀌는 값 | 고칠 곳 | 확인 |
|---|---|---|
| 응답 필드·키 순서 | `lambda/index.py` 의 `_KEYS` | mark 9-2 기대 출력과 문자열 일치 |
| 통합 대상 (ALB / API Gateway) | `_response` 의 봉투 필드 | ALB 는 `statusCode`·`headers`·`isBase64Encoded` 필수 |
| 테이블·인덱스 이름 | Terraform `environment.variables` | 핸들러는 `os.environ` 으로 읽어 코드 무수정 |
| 조회 축·정렬 방향 | `KeyConditionExpression` · `ScanIndexForward` | 최신순은 `False` |

런타임 버전도 과제지를 따라간다. 권한은 조회 대상이 바뀌어도 3-statement 골격을 지킨다 — `dynamodb:Query`, 로그 2종, SSE-KMS 테이블의 `kms:Decrypt`. "env 주입 + Query + 정렬 + 응답 포맷 정확 일치" 원칙도 공통이다.
...
<Quiz title="테이블 설계를 어디서 시작하나">
"앱 동작에 맞게 테이블을 설계하라"는 요구를 받았다. 무엇부터 정하나?

- [x] 앱이 무엇을 묻는지 — 질의 목록을 먼저 적고 키를 거기에 맞춘다
- [ ] 저장할 데이터의 구조
  > 관계형의 순서다. DynamoDB 는 질의를 나중에 자유롭게 쓸 수 없어 순서가 반대다.
- [ ] 예상 트래픽에 맞는 용량 모드
  > 키가 정해진 뒤의 문제다. 대회에서는 온디맨드로 두고 넘어간다.
- [ ] 필요할 만한 GSI 를 미리 두세 개
  > 기본 키로 답할 수 없는 질의가 나온 뒤에 만드는 것이 GSI 다.

**접근 패턴이 먼저다.** 질의를 적고(1) 표로 정리해(2) 가장 잦은 것을 기본 키로 만든 뒤(3) 남은 것을 GSI 로 만든다(4). 운영 지표에 `Scan` 이 보이면 그 축의 인덱스가 없다는 신호다.

질의 목록은 문제지에만 있지 않다 — 바이너리의 라우트와 환경변수가 설계를 바꾼다([Go 앱 읽기](/start/go-app-basics/#3-2-라우트를-뽑는다)).
</Quiz>

<Quiz title="GSI 프로젝션과 응답 시간">
응답 시간이 채점 항목인 조회 API 에 GSI 를 붙인다. 프로젝션을 `KEYS_ONLY` 로 두면 어떻게 되나?

- [x] 인덱스에서 키만 나와, 본문을 얻으려 기본 테이블을 한 번 더 읽는다
- [ ] 조회가 실패한다
  > 실패하지 않는다. 느려질 뿐이라 오히려 찾기 어렵다.
- [ ] 저장 비용이 늘어난다
  > 반대다. 담는 것이 적어 저장은 가장 싸다. 대가로 내는 것은 시간이다.
- [ ] 정렬 키를 쓸 수 없게 된다
  > 키는 어느 프로젝션에서든 인덱스에 들어 있다.

**읽기가 두 번 간다.** 응답 시간이 걸린 항목이면 `ALL` 로 둔다 — 인덱스만으로 답이 끝난다. 저장이 늘지만 대회 규모에서는 무시할 만하다.
</Quiz>
