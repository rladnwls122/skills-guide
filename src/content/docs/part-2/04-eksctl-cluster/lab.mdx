...
앞 절반은 `terraform output` 을 환경 변수로 읽어 지금 셸에 올린다. 뒷 절반의 here-string 이 그 값들을 `set-02\task-1\.env.ps1` 로 굳힌다. 백틱 `` `$ `` 는 "지금 확장하지 말고 `$env:...` 라는 글자 그대로 파일에 쓰라"는 표시이고, 큰따옴표 안의 `$env:VPC_ID` 는 반대로 지금 값으로 바뀐다. 마지막 줄만 큰따옴표가 아니라 작은따옴표인 것은 Grafana 비밀번호에 `$` 가 들어가기 때문이다.

</details>

**검증**: `terraform output` 이 에러 없이 뜨고 `..\.env.ps1` 이 생겼으면 된다. 새 터미널로 이어서 할 땐 `set-02\task-1` 에서 `. .\.env.ps1` 한 줄만 다시 실행한다.
