...
graph TD
    subgraph addonNG["<span class='icon--logos icon--logos--aws-ec2'></span> wskorea26-addon-ng (label node-type=addon)"]
        core@{ icon: "logos:kubernetes", form: "square", label: "coredns", pos: "b", h: 46, w: 46 }
...
같은 `vpc` 블록에서 함께 챙길 것: `securityGroup`(CP ENI 추가 SG — 채점 CloudShell → private API 443 허용, 유의 13), `clusterEndpoints`(set-02 는 public+private, set-03 은 fully private).
</Quiz>

<Quiz title="instanceName vs tags.Name">
managed 노드그룹의 `tags` 는 EC2 인스턴스로 전파되지 않고, `tags.Name` 은 Launch Template 의 TagSpecifications 에 eksctl 기본 Name 과 중복 등록되는 함정까지 있다. EC2 **인스턴스**의 Name 태그를 실제로 지정하는 필드 이름은 [[instanceName]] 이다.
...
`${VAR}` 치환 누락 검사다. 하나라도 남으면 eksctl 이 이상한 에러로 죽는데, **생성에 약 20분**(CloudFormation 스택: 클러스터 → OIDC/SA → 노드그룹 순)이 걸리므로 되돌리는 비용이 가장 비싼 실수다.
