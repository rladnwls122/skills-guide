...
**graceful shutdown 흐름**: Pod 삭제 시작 → (동시에) 엔드포인트 제거 전파 & preStop 실행. `sleep 15` 동안 ALB/TGB가 타깃 등록 해제를 끝내므로 in-flight 요청 유실이 없다. 이후 SIGTERM. `terminationGracePeriodSeconds` 45초는 **preStop 시작 시점부터** 세므로 SIGTERM 이후 남는 시간은 약 30초다.
