...
- [x] Ingress 가 만드는 ALB 는 **이름을 지정할 수 없는데**, 채점은 `wskorea26-book-alb`·`wskorea26-book-tg` 같은 정확한 이름을 검사하기 때문
...
Ingress 가 만드는 ALB 는 이름이 자동 생성이라 지정할 수 없다. 이름 채점이 있는 수상 과제는 전부 **Terraform 으로 ALB/TG 이름 고정 생성 → k8s TGB 로 Pod IP 만 등록** 패턴이다.
...
ALB 리스너 규칙 2개가 검증하는 CloudFront 커스텀 헤더 `X-Origin-Verify` 의 값은 [[wskorea26-cf]] 이고, mark 7-2 는 `describe-rules` 의 `HttpHeaderConfig.Values[]` 출력이 정확히 [[2]] 줄일 것과 `curl http://<ALB>/book` 이 403 을 반환할 것을 검사한다.
...
graceful shutdown 흐름: Pod 삭제 시작 → (동시에) 엔드포인트 제거 전파 & preStop 실행 → `sleep 15` 동안 ALB/TGB 가 타깃 등록 해제를 끝냄 → SIGTERM → 최대 45초(`terminationGracePeriodSeconds`) 내 종료. 두 값은 항상 `preStop sleep < terminationGracePeriodSeconds` 여야 한다.
</Quiz>
