...
- 이미지는 CloudShell에서 build → tag → push. 세션이 초기화되므로 명령을 붙여넣기 가능한 형태로 준비 — [docker-basics](/skills-2026-learn_module/start/docker-basics/)
- `kubectl apply / get / describe / logs / rollout restart` — 검증 단계의 기본기 — [k8s-basics](/skills-2026-learn_module/start/k8s-basics/)
- `terraform output` 값을 `.env`로 영속화하는 `--query`·`jq` 패턴 — [awscli-basics](/skills-2026-learn_module/start/awscli-basics/)
- 클러스터 생성 신원 = 채점 신원. 런북 전체를 한 신원으로 관통한다 — [iam-basics](/skills-2026-learn_module/start/iam-basics/)
...
<LinkButton href="/skills-2026-learn_module/part-2/06-full-deploy-set02/theory/" icon="right-arrow">이론 읽기</LinkButton>
<LinkButton href="/skills-2026-learn_module/part-2/06-full-deploy-set02/lab/" variant="minimal" icon="rocket">실습 시작</LinkButton>
