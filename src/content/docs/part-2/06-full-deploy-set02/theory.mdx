...
| 관측성 | 있음 — Prometheus·Fluent Bit 이름 미지정(도구 자유), Grafana 대시보드 5개 패널만 지정. 배점 3.5/30 | 있음 — Prometheus+Alertmanager+Node Exporter+Fluent Bit+Grafana 스택 지정, 부하 생성 후 실시간 자동 채점. 배점 5.2/30 | 있음 — Fluent Bit+Prometheus+Grafana 스택 지정, 대시보드 패널 구성·배치까지 정밀 채점. 배점 5.0/30(Observability 2.0 + Grafana 1.5 + Log Pipeline Test 1.5) |
...
관측성은 **세 후보 전부에 있다** — "set-02만 있고 나머지는 없다"가 아니라, set-02는 도구를 자유롭게 고르고 수동 채점 위주라 부담이 적고, set-03·set-07은 Prometheus+Grafana+Fluent Bit 스택이 못박혀 있고 mark.sh가 부하를 직접 생성해 실시간으로 확인한다는 게 실제 차이다. WAF는 set-03·set-07 배점이 각각 1.5·2.5인데 관측성은 5.2·5.0이다 — WAF보다 관측성 쪽 배점이 더 크다.
...
|---|---|
| set-02 | 이 모듈 그대로 진행 |
| set-03 | [WAF](/part-1/02-kms-s3-cloudfront/theory/) 커스텀 매치 규칙 절 + 같은 모듈의 root-less KMS 심화 절 + [PART-3 모듈 07](/part-3/07-observability/theory/) 관측성 스택을 4시간 안에 압축할 순서 |
