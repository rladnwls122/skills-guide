---
title: 06. 관측성 이론 — 로그 파이프라인과 로그 기반 메트릭
...
선행 지식과 학습 경로는 [모듈 개요](/part-2/06-observability/)에.
...
Grafana 와 Prometheus 에 설정이 들어가는 세 경로다. 데이터소스 3종은 values 파일이 직접 넣고, 대시보드는 ConfigMap 에 `grafana_dashboard=1` 라벨을 붙이면 사이드카가 그 라벨을 감시하다 자동으로 import 한다. PrometheusRule 과 PodMonitor 는 셀렉터 설정 두 개(`...NilUsesHelmValues=false`)를 꺼야 릴리스 라벨이 없는 리소스까지 잡힌다 — 이 값을 안 바꾸면 알람과 메트릭이 조용히 누락된다.
