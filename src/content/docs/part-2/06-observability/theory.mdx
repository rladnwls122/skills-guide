...
| Fluent Bit 네임스페이스 | 배포 매니페스트의 ns (monitoring / observability / logging) | 채점이 그 ns 의 fluent-bit Running 수를 센다 |
| 인증 방식 (IRSA ↔ Pod Identity) | SA 와 IAM 연결 | 권한이 없으면 수집은 되고 전송만 실패한다 |
| HTTP 메트릭 소스 | log_to_metrics ↔ CloudWatch Exporter(ALB TargetResponseTime) | 앱이 `/metrics` 를 노출하는지로 갈린다 |
