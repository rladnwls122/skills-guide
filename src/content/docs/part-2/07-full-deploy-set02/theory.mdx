...
VPC CloudShell 의 용도는 따로 있다 — 9단계 채점(mark.sh)이다. private 엔드포인트 EKS API 에 닿아야 하므로 priv-subnet-d + `vpc-environment-sg` 로 띄운다. 파일 업로드가 안 되므로 스크립트는 **heredoc 붙여넣기**로 넣는다.
</Quiz>
