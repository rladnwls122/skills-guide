...
**② 왜 (채점 관점)** — set-02 런북만 외우고 가면 WAF·심화 KMS 문항 전체(set-03) 또는 WAF·관측성(set-07)을 놓친다. 세 세트 모두 EKS+ALB+CloudFront+DynamoDB+Lambda 뼈대는 같지만, 아래 표의 항목은 세트마다 있고 없고가 갈린다. 이 표를 못 봤다면 시험장에서 "이런 요구사항은 처음 본다"는 상태로 4시간을 시작하게 된다.
...
관측성은 **세 후보 전부에 있다** — "set-02만 있고 나머지는 없다"가 아니라, set-02는 도구를 자유롭게 고르고 수동 채점 위주라 부담이 적고, set-03·set-07은 Prometheus+Grafana+Fluent Bit 스택이 못박혀 있고 mark.sh가 부하를 직접 생성해 실시간으로 확인한다는 게 실제 차이다. set-02의 부담은 정정으로 한 번 더 줄었다 — "Pod 로그를 CloudWatch Logs로 전송" 문구가 최종 요구사항에서 빠진 잔여 문구로 확인되어 삭제됐고, 채점 대상이 아니다. **set-02가 선정되면 로그 파이프라인(Fluent Bit·CloudWatch)에 시간을 쓰지 않는다.** WAF는 set-03·set-07 배점이 각각 1.5·2.5인데 관측성은 5.2·5.0이다 — WAF보다 관측성 쪽 배점이 더 크다.
