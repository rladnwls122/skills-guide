...
선행 지식과 학습 경로는 [모듈 개요](/part-2/10-k8s-workloads-alb/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

CIDR 표기와 AZ(가용 영역)는 [vpc-basics](/start/vpc-basics/)에 있다. ARN 은 리소스의 전역 고유 이름이고([iam-basics](/start/iam-basics/)), IRSA 는 파드에 IAM 역할을 붙이는 방식이다([eks-basics](/start/eks-basics/)).
...
[09 이론](/part-2/09-eksctl-cluster/theory/)과 같이 **이 문서는 아무것도 만들지 않는다.** `--dry-run=server` 와 `helm template` 은 결과만 보여 주고 클러스터에 저장하지 않으므로 **과금이 0원**이다. 실제 배포는 [10 실습](/part-2/10-k8s-workloads-alb/lab/)이 한다.
:::
...
**① 한 줄 정의** — Deployment 는 Pod 복제본을 선언적으로 유지하는 컨트롤러다. 대회는 고가용성·노드 배치·무중단을 함께 만족하는 스펙을 요구한다.

Deployment·Pod·probe·ServiceAccount 는 [k8s-basics](/start/k8s-basics/)가 먼저 정의한다. Deployment 스펙에 나오는 `requests`/`limits` 와 QoS 등급은 [그 문서의 requests / limits 절](/start/k8s-basics/#2-6-리소스-요청과-상한--requests--limits)에 있다.

**② 왜 (채점 관점)** — mark 5-4 가 wskorea26 파드의 노드 배치(app)를 검사한다. mark 9-1~9-4 는 실제 애플리케이션 동작을 본다. probe가 없으면 죽은 Pod가 타깃에 남아 9번 항목이 간헐 실패하고, 노드 배치가 틀리면 5-4 감점.
...
Deployment 하나에 채점이 요구하는 축이 여섯 개 걸린다. 프로브는 역할이 갈린다 — readiness 는 트래픽을 받을지 정하고 liveness 는 재시작을 건다. 배치는 nodeSelector 로, AZ 분산은 topologySpread 로 정한다. 자원은 requests 와 limits 를 다르게 두어 버스트를 허용한다. 무중단 종료는 preStop 대기와 유예 시간의 조합이고, 신원은 IRSA 로 만든 서비스 계정이다.
...
변형 포인트 표의 값들이 실제로 받아들여지는지 API 서버에 물어본다. [09 실습](/part-2/09-eksctl-cluster/lab/)의 클러스터가 있어야 하고, `${ECR}` 을 치환한 `rendered/app/deployment.yaml` 이 있어야 한다. **저장하지 않으므로 파드는 뜨지 않는다.**
...
`--dry-run` 은 두 가지다. `client` 는 손안에서 YAML 문법만 본다. `server` 는 요청을 API 서버까지 보내 **필드 이름·타입·기본값 채우기·admission 검사까지** 거친 뒤 저장 직전에 버린다. 오타 난 필드 이름은 `client` 가 통과시키고 `server` 가 잡는다.
...
**② 왜** — LBC 나 kube-prometheus-stack 은 manifest 가 수십 장이다. helm 없이는 대회 시간 안에 못 세운다.
...
**① 한 줄 정의** — LBC(AWS Load Balancer Controller)는 클러스터 안의 k8s 리소스를 지켜본다. 그 선언을 보고 ELB API 를 대신 호출하는 컨트롤러다.
...
ALB·타깃그룹·헬스체크가 무엇인지는 [loadbalancer-basics](/start/loadbalancer-basics/)에 있다. "Ingress 로 만들게 하는 길" 대신 "직접 만들고 등록만 맡기는 길"을 고르는 이유가 있다. [그 문서의 길 B 절](/start/loadbalancer-basics/#5-3-길-b--내가-만들고-등록만-맡긴다-targetgroupbinding)이 두 방식을 나란히 놓고 설명한다.
...
ALB 를 Terraform 이 만들고 파드 연결만 클러스터가 맡는 구조다. Terraform 이 이름 고정으로 ALB 와 타깃그룹을 만들어 ARN 을 내준다. 그다음 TargetGroupBinding 이 "이 Service 를 저 타깃그룹에"를 선언한다. 실제 등록·해제를 수행하는 것은 LBC 이고, 그래서 LBC 가 앱보다 먼저 떠 있어야 한다. Service 는 트래픽 경로가 아니라 파드 목록의 출처다 — `targetType: ip` 라 타깃그룹에는 파드 IP 가 직접 들어간다.
...
`serviceAccount.create=false` 와 `serviceAccount.name` 이 IRSA 와 짝이다. eksctl 이 ClusterConfig 의 `iam.serviceAccounts` 로 서비스 계정과 IAM 역할을 이미 만들어 두었다. 여기서 helm 이 같은 이름의 계정을 다시 만들면 IRSA 애노테이션이 없는 빈 계정으로 덮인다. 그래서 만들지 말고 이름만 가리키게 한다.
...
이 한 줄이 IRSA 와 짝인 이유가 여기서 보인다. eksctl 이 ClusterConfig 로 만든 서비스 계정에는 IAM 역할 애노테이션이 붙어 있다. helm 이 같은 이름의 계정을 다시 만들면 그 애노테이션이 없는 빈 계정으로 덮인다. 파드는 뜨지만 AWS 호출만 거부된다.
...
**① 한 줄 정의** — 리스너(80/HTTP)의 기본 액션은 fixed-response 403 이다. 규칙 2개가 CloudFront 커스텀 헤더를 검증해, POST 는 Pod TG 로 그 외는 Lambda TG 로 보낸다.
...
리스너 하나가 요청을 세 갈래로 나눈다. priority 10 은 헤더와 메서드를 함께 보고 POST 를 파드 타깃그룹으로 보낸다. priority 20 은 나머지 헤더 일치 요청을 Lambda 타깃그룹으로 보낸다. 어느 규칙에도 안 걸리면 기본 액션이 403 을 고정 응답한다 — CloudFront 를 거치지 않은 직접 접근이 여기로 떨어진다. 규칙은 번호가 작은 것부터 평가되므로 두 규칙의 순서를 바꾸면 POST 도 Lambda 로 간다.
...
## 5. 파드끼리 못 만나게 하기 — NetworkPolicy :badge[실기출]{variant=note}

지금까지는 **밖에서 들어오는** 트래픽을 다뤘다. [2024 실기출 1과제](/reference/past-exams/#3-4-2024-1과제--web-service-provisioning-4시간)는 **안에서 옆으로 가는** 트래픽을 막으라고 했다.

> 네트워크 격리를 위해 Calico 를 사용합니다. **애플리케이션 컨테이너끼리는 쿠버네티스 네트워크 내에서 서로 통신이 불가능**해야 합니다. 외부로의 DNS, HTTP, HTTPS 접근은 허용합니다.

### 5-1. 기본값이 전부 열림이다

쿠버네티스는 기본적으로 **모든 파드가 모든 파드와 통신할 수 있다.** 보안 그룹처럼 막혀 있다가 여는 것이 아니라, 열려 있다가 막는 구조다.

그래서 `customer` 파드에서 `order` 파드로 `curl` 하면 그냥 된다. 요구는 그것을 막으라는 것이다.

### 5-2. NetworkPolicy 는 선택된 파드에만 적용된다
...
spec:
  podSelector:
    matchLabels:
      wsi: skills          # 이 라벨을 가진 파드에 적용
  policyTypes:
    - Ingress
    - Egress
  # ingress 를 비워 두면 들어오는 것이 전부 차단
  ingress: []
  egress:
    # DNS 는 허용해야 한다 — 막으면 이름 해석이 죽는다
    - to:
        - namespaceSelector: {}
          podSelector:
            matchLabels:
      ...
      ports:
        ...
        - protocol: TCP
          port: 53
    # 외부로 나가는 HTTP·HTTPS 는 허용
    - to:
        - ipBlock:
            cidr: 0.0.0.0/0
            except:
              - 10.0.0.0/16     # VPC 안쪽은 제외 — 곧 파드끼리는 여전히 차단
      ports:
        - protocol: TCP
          port: 80
        - protocol: TCP
...
**첫째, `podSelector` 가 대상을 고른다.** 라벨이 맞는 파드에만 걸린다. 라벨이 없는 파드는 여전히 전부 열려 있다.

**둘째, `policyTypes` 에 적힌 방향만 통제한다.** `Ingress` 만 적으면 나가는 것은 건드리지 않는다.

**셋째, 정책이 하나라도 걸리면 그 방향은 기본 차단이 된다.** 그래서 허용할 것을 명시적으로 적어야 한다.

:::danger[DNS 를 막으면 전부 멈춘다]
`egress` 를 통제하면서 UDP·TCP 53 을 허용하지 않으면 파드가 이름을 해석하지 못한다. 외부 API 호출도, 서비스 이름 접근도 전부 실패한다.

증상이 "아무것도 안 되는데 오류가 이상하다"로 나타나 원인을 찾기 어렵다. **`egress` 정책을 쓰면 DNS 허용을 가장 먼저 적는다.**
:::

:::caution[`except` 로 VPC 대역을 빼는 것이 요구의 핵심이다]
`0.0.0.0/0` 을 그냥 허용하면 파드 IP 도 그 안에 포함돼 파드끼리 통신이 다시 열린다. VPC CIDR 을 `except` 로 빼야 "외부는 되고 안쪽은 안 되는" 상태가 된다.
:::

### 5-3. VPC CNI 로는 안 되고 Calico 가 필요한 이유

EKS 기본 네트워크 플러그인인 VPC CNI 는 **NetworkPolicy 를 오래 지원하지 않았다.** 매니페스트를 적용해도 아무 일이 일어나지 않고 오류도 나지 않는다. 그래서 문제지가 Calico 를 명시한다.
...
:::danger[정책이 적용되지 않아도 조용하다]
NetworkPolicy 를 강제할 플러그인이 없으면 `kubectl apply` 는 성공하고 `kubectl get networkpolicy` 도 목록을 보여 준다. **그런데 실제로는 아무것도 막히지 않는다.**

"만들었다"와 "동작한다"가 다른 대표적인 사례다. 반드시 실제로 통신을 시도해 확인한다.
:::
...
```bash
POD=$(kubectl get pod -l wsi=skills -n skills --no-headers -o custom-columns=":metadata.name" | grep customer | head -n 1)

# 파드끼리 — 실패해야 한다
kubectl exec -it $POD -n skills -- curl -X GET "http://wsi-customer-service.skills.svc.cluster.local/healthcheck" --max-time 10

# 외부 HTTP — 성공해야 한다
kubectl exec -it $POD -n skills -- curl -X GET "http://naver.com" --max-time 10
```

**두 결과가 반대여야 한다.** 둘 다 되면 정책이 적용되지 않은 것이고, 둘 다 안 되면 너무 넓게 막은 것이다.

:::note[`--max-time` 이 붙어 있는 이유]
차단된 연결은 오류를 내지 않고 그냥 응답이 오지 않는다. 시간 제한이 없으면 명령이 멈춘 채로 있는다. 채점 스크립트도 이 옵션을 붙인다.
:::

### 5-5. 세 계층을 구분한다

같은 "막는다"라도 계층이 셋이고 서로를 대체하지 않는다.

| 계층 | 도구 | 막는 것 |
|---|---|---|
| 네트워크 | NetworkPolicy · 보안 그룹 | 패킷이 오가는 것 |
| API | RBAC | `kubectl` 로 오브젝트를 다루는 것 |
| VPC 경계 | Network Firewall | 밖으로 나가는 트래픽의 내용 |

[2024 1과제](/reference/past-exams/#3-4-2024-1과제--web-service-provisioning-4시간)는 셋 중 둘을 함께 요구했다 — Calico 로 네트워크를, assume-role 로 API 를 나눴다. 하나를 막아도 다른 하나는 열려 있으므로 각각 구성해야 한다. API 계층은 [13. Private EKS·IAM 주제 6](/part-3/13-private-eks-iam/theory/#주제-6-리눅스-계정을-iam-역할로-갈라-kubectl-권한을-나눈다)에 있다.
...
<Quiz title="정책을 넣었더니 파드가 전부 멈췄다">
파드끼리 통신을 막으려고 `policyTypes` 에 `Ingress` 와 `Egress` 를 넣고 egress 에 TCP 80·443 만 허용했다. 그랬더니 외부 호출까지 전부 실패한다. 빠진 것은?

- [x] UDP·TCP 53 — DNS 허용
- [ ] `ingress` 에 `0.0.0.0/0`
  > 들어오는 것을 막는 것이 요구다. 열면 요구가 깨진다.
- [ ] 파드 라벨 수정
  > 라벨이 안 맞으면 정책이 적용되지 않아 증상이 "전부 된다"로 나온다. 지금은 적용됐고 너무 좁은 것이다.
- [ ] Calico 재설치
  > 정책이 실제로 걸렸다는 증거가 이 증상이다. 강제자가 없으면 아무것도 막히지 않는다.

**DNS 다.** egress 를 통제하는 순간 이름 해석도 통제 대상이 된다. 53 을 열지 않으면 주소를 얻지 못해 80·443 허용이 소용없고, 서비스 이름 접근까지 함께 죽는다.

증상이 "아무것도 안 되는데 오류가 이상하다"로 나타나 원인을 찾기 어렵다. **`egress` 정책을 쓰면 DNS 허용을 가장 먼저 적는다.**
</Quiz>

<Quiz title="0.0.0.0/0 을 열었더니 격리가 풀렸다">
egress 에 `ipBlock: 0.0.0.0/0` 으로 80·443 을 허용했다. 외부는 잘 나가는데 파드끼리도 다시 통신된다. 왜인가?

- [x] 파드 IP 도 `0.0.0.0/0` 안에 있다 — VPC CIDR 을 `except` 로 빼야 한다
- [ ] `ingress: []` 가 무시됐다
  > 빈 배열은 그 방향 전부 차단이라는 올바른 표기다. 다만 정책은 상대 파드에는 걸리지 않는다.
- [ ] 서비스 이름으로 부르면 정책이 적용되지 않는다
  > 이름은 DNS 로 파드·서비스 IP 가 된다. 정책은 그 IP 로 판단한다.
- [ ] 80·443 말고 전체 포트를 열어야 한다
  > 방향이 반대다. 지금 문제는 너무 넓게 열린 것이다.

**대역이 겹친다.** `except` 로 VPC CIDR 을 빼야 "밖으로는 되고 안쪽으로는 안 되는" 상태가 된다. 이 한 줄이 요구의 핵심이다.
</Quiz>

<Quiz title="정책을 적용했는데 아무것도 안 막힌다">
`kubectl apply` 가 성공했고 `kubectl get networkpolicy` 에도 목록이 나온다. 그런데 파드끼리 `curl` 이 그대로 된다. 가장 먼저 볼 것은?

- [x] 정책을 강제할 플러그인이 있는지 — `kubectl get pods -n calico-system`
- [ ] 매니페스트의 오타
  > 형식이 틀리면 `apply` 에서 거절된다. 통과했다면 문법은 맞다.
- [ ] 정책이 퍼지기를 기다린다
  > 지연으로 설명할 시간이 아니다. 강제자가 없으면 영원히 안 막힌다.
- [ ] `podSelector` 를 빈 값으로 바꾼다
  > 대상 범위를 넓히는 것이지 강제 여부와는 관계없다.

**강제할 플러그인이다.** EKS 기본 VPC CNI 는 NetworkPolicy 를 오래 지원하지 않았고, 없으면 **오류 없이 조용히 무시된다.** 그래서 문제지가 Calico 를 명시한다.

"만들었다"와 "동작한다"가 다른 대표 사례다. 확인은 목록이 아니라 실제 통신으로 한다.
</Quiz>
