...
위 표의 값들이 실제로 받아들여지는지 API 서버에 물어본다. [09 실습](/part-2/09-eksctl-cluster/lab/)의 클러스터가 있어야 하고, `${ECR}` 을 치환한 `rendered/app/deployment.yaml` 이 있어야 한다. **저장하지 않으므로 파드는 뜨지 않는다.**
