...
**② 왜 (채점 관점)** — 30점 중 CloudFront 6.5 + Application 6 + EKS 5 = 17.5점이 이 그림 위에서 결정된다. 외울 것은 상자가 아니라 화살표다. 구간마다 **인증 방식**이 다르고, 그 방식 하나하나가 채점 항목이다. IRSA 는 파드에 AWS 권한을 주는 방식이다. 원리는 [eks-basics 의 IRSA 절](/start/eks-basics/#irsa와-eks-pod-identity)에 있다.
...
위 표는 "세트가 바뀌면 무엇이 다른가"를 항목 단위로 본다. PART-1·2 의 각 모듈은 그것을 **손으로 옮기는 훈련**으로 나눠 이미 한 번씩 시켰다.
