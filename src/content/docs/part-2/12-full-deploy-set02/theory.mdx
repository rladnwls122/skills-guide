---
title: 12. 이론 — set-02 전체 아키텍처와 CloudFront
...
선행 지식과 학습 경로는 [모듈 개요](/part-2/12-full-deploy-set02/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.
...
**① 한 줄 정의** — 콘서트 예매 플랫폼이고 홉이 넷이다. CloudFront 가 정적은 S3 로, 동적(`/book*`)은 ALB 로 보낸다. ALB 는 POST 를 Pod 로, GET 을 Lambda 로 가른다. 저장소는 DynamoDB 다.

**② 왜 (채점 관점)** — 30점 중 17.5점이 이 그림 위에서 결정된다. CloudFront 6.5, Application 6, EKS 5 다. 외울 것은 상자가 아니라 화살표다. 구간마다 **인증 방식**이 다르고, 그 방식 하나하나가 채점 항목이다. IRSA 는 파드에 AWS 권한을 주는 방식이다. 원리는 [eks-basics 의 IRSA 절](/start/eks-basics/#irsa와-eks-pod-identity)에 있다.
...
S3 쪽 화살표에 인증 수단이 둘인 이유는 목적이 다르기 때문이다. 접근 통제는 OAC 혼자 한다 — 버킷 정책이 보는 것은 서명과 `AWS:SourceArn` 이지 헤더가 아니다. 커스텀 헤더 `wskorea26-s3-access: true` 는 과제지가 "S3 로 전달하는 요청에는 이 헤더가 포함되어야 한다"고 지정한 값이다. mark 8-4 가 배포의 커스텀 헤더 목록을 **정확히 2줄**로 비교한다. 즉 이 헤더는 보안 장치가 아니라 채점되는 구성값이므로 이름·값·개수를 그대로 맞춘다.

---

## 2. CloudFront 핵심 — 이중 Origin과 경로 재작성

**① 한 줄 정의** — 배포는 origin 을 둘 가진다 — ALB 커스텀 origin 과 S3 OAC origin 이다. viewer-request 자리의 CloudFront Function 이 `POST /book` 만 `/v1/book` 으로 재작성한다.

S3 쪽 오리진이 OAC 로 비공개를 유지하는 구조는 [s3-basics 의 OAC 절](/start/s3-basics/#cloudfront-oac--원본을-비공개로-유지하는-구조)에 있다. ALB 쪽이 커스텀 헤더로 CloudFront 경유만 받는 구조는 [loadbalancer-basics 3-1 절](/start/loadbalancer-basics/#3-1-cloudfront-를-거친-요청만-받는다)에 있다.
...
ALB 도 2025년 10월부터 리스너 규칙에서 URL·Host 헤더 재작성을 지원하므로 기술적으로 유일한 해법은 아니다. 그래도 CloudFront 쪽에 두는 이유가 둘이다. `/book*` 동작의 분기(POST→ALB, GET→Lambda)와 재작성 지점이 한곳에 모인다. 그리고 `X-Origin-Verify` 검사를 하는 ALB 규칙을 건드리지 않아도 된다.
:::
...
CloudFront Function 이 URI 를 고쳐도 오리진이 바뀌지 않는 이유다. viewer-request 단계에서 POST `/book` 을 `/v1/book` 으로 바꾼다. 그 시점에는 이미 `/book*` 동작이 선택된 뒤라 캐시 동작을 다시 고르지 않는다. 그래서 재작성된 요청도 ALB 로 간다. GET 은 조건에 걸리지 않아 원래 경로 그대로 가고, ALB 에서 규칙 우선순위에 따라 Lambda 로 넘어간다.
...
**① 한 줄 정의** — 의존성 역순으로는 아무것도 못 만든다. 순서는 인프라(terraform)·이미지·클러스터·컨트롤러·앱·관측성·검증·채점·정리 아홉 단계다.
...
**① 한 줄 정의** — 이 모듈과 PART-2 는 set-02 로 고정돼 있다. 그런데 실제 1과제는 경기 시작 직전에 수상 후보 **세 편(set-02·set-03·set-07) 중 하나**가 선정된다.

**② 왜 (채점 관점)** — set-02 런북만 외우고 가면 놓치는 것이 있다. set-03 이면 WAF 와 심화 KMS 문항 전체, set-07 이면 WAF 와 관측성이다.
...
**관측성** — 세 후보 전부에 있다. 차이는 있고 없고가 아니라 채점 방식이다. set-02 는 Prometheus 도구 이름을 지정하지 않고 Grafana 패널 5개만 요구해 수동 채점 위주라 부담이 적다. set-03 은 Prometheus·Alertmanager·Node Exporter·Fluent Bit·Grafana 스택이 못박혀 있다. mark.sh 가 부하를 직접 만들어 실시간으로 확인한다. set-07 은 Fluent Bit·Prometheus·Grafana 스택에 더해 대시보드 패널 구성과 배치까지 정밀 채점한다. 5.0점이 Observability 2.0, Grafana 1.5, Log Pipeline Test 1.5 로 쪼개져 있다.

set-02 의 관측성 부담은 정정으로 한 번 더 줄었다. "Pod 로그를 CloudWatch Logs 로 전송" 문구가 최종 요구사항에서 빠진 잔여 문구로 확인되어 삭제됐다. **set-02 가 선정되면 로그 파이프라인(Fluent Bit·CloudWatch)에 시간을 쓰지 않는다.**

**KMS 키 정책** — set-02 와 set-07 은 키 정책에 root 위임을 그대로 쓴다. set-07 만 WAF 로그용 Platform 키 하나를 us-east-1 에 MRK 로 더 둔다. set-03 은 root principal 과 `kms:*` 를 둘 다 금지하므로 관리 액션을 개별 나열하고 KMS 의 lockout safety check 를 통과시켜야 한다. 그 구현은 [PART-1 모듈 07 이론 3절](/part-1/07-kms-s3-cloudfront/theory/)에 있다.

**기타** — set-03 은 Software Stack 목록에 EC2 가 적혀 있다. 다만 본문 요구사항은 EKS 노드그룹으로 충족되므로 별도 EC2 를 만들지 않는다. set-07 은 External ID 를 요구하고 최대 세션을 1시간으로 제한하는 Audit IAM Role 절이 따로 있다.
...
|---|---|
| set-02 | 이 모듈 그대로 진행 |
| set-03 | [WAF](/part-1/07-kms-s3-cloudfront/theory/) 커스텀 매치 규칙 절 + 같은 모듈의 root-less KMS 심화 절 + [PART-2 모듈 11](/part-2/11-observability/theory/) 관측성 스택을 4시간 안에 압축할 순서 |
...
### 이 커리큘럼은 set-02 → set-07 이행을 이미 훈련시켰다 :badge[실측]{variant=note}

세트 대조표는 "세트가 바뀌면 무엇이 다른가"를 항목 단위로 본다. PART-1·2 의 각 모듈은 그것을 **손으로 옮기는 훈련**으로 나눠 이미 한 번씩 시켰다.

| 모듈 | 옮긴 것 |
|---|---|
| [06](/part-1/06-terraform-vpc/lab/) | CIDR 규칙 계산 · 3 AZ 6 서브넷 · VPC Endpoint · Flow Log |
| [07](/part-1/07-kms-s3-cloudfront/lab/) | 키 분류 축(리소스별 → 용도별) · 90일 회전 · 다중 리전 키 · 버킷 버전 관리 · WAF 신설 |
| [08](/part-1/08-container-lambda-dynamodb/lab/) | 파티션 키 · GSI 축 · PITR · 태그 정책 · 로그 그룹 지정 |
| [09](/part-2/09-eksctl-cluster/lab/) | IRSA → Pod Identity · private 클러스터 · 3 AZ · CMK 볼륨 · KST |
| [10](/part-2/10-k8s-workloads-alb/lab/) | 인터넷 ALB → internal ALB + VPC Origin · SA 직접 생성 |
| [11](/part-2/11-observability/lab/) | 수집 범위 · 로그 형식 · 저장소 · 네임스페이스 분리 |

**그래서 이 모듈에서 새로 배울 것은 없다.** 여기서 하는 것은 여섯 조각을 **한 번에, 시간 안에** 하는 것이고, 그때 무너지는 것은 지식이 아니라 순서다.

<details class="build-step">
<summary>지금 확인하기 — 지금 계정에 무엇이 떠 있나 한 장으로</summary>

3절의 배포 순서 도식이 머릿속 그림이라면, 이것은 실제다. 태그가 붙은 리소스를 계정 전체에서 긁어 **서비스별로 센다.** 조회만 하므로 과금이 없다.
...
기대: `ec2` · `eks` · `kms` · `elasticloadbalancing` 같은 서비스 이름 옆에 개수가 붙는다. 아무것도 안 띄운 계정이면 빈 출력이다.

`ec2` 개수가 압도적으로 큰 것이 정상이다 — 서브넷·라우트 테이블·보안 그룹·ENI·EIP 가 전부 `ec2` 로 잡힌다. **도식에서 상자 하나로 그린 VPC 가 실제로는 수십 개 리소스라는 것이 여기서 보인다.**

**이 명령은 태그가 붙은 것만 센다.** 태그 없는 리소스는 목록에 없다. 그래서 이 출력이 비어 있다고 계정이 깨끗한 것은 아니고, 정리 확인은 [잔존 리소스 점검](/reference/cleanup-check/)이 따로 한다. 뒤집어 보면 **채점이 `Name` 태그로 리소스를 찾는 이유**도 같다 — 태그가 곧 이름표다.

`AccessDeniedException` 이면 이 API 권한이 없는 것이다. 조회 자체가 안 되므로 정리 확인은 서비스별 `describe-*` 로 대신한다.
