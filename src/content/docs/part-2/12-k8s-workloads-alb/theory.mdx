...
선행 지식과 학습 경로는 [모듈 개요](/part-2/12-k8s-workloads-alb/)에.
...
[11 이론](/part-2/11-eksctl-cluster/theory/)과 같이 **이 문서는 아무것도 만들지 않는다.** `--dry-run=server` 와 `helm template` 은 결과만 보여 주고 클러스터에 저장하지 않으므로 **과금이 0원**이다. 실제 배포는 [12 실습](/part-2/12-k8s-workloads-alb/lab/)이 한다.
:::
...
위 표의 값들이 실제로 받아들여지는지 API 서버에 물어본다. [11 실습](/part-2/11-eksctl-cluster/lab/)의 클러스터가 있어야 하고, `${ECR}` 을 치환한 `rendered/app/deployment.yaml` 이 있어야 한다. **저장하지 않으므로 파드는 뜨지 않는다.**
