---
title: 07. 관측성 — Fluent Bit + kube-prometheus-stack + Grafana
description: "로그 파이프라인, log_to_metrics, Grafana provisioning, 알람 6종의 학습 목표."
---
...
- ConfigMap으로 설정을 주입하는 두 방법(환경변수 / 볼륨 마운트) — Fluent Bit·Grafana 설정이 전부 이 방식 — [k8s-basics](/skills-2026-learn_module/start/k8s-basics/)
- Helm `install`/`upgrade --install`과 values 오버라이드 — [k8s-basics](/skills-2026-learn_module/start/k8s-basics/)
- 라벨 셀렉터 — sidecar가 `grafana_dashboard=1` 라벨로 대시보드를 찾는다 — [k8s-basics](/skills-2026-learn_module/start/k8s-basics/)
- 파이프·`grep`·`jq`로 로그에서 필요한 줄만 뽑는 원라이너 — [shell-basics](/skills-2026-learn_module/start/shell-basics/)
...
<LinkButton href="/skills-2026-learn_module/part-3/07-observability/theory/" icon="open-book">이론 읽기</LinkButton>
<LinkButton href="/skills-2026-learn_module/part-3/07-observability/lab/" variant="minimal" icon="rocket">실습 시작</LinkButton>
