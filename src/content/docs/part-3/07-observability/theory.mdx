...
flowchart LR
    ...
    C@{ icon: "mdi:script-text-outline", form: "square", label: "Fluent Bit<br/>DaemonSet", pos: "b", h: 46, w: 46 }
...
flowchart TD
    subgraph 노드["<span class='icon--logos icon--logos--aws-ec2'></span> 노드"]
        ...
        FB@{ icon: "mdi:script-text-outline", form: "square", label: "fluent-bit 컨테이너<br/>image: aws-for-fluent-bit", pos: "b", h: 46, w: 46 }
    end
...
flowchart TD
    ...
    CM["ConfigMap wsc2026-grafana-dashboard<br/>(dashboard.json) + label grafana_dashboard=1"] -->|sidecar가 라벨 watch| SC
...
- [ ] Grafana cloudwatch datasource로 CloudWatch Logs Insights 쿼리를 알람 조건에 건다
  > 알람은 PrometheusRule(PromQL)로 평가된다. Grafana datasource는 대시보드 조회용이지 Prometheus 룰의 입력이 아니다.
