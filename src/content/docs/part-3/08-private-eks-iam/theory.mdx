...
"private 클러스터"는 느낌이 아니라 두 불리언 값이다. [PART-2](/part-2/04-eksctl-cluster/lab/)에서 만든 클러스터가 있으면 그대로 물어본다. **조회만 하므로 아무것도 안 바뀐다.**
...
| 같은 문자열 | kps Alertmanager 의 `clusterDomain`(모듈 06) | helm values 가 같은 값 |
...
모듈 02·06 의 CloudFront 내용과 교차 확인한다.
