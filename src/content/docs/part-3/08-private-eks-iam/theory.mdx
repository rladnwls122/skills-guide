...
```mermaid
flowchart LR
    subgraph private-subnet["<span class='icon--mdi icon--mdi--lan-connect'></span> 프라이빗 서브넷 노드"]
        N[노드/파드]
    end
...
flowchart LR
    ...
    CS@{ icon: "mdi:console-line", form: "square", label: "VPC CloudShell / bastion<br/>kubectl·helm·채점", pos: "b", h: 46, w: 46 }
    S3 -->|"aws s3 cp + tar xzf"| CS
    CS2@{ icon: "mdi:console-line", form: "square", label: "일반 CloudShell<br/>docker build → ECR push", pos: "b", h: 46, w: 46 }
    CS2 --> ECR[(ECR)]
...
flowchart TD
    ...
    B@{ icon: "logos:kubernetes", form: "square", label: "② CoreDNS Corefile 패치<br/>(k8s manifest로 ConfigMap 교체<br/>+ rollout restart)", pos: "b", h: 46, w: 46 }
    B --> C
    C --> D["nslookup kubernetes.default.svc.wsc2026.skills.local 성공"]
    E@{ icon: "logos:kubernetes", form: "square", label: "coredns addon 업데이트", pos: "b", h: 46, w: 46 }
    E -.->|Corefile 초기화!| B
```
