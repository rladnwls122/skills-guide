...
본 PC·VPC CloudShell·일반 CloudShell 3분할과 bastion 패턴이 그대로 공인된 셈이다. 다만 "채점 전 bastion 삭제"는 세트별 과제지의 미요구 리소스 정리 원칙에서 나온 것이므로, 과제지가 bastion을 언급하면 그 지시를 우선한다.
...
네 고리다. IAM 역할을 맡는 것까지가 AWS 쪽이고, 그 역할이 클러스터에 등록돼 어느 그룹으로 매핑되는지, 그 그룹에 무슨 동사가 허용되는지가 쿠버네티스 쪽이다.
