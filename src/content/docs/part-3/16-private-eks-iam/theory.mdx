---
title: 16. 이론 — Fully-Private EKS 운용과 IAM 심화
...
선행 지식과 학습 경로는 [모듈 개요](/part-3/16-private-eks-iam/)에.
...
"private 클러스터"는 느낌이 아니라 두 불리언 값이다. [PART-2](/part-2/11-eksctl-cluster/lab/)에서 만든 클러스터가 있으면 그대로 물어본다. **조회만 하므로 아무것도 안 바뀐다.**
...
- kps의 Alertmanager도 `clusterDomain` 설정이 있어 함께 맞춘다(모듈 13 연계).
...
| 같은 문자열 | kps Alertmanager 의 `clusterDomain`(모듈 13) | helm values 가 같은 값 |
