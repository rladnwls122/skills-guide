...
평균 계산은 반드시 `decimal.Decimal`. boto3가 DynamoDB에 float 저장 자체를 거부하기 때문이다 — `TypeError: Float types are not supported. Use Decimal types instead.` 값이 틀려서가 아니다(`483/5`는 파이썬 float로도 정확히 `96.6`이다). 저장 시점에 예외로 죽는다.
:::
...
근거: set-08 task-2 과제지, 2026-08-01 정정본의 판정 기준, `asgmt2_module3_check.sh`. 정답 구현은 `skills-2026`의 브랜치 `set-08/task-2`, `set-08/task-2/module-3-event-handling`에 있다.
