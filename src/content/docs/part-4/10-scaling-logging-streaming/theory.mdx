...
선행 지식과 학습 경로는 [모듈 개요](/part-4/10-scaling-logging-streaming/)에.

유형 3·4는 실습까지, 유형 5는 아키텍처 이해와 채점 항목 대조까지 한다(배포는 [Lab](/part-4/10-scaling-logging-streaming/lab/)에서 선택). 각 유형은 ① 도식 ② 언제 나오나 ③ 핵심 연결 고리 ④ 채점이 보는 상태 ⑤ 변형 포인트 ⑥ 퀴즈 순서다. `유형 3 변형` 절만 ④ 가 고정 이름·값이고, 절 전체가 두 세트를 대조하는 변형 축이라 별도의 변형 포인트를 두지 않는다.
...
- **유형 5 — 스트리밍** · set-02 task-2 module-2·module-4 — Kinesis + Flink Studio 노트북 갈래와 MSK + Lambda 컨슈머 갈래.
