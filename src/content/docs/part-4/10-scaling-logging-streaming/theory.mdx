...
여유가 없다는 점도 함께 안다. 환경에 따라 Karpenter 통합과 EC2 종료가 150초를 넘겨 `Final Pods 1 / Final Nodes 2`로 끝나는 경우가 보고됐고(`seq 1 60`으로 늘리면 정상 수렴), 반복 횟수 상향이 요청된 상태다([정정 내역](/reference/errata/)). 상향 전까지는 **150초를 기준으로 맞추고**, 채점 직전 노드가 이미 1대인 상태에서 시작한다.
:::
...
근거: set-08 task-2 과제지·채점기준표·`asgmt2_module4_check.sh`. `skills-2026`에 **정답지 구현이 없다**.
...
flowchart LR
    ...
    SO["ScaledObject sqs-worker-scaledobject<br/>queueLength 2 · min 0 · max 6<br/>+ TriggerAuthentication"]
...
| `minReplicaCount` | 1 | **0** |
| `pollingInterval` | **두지 않는다**(min ≥ 1이면 무효, webhook 경고) | **15초 이하로 반드시 둔다** — min 0이라 0↔1 활성화를 오퍼레이터가 직접 폴링한다 |
...
set-07에서는 `pollingInterval`을 **지우는 것**이 정답이었다. `minReplicaCount ≥ 1`이면 1→N 스케일링이 전부 HPA 폴링이라 이 필드가 무효고 KEDA 2.20 webhook이 경고를 낸다. set-08은 **`min 0`**이라 정반대다 — 0↔1 활성화는 HPA가 아니라 KEDA 오퍼레이터가 트리거를 직접 폴링해 판단하므로 `pollingInterval`이 실제 감지 지연을 결정하고, 과제지가 15초 이하를 요구한다. **필드 하나의 유효 여부가 `minReplicaCount` 값에 달려 있다** — 외운 값을 옮기지 말고 min이 0인지부터 본다.
:::
...
3. **`spec.disruption.consolidationPolicy`가 필수 필드다.** 과제지가 명시적으로 요구하고, 채점 4-5가 NodePool YAML 전문을 덤프해 확인한다.
4. **IRSA는 3개 ServiceAccount 전부.** `keda/keda-operator`, `karpenter/karpenter`, `skills-sqs/sqs-worker-sa`에 `eks.amazonaws.com/role-arn` annotation이 있어야 한다(채점 4-2가 SA YAML을 직접 본다). 워커도 SQS를 직접 호출하므로 워커 SA에도 권한이 필요하다 — set-07은 KEDA만 큐를 읽었지만 여기서는 워커가 `receive_message`·`delete_message`를 한다.
5. **채점 스크립트가 kubectl을 설치해 주지 않는다.** 채점지 0번이 "CloudShell에 kubectl이 없으면 EKS 버전에 맞춰 직접 설치"를 채점 준비 절차로 둔다. 클러스터 엔드포인트도 CloudShell에서 접근 가능해야 한다(퍼블릭 엔드포인트 필요).
...
정답지가 없는 모듈이라 `CHANGELOG.txt`가 중요한 근거다([정정 내역](/reference/errata/)).
...
- 4-5-B `--output json` 으로 `SENSOR-002` 의 `sensorId`·`timestamp` **두 항목만** 조회하고, `timestamp`가 ISO 8601 KST(`2026-06-01T18:28:24+09:00`) 형식이면 정답(producer가 계속 돌고 있다는 증거). 정정 전 명령은 `--output table`·`max-items 3` 이었다 — [정정 내역](/reference/errata/) 참고.
...
한 글자 차이지만 통일하거나 "고쳐주면" 동작이 깨진다. 제공 문서의 이름을 원문 그대로 유지하는 원칙은 과제지에도 그대로 적용된다 — 다만 오타의 대표 사례였던 set-02의 `wsc2026-alaytics-ec2-role`(alaytics)은 정정으로 수정됐고 채점 대상도 아니다([정정 내역](/reference/errata/)). 이름을 확정하기 전에 정정 공지를 먼저 본다.
</Quiz>
