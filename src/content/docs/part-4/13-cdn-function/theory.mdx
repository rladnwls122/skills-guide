...
선행 지식과 학습 경로는 [모듈 개요](/part-4/13-cdn-function/)에.

S3 퍼블릭 차단·OAC 버킷 정책·순환 참조 같은 CloudFront 공통 구조는 이미 [02. KMS + S3 + CloudFront 이론](/part-1/02-kms-s3-cloudfront/theory/)이 다룬다. 기초 정의만 다시 보려면 [s3-basics 의 OAC 절](/start/s3-basics/#cloudfront-oac--원본을-비공개로-유지하는-구조)로 간다. 여기서는 반복하지 않고 함수 자체에 집중한다.
