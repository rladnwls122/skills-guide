...
실측 근거는 set-07 task-2 module-3 이다(ap-northeast-2, `skm-` 접두어). 2026-07-31 실채점에서 `mark3.sh` 전 항목(3-1~3-7) 통과가 확정됐다

HPA·KEDA·Karpenter 가 각각 무엇을 늘리는지는 [autoscaling-basics](/start/autoscaling-basics/)에 있다. SQS 의 가시성 제한 시간은 [serverless-basics 의 SQS 절](/start/serverless-basics/#5-sqs--가시성-제한-시간이-핵심이다)에 있다.
...
큐 길이 하나가 파드와 노드를 차례로 늘리는 경로다. KEDA 는 IRSA 로 SQS 길이를 읽어 HPA 를 통해 replicas 를 올린다. 늘어난 파드가 스케줄될 자리를 못 찾으면 그 Pending 을 보고 Karpenter 가 노드를 만든다. 점선이 축소 경로다 — 노드는 비거나 저활용 상태로 60초가 지나야 반환된다. 파드 축소와 노드 축소가 시차를 두는 이유다.
...
HPA behavior `scaleDown.stabilizationWindowSeconds`(기본 300초 → **15초**) + `policies: Percent 100/15s`(이건 이미 기본값이라 명시만 하는 것) + NodePool `consolidateAfter: 60s`가 한 쌍. 이 세 값이 왜 합산으로 움직이는지는 [autoscaling-basics 의 수렴 시간 절](/start/autoscaling-basics/#수렴-시간은-타이머-세-개의-합이다)에 있다. purge 를 HPA sync(~15초)가 감지해 ~35초에 Pod 이 1 이 된다. consolidateAfter 60초 뒤 ~140초에 노드도 1 로 수렴한다(실채점 확정). 기본값 300이면 Pod 축소만 5분이라 150초 창을 구조적으로 못 맞춘다. ScaledObject에 `pollingInterval`은 두지 않는다 — `minReplicaCount ≥ 1` 이면 1→N 스케일링을 HPA 가 자기 sync 주기로 돌린다. 그래서 이 값이 감지 속도를 바꾸지 못한다. 요구가 바뀌어 `min=0` 이 되면 다시 의미를 갖는다.
:::
...
여유가 없다는 점도 함께 안다. 환경에 따라 Karpenter 통합과 EC2 종료가 150초를 넘기기도 한다. 그러면 `Final Pods 1 / Final Nodes 2` 로 끝난다 — `seq 1 60` 으로 늘리면 정상 수렴한다. 상향 전까지는 **150초를 기준으로 맞추고**, 검증 직전 노드가 이미 1대인 상태에서 시작한다.
:::
