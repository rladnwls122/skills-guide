...
여유가 없다는 점도 함께 안다. 환경에 따라 Karpenter 통합과 EC2 종료가 150초를 넘겨 `Final Pods 1 / Final Nodes 2`로 끝나는 경우가 보고됐고(`seq 1 60`으로 늘리면 정상 수렴), 반복 횟수 상향이 요청된 상태다. 상향 전까지는 **150초를 기준으로 맞추고**, 채점 직전 노드가 이미 1대인 상태에서 시작한다.
:::
