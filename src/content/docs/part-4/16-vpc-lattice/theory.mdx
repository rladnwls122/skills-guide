...
2과제 set-08 task-2 module-2(Simplify Service Networking with VPC Lattice, **도쿄 ap-northeast-1**)를 다룬다. `skills-2026` 저장소에 이 모듈의 참고 구현이 없어 이 문서와 [lab](/part-4/16-vpc-lattice/lab/)이 유일한 근거다.
...
| Transit Gateway | 허브·스포크 | 불가 | 있음 (TGW RTB로 스포크 간 선택 허용) | L3 라우팅 |
...
이 프리픽스 리스트 접근 제어는 Service EC2 쪽 SG의 역할이고, **VPC Association의 SG는 별개다.** Association SG는 Client VPC(`10.61.0.0/16`)에서 오는 TCP/80을 열어 클라이언트가 Lattice 생성 도메인에 닿게 하는 것이 목적이다. 하나는 "누가 타깃에 도달할 수 있는가"(프리픽스 리스트), 다른 하나는 "누가 Lattice 도메인을 호출할 수 있는가"(Client VPC CIDR)를 통제한다 — 이름이 비슷해 보여도 막는 지점이 다르다.
...
Service Network가 가장 바깥 경계(VPC·Service를 묶는다), 그 안에 Service(도메인 이름을 발급받는 단위), Service 안에 Listener(프로토콜·포트), Listener가 Forward하는 곳이 Target Group(실제 인스턴스 목록)이다.
</Quiz>
