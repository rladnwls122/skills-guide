...
선행 지식과 학습 경로는 [모듈 개요](/part-4/17-streaming/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

ENI(Elastic Network Interface)는 리소스에 붙는 가상 랜카드다 — 자세한 것은 [vpc-basics](/start/vpc-basics/)에 있다.
...
유형 5 두 갈래를 나란히 놓았다. 위쪽은 Flask 앱이 Kinesis 로 주문을 흘리고 Flink Studio 노트북이 SQL 두 개를 돌리는 구성이다. 결과는 노트북 화면에만 남고 별도 저장소가 없다. 아래쪽 MSK 는 토픽이 둘이다 — 원본 토픽을 읽은 함수가 정상 데이터는 DynamoDB 에 넣고 이상치는 경보 토픽으로 되쏜다. 그 경보 토픽을 두 번째 함수가 다시 읽어 SNS 와 S3 로 보낸다. 같은 클러스터를 두 번 지나는 구조가 이 아키텍처의 특징이다.
...
### ②′ 왜 큐를 사이에 두는가 :badge[강의]{variant=tip}

생산 속도가 소비 속도보다 빠르면 수집기 버퍼가 찬다. 버퍼가 다 차면 둘 중 하나가 일어난다 — 데이터를 버리거나, 보내는 쪽이 멈춘다. 이 상태를 back-pressure 라고 부른다. 프로듀서가 잘못한 것이 아니라 두 속도가 다른 것이 원인이다.

큐를 중간에 두면 급증분이 큐에 먼저 쌓인다. 컨슈머는 자기가 감당할 만큼만 당겨 간다(pull). 그래서 초당 처리량이 열 배로 뛰어도 컨슈머가 죽지 않는다. 대신 밀린 양이 **consumer lag** 으로 남는다.

여기서 갈리는 판단이 하나 있다. lag 이 쌓일 때 컨슈머의 버퍼 크기를 키우는 것은 해법이 아니다. 버퍼를 키워도 처리 속도는 그대로라서 잠깐 미루기만 한다. 늘려야 하는 것은 **컨슈머 수**다. [모듈 15](/part-4/15-eks-scaling/)의 KEDA 가 큐 깊이를 보고 파드를 늘리는 것이 정확히 이 조치다.

MSK 갈래에서 이 개념이 걸리는 자리는 ESM(event source mapping, Lambda 가 큐를 폴링하는 연결) 의 `starting_position` 이다. `LATEST` 가 아니면 재배포한 컨슈머가 큐에 남아 있던 과거 메시지를 처음부터 다시 읽는다. 밀린 백로그가 그대로 폭주로 돌아온다.
