...
선행 지식과 학습 경로는 [모듈 개요](/part-4/18-documentdb/)에.

이 모듈은 2과제 후보 **set-08 task-2 module-1**이다. 근거는 지급 코드 `docdb_client.py`와 문제지, 그리고 2026-08-01 정정본의 판정 기준이다 — 정답 Terraform 은 `skills-2026` `main` 에 있다. Amazon DocumentDB가 무엇인지, DynamoDB와 무엇이 다른지, 왜 VPC 밖에서 접근할 수 없는지, 인덱스를 왜 직접 만들어야 하는지를 채점 관점에서 정리한다. 절차는 [lab](/part-4/18-documentdb/lab/)에서 다룬다.
...
구성 전체가 한 장이다. 채점자는 EC2 의 8080 으로만 들어오고, DocumentDB 는 VPC 안에서 클라이언트 보안 그룹으로만 열려 있어 밖에서 직접 닿을 수 없다. 비밀번호는 EC2 에 저장되지 않고 요청마다 Secrets Manager 에서 읽는다 — 코드나 user-data 에 자격 증명이 남지 않는 구조다. KMS 는 트래픽 경로가 아니라 저장 암호화 쪽에 붙는다.
