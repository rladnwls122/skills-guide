...
선행 지식과 학습 경로는 [모듈 개요](/part-4/18-documentdb/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

이 모듈은 2과제 후보 **set-08 task-2 module-1**이다. 근거는 지급 코드 `docdb_client.py`와 문제지, 그리고 2026-08-01 정정본의 판정 기준이다 — 정답 Terraform 은 `skills-2026` `main` 에 있다. Amazon DocumentDB 가 무엇이고 DynamoDB 와 무엇이 다른지를 정리한다. 왜 VPC 밖에서 접근할 수 없는지, 인덱스를 왜 직접 만들어야 하는지도 채점 관점에서 본다. 절차는 [lab](/part-4/18-documentdb/lab/)에서 다룬다.
...
<summary>지금 확인하기 — 이 엔진이 어떤 API 를 흉내 내는가</summary>

DocumentDB 는 MongoDB 와 **호환**이지 같은 것이 아니다. 무엇이 지원되는지는 CLI 도움말에 클러스터 파라미터로 드러난다. 조회와 도움말뿐이라 과금이 없다.

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  aws docdb create-db-cluster help | Select-String -Pattern 'engine-version|EngineVersion|storage-encrypted|DeletionProtection' | Select-Object -First 8
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  aws docdb create-db-cluster help | grep -E 'engine-version|EngineVersion|storage-encrypted|DeletionProtection' | head -8
  ```
  </TabItem>
</Tabs>

기대: 엔진 버전과 암호화·삭제 보호 옵션이 보인다. **버전이 곧 호환되는 MongoDB API 수준**이라, 지급 코드가 쓰는 연산이 그 버전에서 되는지가 먼저 걸린다.

`aws: command not found` 면 도구가 없는 것이다 — [사전 준비 §2 도구 설치](/start/#2-도구-설치-본-pc)로 돌아간다.

</details>

## 2. DynamoDB와 무엇이 다른가 — 문서 모델·인덱스·쿼리

PART-1에서 다룬 DynamoDB와 같은 "NoSQL"로 묶이지만 설계 방식은 다르다. 비교 대상인 DynamoDB 쪽 개념(PK·GSI·Query 와 Scan)은 [dynamodb-basics](/start/dynamodb-basics/)에 있다.
...
| 인덱스 | GSI/LSI(같은 PK 안에서 정렬 키만 바꾸는 보조 인덱스) 설계를 테이블 설계와 함께 한다 | `createIndex()`로 언제든 추가. unique·복합·정렬 방향·TTL을 조합한다 |
...
`docdb_client.py`의 `low_stock()`은 `warehouseId`로 Mongo에서 문서를 가져온 뒤, `stock <= reorderPoint`(같은 문서의 두 필드 비교)는 **Python 코드로** 걸러낸다. MongoDB 계열 `find()` 도 이런 필드 간 비교는 `$expr` 가 있어야 한다. DynamoDB `FilterExpression` 도 속성과 상수를 비교하는 것이 기본이라 속성끼리 비교는 매끄럽지 않다. `{warehouseId:1, stock:1}` 인덱스는 후보를 정렬해 가져오는 역할까지만 하고, 최종 판단은 애플리케이션이 한다.
:::
...
구성 전체가 한 장이다. 채점자는 EC2 의 8080 으로만 들어온다. DocumentDB 는 VPC 안에서 클라이언트 보안 그룹으로만 열려 있어 밖에서 직접 닿을 수 없다. 비밀번호는 EC2 에 저장되지 않고 요청마다 Secrets Manager 에서 읽는다 — 코드나 user-data 에 자격 증명이 남지 않는 구조다. KMS 는 트래픽 경로가 아니라 저장 암호화 쪽에 붙는다.
...
<details class="build-step">
<summary>지금 확인하기 — 비밀번호가 어디에 있고 어떤 모양인가</summary>

자격 증명을 코드에 넣지 않고 비밀 저장소에서 꺼내 쓰는 구조다. 무엇이 저장되는지 도움말로 확인한다. 조회뿐이라 과금이 없다.

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  aws secretsmanager get-secret-value help | Select-String -Pattern 'SecretString|VersionStage|AWSCURRENT' | Select-Object -First 6
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  aws secretsmanager get-secret-value help | grep -E 'SecretString|VersionStage|AWSCURRENT' | head -6
  ```
  </TabItem>
</Tabs>

기대: 값이 문자열 하나로 나오고 버전 단계가 붙는다는 설명이 보인다. 데이터베이스가 만든 비밀은 보통 **JSON 한 덩어리**라, 꺼낸 뒤 한 번 더 파싱해야 사용자명과 비밀번호가 나온다.

지급 코드가 어떤 키 이름을 기대하는지는 코드 쪽이 정본이다 — 키 이름이 다르면 접속이 인증 오류로 끝난다.
...
<details class="build-step">
<summary>지금 확인하기 — TTL 인덱스가 무엇을 언제 지우나</summary>

TTL 은 편의 기능이 아니라 **데이터를 지우는 기능**이다. 정답지의 인덱스 부트스트랩에서 무엇을 기준으로 지우는지 확인한다. 파일 읽기라 과금이 없다.

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  Get-ChildItem -Recurse $HOME\skills-2026\set-08\task-2\module-1-nosql |
    Select-String -Pattern 'expireAfterSeconds|createIndex|unique|expiresAt' | Select-Object -First 10
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  grep -rnE 'expireAfterSeconds|createIndex|unique|expiresAt' \
    ~/skills-2026/set-08/task-2/module-1-nosql | head -10
  ```
  </TabItem>
</Tabs>

기대: 만료 기준 필드와 인덱스 정의가 보인다.

:::danger[당일 데이터가 이미 지난 시각이면 조용히 사라진다]
만료 기준 필드의 값이 **과거**인 문서는 인덱스를 만드는 순간부터 삭제 대상이다. 배부 데이터의 시각이 지났다면 문서 수를 세는 채점 항목이 그대로 0 이 된다.

배부 데이터를 넣은 직후 **문서 수를 한 번 세어 두고**, 몇 분 뒤 다시 센다. 줄어들면 이 문제다.
:::
...
| 바뀌는 값 | 고칠 곳 | 확인 |
|---|---|---|
| 클러스터·인스턴스 이름 | `skills-nosql-docdb-cluster` · `skills-nosql-docdb-instance-1` | `describe-db-clusters` · `describe-db-instances` |
| 인스턴스 클래스·리전 | `db.t3.medium` · 서울 ap-northeast-2 | `describe-db-instances` |
| Secret 이름과 `host` 값 | `skills-nosql-docdb-secret` 의 `username`·`password`·`host` | `describe-db-clusters` 의 `Endpoint` |
| 데이터베이스·컬렉션 이름 | `skills_retail` 의 `orders`·`products`·`sessions` | `GET /v1/admin/summary` 의 `counts` |
| 적재 건수 | 주문 8 · 상품 6 · 세션 3 | `GET /v1/admin/summary` 의 `counts` |
| 인덱스 필드·정렬 방향·`unique` | `create_index()` 로 직접 생성 | `GET /v1/admin/indexes` |
| TTL 기준 필드 | `sessions` 의 `{expiresAt: 1}` · `expireAfterSeconds: 0` | `GET /v1/admin/indexes` |
| CA 번들 경로 | `docdb_client.py` 의 `CA_FILE` | `/opt/skills-nosql/global-bundle.pem` 존재 |

`docdb_client.py` 는 지급 코드이므로 이름·건수·인덱스 사양이 바뀌어도 코드를 고치는 자리는 `CA_FILE` 경로 하나다. 나머지는 전부 AWS 리소스와 Secret 값, 그리고 직접 만드는 인덱스 쪽에서 흡수한다.
