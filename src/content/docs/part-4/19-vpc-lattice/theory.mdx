...
선행 지식과 학습 경로는 [모듈 개요](/part-4/19-vpc-lattice/)에.

2과제 set-08 task-2 module-2(Simplify Service Networking with VPC Lattice, **도쿄 ap-northeast-1**)를 다룬다. 참고 구현은 `skills-2026` `main` 의 `set-08/task-2/module-2-lattice` 에 있고, 항목별 판정 기준은 2026-08-01 정정본이 정본이다([정정 내역](/reference/errata/)). 정정본 2-4가 **Service EC2 보안 그룹의 TCP/8080 인바운드는 Lattice 관리형 프리픽스 리스트로만 열고 `0.0.0.0/0`이면 미충족**임을 명문화했다.
...
피어링·TGW가 CIDR 겹침에 막히는 이유는 라우팅 테이블에 상대 CIDR을 목적지로 올리기 때문이다([vpc-basics 의 VPC 연결 절](/start/vpc-basics/#vpc-연결--피어링과-transit-gateway)과 [L3·L4·L7 절](/start/vpc-basics/#연결-방식이-무엇을-보고-판단하는가--l3l4l7)이 이 표의 근본 원리다). PrivateLink(내부 ALB/NLB + VPC 엔드포인트)는 이미 CIDR 제약이 없는 L4/L7 방식이지만, 서비스마다 NLB와 엔드포인트 서비스를 개별로 만들어야 한다. Lattice는 그 구성을 Service Network 하나로 묶어 다대다로 확장한다 — VPC 여러 개, 서비스 여러 개가 같은 Service Network에 붙으면 서로 접근할 수 있다.
...
당일 과제는 전체 채점 점수의 약 30% 범위 안에서 수정된다([대회 운영 규칙](/reference/competition-rules/)). 이 모듈에서 바뀔 값과 고칠 자리는 정해져 있다.
