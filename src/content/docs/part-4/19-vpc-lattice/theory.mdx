...
선행 지식과 학습 경로는 [모듈 개요](/part-4/19-vpc-lattice/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

2과제 set-08 task-2 module-2 를 다룬다 — Simplify Service Networking with VPC Lattice, **도쿄 `ap-northeast-1`** 이다. 참고 구현은 `skills-2026` `main` 의 `set-08/task-2/module-2-lattice` 에 있고, 항목별 판정 기준은 2026-08-01 정정본이 정본이다. 정정본 2-4 가 판정 기준을 명문화했다. **Service EC2 보안 그룹의 TCP/8080 인바운드는 Lattice 관리형 프리픽스 리스트로만 열어야 하고, `0.0.0.0/0` 이면 미충족이다.**
...
두 VPC 가 IP 경로 없이 통신하는 구조다. 대역이 겹치든 말든 상관없고 피어링도 없다 — 클라이언트는 Lattice 가 발급한 DNS 이름을 부르고, Service Network 에 묶인 양쪽이 그 이름을 통해 만난다. 서비스 EC2 의 보안 그룹은 Lattice 관리형 프리픽스 리스트만 허용한다. 그래서 트래픽이 실제로 Lattice 를 거쳐 들어온다는 것이 방화벽 수준에서 강제된다.

</details>

Client EC2는 Client VPC 안에서 Lattice가 생성한 도메인 이름을 호출한다. 그 요청은 Client VPC 에 붙은 **VPC Association** 을 거쳐 Service Network 로 들어간다. 거기서 Service → Listener → Target Group 을 거쳐 Service VPC 안의 EC2 로 전달된다. 이 경로 어디에도 두 VPC 사이의 IP 라우팅은 없다 — Target Group 이 Service VPC 를 `vpcIdentifier` 로 지정할 뿐이다. Service VPC 자체는 Service Network 에 Association 되지 않는다.
...
피어링·TGW 가 CIDR 겹침에 막히는 이유는 라우팅 테이블에 상대 CIDR 을 목적지로 올리기 때문이다. 이 표의 근본 원리는 [vpc-basics 의 VPC 연결 절](/start/vpc-basics/#vpc-연결--피어링과-transit-gateway)과 [L3·L4·L7 절](/start/vpc-basics/#연결-방식이-무엇을-보고-판단하는가--l3l4l7)에 있다. PrivateLink(내부 ALB/NLB + VPC 엔드포인트)는 이미 CIDR 제약이 없는 L4/L7 방식이지만, 서비스마다 NLB와 엔드포인트 서비스를 개별로 만들어야 한다. Lattice는 그 구성을 Service Network 하나로 묶어 다대다로 확장한다 — VPC 여러 개, 서비스 여러 개가 같은 Service Network에 붙으면 서로 접근할 수 있다.
...
**VPC Association은 Client VPC에만 만든다.** Service VPC는 Target Group의 `vpcIdentifier`로만 연결되고 별도 Association이 없다 — Lattice가 Target Group 설정을 보고 Service VPC 안에 필요한 경로를 알아서 만든다. Client VPC 를 Association 하는 이유가 있다. 그 VPC 안의 Client EC2 가 Lattice 생성 도메인을 **호출**해야 하기 때문이다. 호출하는 쪽만 Association이 필요하다.

## 관리형 프리픽스 리스트로 접근 제어

프리픽스 리스트가 무엇이고 보안 그룹 규칙의 소스로 어떻게 쓰이는지는 [vpc-basics 의 프리픽스 리스트 절](/start/vpc-basics/#프리픽스-리스트--cidr-묶음-하나를-규칙의-소스로-쓴다)에 있다.
...
<summary>지금 확인하기 — 관리형 프리픽스 리스트가 실제로 무엇인가</summary>

보안 그룹을 `0.0.0.0/0` 대신 프리픽스 리스트로 여는 것이 이 모듈의 채점 지점이다. 그 리스트가 무엇인지 계정에서 직접 본다. **조회뿐이라 과금이 없다.**

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  aws ec2 describe-managed-prefix-lists --region ap-northeast-1 `
    --query "PrefixLists[?contains(PrefixListName, 'vpc-lattice')].[PrefixListId,PrefixListName,AddressFamily]" --output table
  ```
  </TabItem>
...
기대: `com.amazonaws.<리전>.vpc-lattice` 형태의 이름과 ID 가 나온다. **AWS 가 관리하는 목록**이라 내가 만들 필요가 없고, 리전마다 ID 가 다르므로 값을 하드코딩하면 리전이 바뀔 때 깨진다.

비어 있으면 리전을 잘못 준 것이다. 이 모듈의 리전은 도쿄다.

</details>

## CIDR이 겹쳐도 되는 이유

Client VPC(`10.61.0.0/16`)와 Service VPC(`10.62.0.0/16`)는 겹치지 않지만, 겹쳤어도 이 아키텍처는 그대로 동작한다. 이유는 앞의 두 절을 합치면 나온다 — Client 는 Lattice 도메인을 호출할 뿐 Service VPC 의 IP 를 직접 알지 못한다. Lattice 의 데이터 플레인이 Target Group 설정을 보고 Service VPC 안에서 별도로 타깃에 연결한다. 두 VPC의 라우팅 테이블 어디에도 상대 VPC의 CIDR을 가리키는 경로가 생기지 않는다. 피어링·TGW의 CIDR 제약은 "상대 CIDR을 내 라우팅 테이블에 올린다"는 전제에서만 나오는데, Lattice는 그 전제 자체가 없다.
...
<details class="build-step">
<summary>지금 확인하기 — 서비스 네트워크에 무엇을 붙이나</summary>

Lattice 는 계층이 넷이라 무엇을 무엇에 붙이는지가 헷갈린다. 도움말로 연결 관계를 확인한다. 조회뿐이라 과금이 없다.

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  aws vpc-lattice create-service-network-vpc-association help | Select-String -Pattern 'vpc-identifier|service-network-identifier|security-group' | Select-Object -First 6
  aws vpc-lattice create-service-network-service-association help | Select-String -Pattern 'service-identifier|service-network-identifier' | Select-Object -First 4
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  aws vpc-lattice create-service-network-vpc-association help | grep -E 'vpc-identifier|service-network-identifier|security-group' | head -6
  aws vpc-lattice create-service-network-service-association help | grep -E 'service-identifier|service-network-identifier' | head -4
  ```
  </TabItem>
</Tabs>

기대: **연결이 두 종류**라는 것이 보인다 — VPC 를 네트워크에 붙이는 것과 서비스를 네트워크에 붙이는 것. 둘 다 있어야 클라이언트 VPC 에서 서비스가 보인다. 하나만 하면 이름은 풀리는데 닿지 않는다.

`Invalid choice` 가 나오면 CLI 가 오래된 것이다. 버전을 올리거나 CloudShell 에서 실행한다.

</details>

## 변형 포인트

요구가 바뀌어도 이 모듈에서 고칠 자리는 정해져 있다.

| 바뀌는 값 | 고칠 곳 | 확인 |
|---|---|---|
| 4계층 리소스 이름 | `skills-lattice-sn` · `skills-lattice-order-service` · `skills-lattice-http-listener` · `skills-lattice-order-tg` | `asgmt2_module2_check.sh` [2-3] |
| EC2 이름·앱 포트 | `skills-lattice-client-ec2` :80 · `skills-lattice-service-ec2` :8080 | `asgmt2_module2_check.sh` [2-2] |
| 리스너·타깃 포트, 헬스 체크 경로 | Listener HTTP/80 · Target Group HTTP/8080 `/health` | `curl /health` |
| 리전 | 프리픽스 리스트 `com.amazonaws.ap-northeast-1.vpc-lattice` | `aws ec2 describe-managed-prefix-lists` |
| Service EC2 SG 인바운드 소스 | 8080 소스 = 프리픽스 리스트 ID(`pl-...`) | `describe-security-groups` |
| Client VPC CIDR | `10.61.0.0/16` — Association SG 의 TCP/80 소스 | `get-service-network-vpc-association` 의 `securityGroupIds` |
| 클라이언트 호출 대상 | `SERVICE_URL` = `get-service` 의 `dnsEntry.domainName` | `curl /v1/client/orders?id=1001` |
| 주문 ID·응답 문자열 | `id=1001` → `order_id=1001, via=vpc-lattice` | `asgmt2_module2_check.sh` [2-5] |

리전이 바뀌면 프리픽스 리스트 이름의 리전 부분도 함께 바뀐다 — 이름을 그대로 두면 `pl-` ID 조회가 빈 결과를 내고 Service EC2 SG 규칙을 만들 소스가 없어진다. `SERVICE_URL` 은 사설 IP를 넣어도 응답이 오므로 값이 틀려도 증상이 늦게 드러난다. 두 값은 치환 후 바로 확인한다.


<details class="build-step">
<summary>지금 확인하기 — 채점이 무엇을 조회하나</summary>

채점 스크립트를 먼저 읽으면 만들 것이 정해진다. **파일 읽기라 과금이 없다.**

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  Select-String -Pattern 'aws vpc-lattice|describe-security-group|PrefixList|8080' `
    -Path $HOME\skills-2026\set-08\task-2\mark\mark2-2.sh | Select-Object -First 10
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  grep -nE 'aws vpc-lattice|describe-security-group|PrefixList|8080' \
    ~/skills-2026/set-08/task-2/mark/mark2-2.sh | head -10
  ```
  </TabItem>
</Tabs>

기대: 서비스 네트워크·서비스·타깃그룹 조회와 함께 **보안 그룹의 8080 인바운드를 프리픽스 리스트로만 열었는지** 보는 줄이 나온다. `0.0.0.0/0` 이면 그 항목은 미충족이다.

이 한 줄이 "열리기만 하면 된다"와 "어떻게 열었는가"의 차이를 만든다.
