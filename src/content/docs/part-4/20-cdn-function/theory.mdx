...
선행 지식과 학습 경로는 [모듈 개요](/part-4/20-cdn-function/)에.
...
당일 과제는 전체 채점 점수의 약 30% 범위 안에서 수정된다([대회 운영 규칙](/reference/competition-rules/)). 함수 JS 는 Terraform 변수가 아니라 정적 파일이라 접두어 치환이 닿지 않는다 — 바뀐 값을 파일 안에서 직접 고친다.
...
쿠키명은 두 자리를 함께 바꾼다. 함수 JS 만 고치고 캐시 정책의 화이트리스트 쿠키 이름을 그대로 두면 CloudFront 가 옛 이름 쿠키로 캐시 키를 만들어, 배정이 바뀌어도 같은 캐시 항목이 나간다.
