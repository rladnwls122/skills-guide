...
CloudFront 가 요청을 처리하는 동안 코드가 끼어들 수 있는 지점은 이론상 넷이다. **viewer-request → (캐시 미스 시) origin-request → origin-response → viewer-response** 순이다. 그런데 CloudFront Functions 는 이 중 **viewer-request·viewer-response 둘만** 지원한다. mTLS 연결 단계 전용의 connection-request 도 있지만 이 모듈과는 무관하다. origin-request·origin-response는 **Lambda@Edge 전용**이다.
...
CloudFront 의 훅 네 자리와 이 모듈이 쓰는 두 자리를 구분한다. ①viewer-request 와 ④viewer-response 는 CloudFront Functions 가 붙을 수 있는 자리고, 가운데 origin-request·origin-response 는 Lambda@Edge 전용이라 여기서는 쓰지 않는다. 배정은 ①에서 하고 쿠키 발급은 ④에서 한다 — 요청 단계에서 정한 값을 헤더로 남겨 응답 단계가 읽는 형태다.
