---
title: 20. 이론 — CloudFront Functions vs Lambda@Edge (버지니아 북부 us-east-1)
...
선행 지식과 학습 경로는 [모듈 개요](/part-4/20-cdn-function/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

S3 퍼블릭 차단·OAC 버킷 정책·순환 참조 같은 CloudFront 공통 구조는 이미 [07. KMS + S3 + CloudFront 이론](/part-1/07-kms-s3-cloudfront/theory/)이 다룬다. 기초 정의만 다시 보려면 [s3-basics 의 OAC 절](/start/s3-basics/#cloudfront-oac--원본을-비공개로-유지하는-구조)로 간다. 여기서는 반복하지 않고 함수 자체에 집중한다.

---

## 1. CloudFront Functions vs Lambda@Edge — 4개 이벤트 훅

CloudFront 가 요청을 처리하는 동안 코드가 끼어들 수 있는 지점은 이론상 넷이다. 순서는 **viewer-request → origin-request → origin-response → viewer-response** 다. 가운데 둘은 캐시 미스일 때만 돈다. 그런데 CloudFront Functions 는 **viewer-request·viewer-response 둘만** 지원한다. origin-request 와 origin-response 는 **Lambda@Edge 전용**이다. mTLS 연결 단계 전용의 connection-request 도 있지만 이 모듈과는 무관하다.
...
CloudFront 의 훅 네 자리와 이 모듈이 쓰는 두 자리를 구분한다. ①viewer-request 와 ④viewer-response 는 CloudFront Functions 가 붙을 수 있는 자리다. 가운데 origin-request·origin-response 는 Lambda@Edge 전용이라 여기서는 쓰지 않는다. 배정은 ①에서 하고 쿠키 발급은 ④에서 한다 — 요청 단계에서 정한 값을 헤더로 남겨 응답 단계가 읽는 형태다.
...
**어느 쪽을 언제 쓰나** — 가벼운 문자열 로직은 CloudFront Functions 다. 캐시 키 정규화, 헤더 조작, URL 재작성, 간단한 인가 검증이 그렇다. 외부 API 호출, 파일시스템, 요청 본문 파싱처럼 무거운 작업이 필요하면 Lambda@Edge. 이 모듈은 KVS 조회 + URI/쿠키 조작뿐이라 CloudFront Functions로 충분하다 — 오히려 **Lambda@Edge는 KeyValueStore를 아예 못 써서** 이 설계를 그대로 옮길 수 없다(훅·기능 지원 표).

---


<details class="build-step">
<summary>지금 확인하기 — 함수가 걸리는 훅이 넷이라는 것</summary>

어느 시점에 함수를 걸지가 이 모듈의 첫 판단이다. 도움말로 훅 이름을 확인한다. **조회뿐이라 과금이 없다.**

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  aws cloudfront create-function help | Select-String -Pattern 'viewer-request|viewer-response|Runtime|EventType' | Select-Object -First 6
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  aws cloudfront create-function help | grep -E 'viewer-request|viewer-response|Runtime|EventType' | head -6
  ```
  </TabItem>
</Tabs>

기대: 뷰어 쪽 요청·응답 훅과 런타임 이름이 보인다. **CloudFront Functions 는 뷰어 훅 둘에만 걸린다** — 오리진 쪽 훅이 필요하면 다른 서비스를 써야 한다.

경로를 고쳐 오리진으로 넘기는 일은 요청 훅에서 한다. 응답 훅에서 하면 이미 늦다.
...
**viewer-request** (`skillsphone-cdn-ab-req-fn`) — 요청 쿠키 `x-sp-ab`가 있으면 그 값을 유지하고, 없으면 KVS의 `weight`를 읽어 `Math.random() < weight`면 `b`, 아니면 `a`로 배정한다. 새로 배정한 경우에만 `x-sp-ab-assigned` 요청 헤더를 세운다. 기존 배정을 유지했다면 세우지 않는다 — viewer-response 가 이 헤더로 "새로 배정했는가"를 판단한다. 마지막에 배정된 버전에 해당하는 KVS 값(`version_a`/`version_b`)으로 `request.uri`를 덮어쓴다.
...
요청 단계 함수다. 쿠키가 이미 a 나 b 면 그 배정을 그대로 쓰고 헤더를 세우지 않는다 — 그래야 응답 단계가 `Set-Cookie` 를 다시 내보내지 않는다. 쿠키가 없을 때만 KVS 의 `weight` 를 읽어 난수와 비교해 배정하고, 그 사실을 헤더로 남긴다. 마지막에 배정에 맞는 경로를 KVS 에서 읽어 URI 를 바꾼다. `try` 로 감싼 이유는 KVS 키가 없을 때 오류로 죽지 않고 원본 요청을 그대로 통과시키기 위해서다.

</details>

**viewer-response** (`skillsphone-cdn-ab-res-fn`) — `x-sp-ab-assigned` 요청 헤더가 있을 때만 `Set-Cookie`를 추가한다. 즉 방금 새로 배정된 방문자에게만 쿠키를 발급한다. 이미 쿠키가 있던 재방문자에게는 다시 발급하지 않는다 — mark 2-4·2-5 가 검사하는 지점이다.
...
응답 단계 함수다. 요청 단계가 남긴 헤더가 있을 때만 쿠키를 굽는다. `Max-Age=86400` 이라 하루 동안 같은 방문자는 같은 버전을 본다. 헤더가 없으면 아무것도 하지 않으므로, 이미 배정된 방문자의 쿠키 만료 시간이 매 요청마다 갱신되지 않는다.

</details>

구현체 주석이 이유를 적어 두었다. KVS 조회 두 번(weight, version_*)을 `Promise.all` 이 아니라 순차 `await` 로 처리했다. 이유는 "함수 메모리 한도(2 MB) 초과 위험" 이다. 이번 조사에서 AWS 공식 문서에서 그 메모리 근거 문구를 별도로 재확인하지는 못했지만, 2 MB라는 한도 자체는 위 비교표에서 확인된다.

---


<details class="build-step">
<summary>지금 확인하기 — KeyValueStore 를 함수 밖에서 읽어 본다</summary>

A/B 비율 같은 값을 코드가 아니라 저장소에 두는 구조다. 저장소가 무엇을 담는지 도움말로 본다. 조회뿐이라 과금이 없다.

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  aws cloudfront-keyvaluestore help | Select-String -Pattern 'get-key|put-key|list-keys|describe-key-value-store' | Select-Object -First 6
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  aws cloudfront-keyvaluestore help | grep -E 'get-key|put-key|list-keys|describe-key-value-store' | head -6
  ```
  </TabItem>
</Tabs>

기대: 키 단위 읽기·쓰기 명령이 보인다. **함수를 다시 배포하지 않고 값만 바꿀 수 있다**는 것이 이 구조의 이유다 — 비율을 바꾸는 데 배포 전파를 기다리지 않는다.

`Invalid choice: 'cloudfront-keyvaluestore'` 가 나오면 CLI 가 오래된 것이다. CloudShell 에서 실행한다.
...
<details class="build-step">
<summary>지금 확인하기 — 실측 함정 목록을 원문으로 본다</summary>

이 모듈의 함정은 대부분 문법이 아니라 **배포 절차**에서 나온다. 정답지 기록을 직접 읽는다. 파일 읽기라 과금이 없다.

<Tabs syncKey="shell">
  <TabItem label="PowerShell">
  ```powershell
  Select-String -Pattern 'await|publish|LIVE|DEVELOPMENT|KVS' `
    -Path $HOME\skills-2026\set-07\task-2\NOTES.md | Select-Object -First 10
  ```
  </TabItem>
  <TabItem label="Bash (Linux/CloudShell)">
  ```bash
  grep -nE 'await|publish|LIVE|DEVELOPMENT|KVS' ~/skills-2026/set-07/task-2/NOTES.md | head -10
  ```
  </TabItem>
</Tabs>

기대: 함수를 만들었어도 **게시(publish)하지 않으면 배포에 반영되지 않는다**는 기록과, 문법 제약에 대한 메모가 나온다. 콘솔에서 저장까지 하고 게시를 안 해 "코드는 맞는데 동작이 안 바뀐다"로 헤매는 것이 이 모듈의 대표 사고다.
...
요구가 바뀌면 여기가 움직인다. 함수 JS 는 Terraform 변수가 아니라 정적 파일이라 접두어 치환이 닿지 않는다 — 바뀐 값을 파일 안에서 직접 고친다.

| 바뀌는 값 | 고칠 곳 | 확인 |
|---|---|---|
| 쿠키명 `x-sp-ab` | `terraform/cloudfront/req-fn.js` · `res-fn.js` + 캐시 정책 화이트리스트 쿠키 | `aws cloudfront test-function --stage LIVE` |
| 헤더명 `x-sp-ab-assigned` | `terraform/cloudfront/req-fn.js` · `res-fn.js` | `aws cloudfront test-function --stage LIVE` |
| KVS 키명 `weight`·`version_a`·`version_b` | `terraform/cloudfront/req-fn.js` + `aws_cloudfrontkeyvaluestore_keys_exclusive` | `list-keys` 정확 일치 |
| A/B 비율·버전 경로 값 | `aws_cloudfrontkeyvaluestore_keys_exclusive` 의 키 값 | `list-keys` |
| 쿠키 수명 `Max-Age=86400` | `terraform/cloudfront/res-fn.js` 의 `attributes` | `aws cloudfront test-function --stage LIVE` |
| 함수 이름 | `aws_cloudfront_function` 의 `skillsphone-cdn-ab-req-fn` · `skillsphone-cdn-ab-res-fn` | `describe-function --stage LIVE` 가 `DEPLOYED` |

쿠키명은 두 자리를 함께 바꾼다. 함수 JS 만 고치고 캐시 정책의 화이트리스트 쿠키 이름을 그대로 두면 어떻게 되나. CloudFront 가 옛 이름 쿠키로 캐시 키를 만들어, 배정이 바뀌어도 같은 캐시 항목이 나간다.
