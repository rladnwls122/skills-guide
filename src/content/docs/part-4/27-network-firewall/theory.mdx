...
선행 지식과 학습 경로는 [모듈 개요](/part-4/27-network-firewall/)에.
...
TLS 는 내용을 암호화하지만 **연결을 시작할 때 접속하려는 서버 이름을 평문으로 보낸다.** 이것을 SNI(Server Name Indication) 라고 하고, 한 IP 에서 여러 도메인을 서비스할 때 서버가 어느 인증서를 줄지 고르는 데 쓴다. 방화벽은 이 평문 조각을 읽는다.
...
<Quiz
  question="보안 그룹으로 특정 도메인 접근을 차단할 수 없는 이유는?"
  options={[
    '아웃바운드 규칙을 지원하지 않아서',
    'IP 와 포트만 알고 도메인 이름을 모르며, 허용 규칙만 있어 예외 차단을 표현할 수 없어서',
    '리소스마다 붙어야 해서',
    'VPC 밖에서 동작해서',
  ]}
  correct={1}
  explanation="보안 그룹은 IP·포트·프로토콜만 본다. 도메인의 IP 는 바뀌고 CDN 뒤면 여러 개다. 게다가 허용 목록 방식이라 '이것만 제외하고 허용'을 쓸 수 없다."
/>

<Quiz
  question="방화벽을 만들고 규칙도 넣었는데 차단이 되지 않는다. 가장 먼저 볼 것은?"
  options={[
    'Suricata 규칙 문법',
    '앱 서브넷 라우팅 테이블의 기본 경로가 방화벽 엔드포인트를 가리키는지',
    'CloudWatch 로그 그룹 이름',
    '보안 그룹 아웃바운드',
  ]}
  correct={1}
  explanation="트래픽이 방화벽을 지나지 않으면 규칙이 아무리 정확해도 소용없다. 기본 경로가 NAT 를 직접 가리키면 방화벽을 그냥 비켜 간다. 가장 흔한 원인이다."
/>

<Quiz
  question="HTTP 로 `ifconfig.io` 를 막았더니 HTTP 는 막히는데 HTTPS 는 통한다. 원인은?"
  options={[
    '방화벽이 HTTPS 를 지원하지 않는다',
    'TLS SNI 를 보는 규칙을 따로 쓰지 않았다',
    '인증서가 없다',
    'Stateless 규칙으로 써야 한다',
  ]}
  correct={1}
  explanation="http.host 규칙은 HTTP 만 본다. HTTPS 는 tls.sni 를 보는 규칙이 따로 필요하다. 두 줄이 한 쌍이다."
/>

<Quiz
  question="요구가 'FLOW 유형의 로그만 수집' 인데 ALERT 도 함께 켰다. 무엇이 문제인가?"
  options={[
    '비용만 조금 더 든다',
    '요구를 넘긴 구성이고 채점 스크립트가 로깅 설정을 그대로 출력해 확인한다',
    'FLOW 로그가 기록되지 않는다',
    '방화벽이 동작하지 않는다',
  ]}
  correct={1}
  explanation="채점이 describe-logging-configuration 결과를 그대로 본다. '만'이 붙은 요구는 초과 구성도 불일치다."
/>

<Quiz
  question="Stateless 규칙 그룹으로 처리하기에 적합한 것은?"
  options={[
  ...
  correct={1}
  explanation="ICMP 는 프로토콜 번호만 보면 판단할 수 있어 상태를 기억할 필요가 없다. 나머지 셋은 연결 상태나 애플리케이션 계층 내용을 봐야 하므로 Stateful 이다."
/>

<Quiz
  question="`drop` 대신 `reject` 를 쓰면 확인 명령에서 무엇이 달라지나?"
  options={[
    '아무 차이가 없다',
    'curl 이 타임아웃까지 기다리지 않고 즉시 연결 거부로 실패한다',
    '요청이 통과한다',
    '로그가 남지 않는다',
  ]}
  correct={1}
  explanation="drop 은 조용히 버려 보낸 쪽이 타임아웃할 때까지 기다린다. reject 는 거부를 알려 즉시 실패한다. 채점이 타임아웃을 기대하면 drop 이어야 한다."
/>
