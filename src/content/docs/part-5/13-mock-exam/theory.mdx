...
구현 전에 mark 항목 표(11-mutation-drill 훈련 ③ 산출물)를 머리에 올리고 시작한다. 채점 직전에는 [mark 스크립트 가이드](/reference/mark-script-guide/)의 **리셋 체크리스트**를 기계적으로 수행 — 시연 데이터 정리(재현 절차는 정확히 1회), 상태 복원, 트래픽 시드, 임시 리소스 제거, 채점 신원 확인.
