...
`requests`는 스케줄링 단위이자 [HPA 이용률의 분모](/start/autoscaling-basics/#1-hpa--이용률의-분모는-requests다)다. t3.medium(allocatable 약 1930m)에 앱 3파드를 패킹하려면 파드당 600m가 실측 균형점이다. 메모리는 request = limit로 고정한다 — 메모리는 압축 불가능한 자원이라 초과 시 스로틀링이 아니라 OOMKill(메모리 한도를 넘긴 컨테이너를 커널이 강제 종료하는 것)이고, 재시작은 availability를 직접 깎는다.
