...
RDS Proxy 의 커넥션 풀 설정이다. 파드가 늘면 클라이언트 커넥션은 함께 늘지만 백엔드 커넥션은 이 풀 안에서 재사용되므로 DB 의 `max_connections` 에 먼저 닿지 않는다. 유휴 커넥션 상한을 절반으로 둔 것은 부하가 빠진 뒤 커넥션을 붙들고 있지 않게 하려는 것이다.
