...
부하 중에는 문서를 찾을 시간이 없다. 그래서 지금 읽는다. 각 문서에서 **읽을 자리**만 적었다.

**먼저 읽는다**

- [파드에 CPU·메모리 리소스 할당](https://kubernetes.io/docs/tasks/configure-pod-container/assign-cpu-resource/) · CPU 요청이 초과될 때의 동작을 설명하는 부분. CPU limit 을 걸지 않는 판단의 근거다.
- [MySQL — 옵티마이저가 인덱스를 쓰는 방식](https://dev.mysql.com/doc/refman/8.0/en/mysql-indexes.html) · WHERE 절과 인덱스의 관계를 설명하는 앞부분. 인덱스 하나가 성능 축을 좌우하는 이유가 여기 있다.
...
- [RDS Proxy 개념](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/rds-proxy.html) · 커넥션 풀링과 다중화 항목. 파드가 늘어도 백엔드 커넥션이 늘지 않는 구조를 설명한다.
- [WAF — Web ACL 규칙 평가 순서](https://docs.aws.amazon.com/waf/latest/developerguide/web-acl-processing.html) · 규칙 우선순위와 종료 동작 항목. 스코프다운과 텍스트 변환이 어느 층에서 도는지가 갈리는 자리다.
- [k6 — 시나리오 실행기](https://grafana.com/docs/k6/latest/using-k6/scenarios/) · `ramping-vus` 와 `constant-arrival-rate` 두 항목. 부하 프로파일 3단이 이 두 실행기로 만들어진다.
...
1. **스케줄링** — 어느 노드에 배치할지 정한다. t3.medium(allocatable 약 1930m)에 250m짜리 파드는 7개까지 들어간다.
...
이 둘 외에 `requests`가 실사용량을 제한하지는 않는다. requests를 0.1코어로 잡은 파드도 노드에 여유가 있으면 1코어를 쓴다. 그래서 requests를 낮추면 앱이 느려지는 것이 아니라, **같은 사용량에서 이용률이 커져 HPA가 더 빨리 늘린다.** requests 0.8코어인 파드가 1.0코어를 쓰면 이용률은 125%이고, 목표가 60%면 파드가 늘어난다.

:::note[사용량과 이용률은 다른 값이다]
사용량은 절대값(`kubectl top pods`의 `1024m`)이고, 이용률은 그 값을 requests로 나눈 백분율(`kubectl get hpa`의 `125%/60%`)이다. requests를 바꾸면 사용량은 그대로인데 이용률만 움직인다 — 부하 중에 requests를 건드리면 지표가 통째로 재해석되므로, 노브 하나 규칙에서 이것이 가장 위험한 노브다.

**requests가 선언되지 않으면 이용률 자체가 계산되지 않는다.** HPA가 `<unknown>/60%`를 띄우고 스케일이 걸리지 않는다.
:::
...
t3.medium 한 대에 시스템 파드 약 850m를 먼저 떼고 앱 3종을 얹으려면 파드당 250m가 균형점이다. allocatable 1930m의 약 13%다. requests를 작게 잡아도 CPU limit이 없으므로 실제 처리량은 그보다 크다.
...
그래서 메모리는 request = limit로 고정한다. 두 값을 벌려 두면 스케줄러는 작은 request 기준으로 파드를 밀어 넣는다. 실사용이 limit 근처까지 오르면 노드 메모리가 먼저 바닥나 다른 파드까지 죽는다.
...
정답지가 택한 방법은 **모든 규칙에 API 경로 화이트리스트를 스코프 다운으로 붙이는 것**이다. 알려진 API 경로가 아니면 WAF 평가에 아예 들어가지 않고 ALB 의 기본 액션 404 로 떨어진다. 기각된 대안들도 같이 기억해 둘 값어치가 있다 — 관리형 규칙 그룹을 `and_statement` 로 감싸는 것은 **WAF 가 중첩을 금지**해서 안 된다. 404 전용 차단 규칙을 따로 두면 화이트리스트를 두 벌 관리하게 된다. 소문자 변환(`LOWERCASE`)은 **ALB 경로 패턴이 대소문자를 구분**하는 탓에 두 계층의 판정이 어긋난다.
:::
...
점수 구조는 [21. 이론의 비용 최적화 절](/part-5/21-task3-deploy/theory/#비용-최적화--최소화가-아니라-밴드-맞추기)에 있다. 여기서는 부하 중에 그 값을 움직이는 노브를 본다.

밴드 안에 머무는 구조는 세 층으로 만든다.

- HPA 목표 이용률 60% — 파드가 포화되기 전에 늘려 스케일 리드타임을 벌어 둔다.
- NodePool `limits.cpu` — 폭주 상한. HPA 전체 상한(3앱 × 10파드 × 250m = 7.5 vCPU)에 데몬셋 오버헤드를 얹어 t3.medium 6대 = 12 vCPU.
- Karpenter consolidation 30초 — 스파이크가 끝나면 빈 노드를 즉시 회수한다. `budgets` 의 `nodes: "1"`이 회수 중 가용성을 지킨다.

유휴 1대에서 부하 최대 7대 사이를 오가는 구성이다. 그 안에서 실제로 어느 구간의 노드 시간이 큰지는 재 봐야 알 수 있고, 재는 절차가 [Drill 22](/part-5/22-task3-tuning/drill/)다.
...
Karpenter NodePool 의 네 가지 결정이다. 인스턴스 타입을 하나로 고정해 비용 계산을 예측 가능하게 하고, `limits.cpu` 로 폭주 시 상한을 건다. `consolidateAfter: 30s` 는 빈 노드를 빨리 회수해 비용 비율을 낮춘다. `budgets` 의 `nodes: "1"` 은 자발적 중단을 한 번에 한 대로 제한해 회수 도중 가용성이 흔들리지 않게 한다.
...
기대하는 것이 셋이다. 트래픽 대부분이 한 주소에서 온다는 것, 민감도를 올린 규칙이 정상 요청을 대량 차단한 사고, 인코딩된 공격이 관리형 규칙을 통과한 기록이다.
...
파드의 CPU `requests` 를 250m에서 100m로 낮췄다. 실제로 바뀌는 것을 모두 고르라.
