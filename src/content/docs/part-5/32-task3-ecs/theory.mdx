...
ENI(Elastic Network Interface)는 리소스에 붙는 가상 랜카드이고([vpc-basics](/start/vpc-basics/)), HPA(Horizontal Pod Autoscaler)는 파드 수를 지표로 조절한다([autoscaling-basics](/start/autoscaling-basics/)). 이 모듈에서 태스크 밀도를 정하는 것이 앞의 값이다.
