...
- 드릴 1회는 3시간 안에 끝내고 destroy한다 ([README.md](/part-6/14-task3-load-ops/)의 과금 절차).
...
부하를 돌린 채로 [../../reference/troubleshooting.md](/reference/troubleshooting/)의 "부하 운영(3과제)" 10개 항목을 하나씩 재현하고 복구한다. 규칙은 모듈 12와 같다 — **확인 명령 → 원인 → 조치**, 추측 수리는 실패 처리.
...
종료 후 부하 중지 → 스택 destroy → [../../reference/cleanup-check.md](/reference/cleanup-check/).
