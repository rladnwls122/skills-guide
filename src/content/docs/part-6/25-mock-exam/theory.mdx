...
선행 지식과 학습 경로는 [모듈 개요](/part-6/25-mock-exam/)에. 처음 보는 약어나 말은 [이 사이트의 말](/reference/glossary/)에 있다.

OAC(CloudFront 만 버킷을 읽게 하는 장치)는 [s3-basics](/start/s3-basics/), IRSA(파드에 IAM 역할을 붙이는 방식)는 [eks-basics](/start/eks-basics/)에 있다.
...
4시간을 어떻게 겹쳐 쓰는지를 그린 시간 계획이다. 두 군데서 갈라지는 점선이 핵심이다 — 클러스터가 만들어지는 20분 동안 이미지 빌드를 병렬로 돌리고, CloudFront 전파를 기다리는 동안 검증 시드를 준비한다. 기다리는 시간에 다른 일을 넣지 않으면 그만큼이 그대로 손해다. 마지막은 리셋 체크리스트를 거쳐 채점 스크립트를 직접 돌리는 순서다.
...
:::tip[15분 규칙]
- 막히면 **15분까지는 자력** (16-break-fix에서 만든 확인→원인→조치 사슬).
- 15분 초과 시 저장소 README 참조 허용 — 단 **참조한 순간 오답 노트에 기록**. 참조 없이 못 푼 것 = 실전에서 못 푸는 것.
:::

## mark가 시험지다

구현 전에 mark 항목 표(15-mutation-drill 훈련 ③ 산출물)를 머리에 올리고 시작한다.

:::caution[검증 직전 리셋 체크리스트]
[mark 스크립트 가이드](/reference/mark-script-guide/)의 **리셋 체크리스트**를 기계적으로 수행한다.

- 시연 데이터 정리 (재현 절차는 정확히 1회)
- 상태 복원
- 트래픽 시드
- 임시 리소스 제거
- 채점 신원 확인
...
종료 후 아무것도 안 보고 아키텍처를 그린다 — **화살표마다 인증 방식 주석**(OAC SigV4 / 커스텀 헤더 / IRSA / IAM 역할)이 달려야 완성. 화살표의 인증을 모르면 break-fix ⑦⑧⑪을 실전에서 못 푼다.
...
- [ ] 참조하고 넘어간다 — 기록까지 하는 건 시간 낭비다
  > 참조 없이 못 푼 것 = 실전에서 못 푸는 것이다. 기록이 없으면 같은 항목에서 다시 똑같이 막힌다. 기록이 모의고사의 산출물이다.
- [ ] 참조는 금지 — 자력으로 풀 때까지 붙든다
  > 4시간은 유한하다. 한 항목에 30분을 태우면 손도 못 댄 항목이 생겨 점수 손실이 훨씬 크다.
- [ ] 해당 항목을 버리고 다음으로 넘어간다

**README 참조 허용 + 오답 노트 기록.** 15분까지는 자력으로 확인→원인→조치 사슬을 돌린다. 참조한 순간 기록하는 이유는 단순하다 — 참조 없이 못 푼 것은 실전에서 못 푸는 것이고, 그 목록이 남은 기간의 우선 복습 대상이다.
</Quiz>

<Quiz title="검증 직전 리셋 체크리스트">
검증 직전 리셋 체크리스트에 들어가는 항목을 모두 고르라.
...
화살표의 인증을 모르면 break-fix ⑦⑧⑪(OAC 403, 커스텀 헤더 우회, kubectl Unauthorized 계열)을 실전에서 못 푼다. 장애의 상당수가 "연결은 되는데 인증이 안 되는" 형태이기 때문이다.
</Quiz>
