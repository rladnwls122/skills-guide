...
2과제의 스케일링 모듈은 **Pod를 늘리는 층**과 **노드를 늘리는 층**이 따로 돌아간다. [모듈 10](/part-4/10-scaling-logging-streaming/) 이론은 그 두 층이 이미 머릿속에 있다는 전제로 시간 예산과 채점 함정만 다루므로, KEDA가 HPA와 무슨 관계인지나 Karpenter가 왜 노드그룹을 안 쓰는지가 흐릿하면 여기서 먼저 채운다.
...
**2층은 1층의 결과를 보고 움직인다.** 노드 오토스케일러는 메트릭을 보지 않는다 — 오직 "스케줄되지 못한 Pod가 있는가"만 본다. 그래서 Pod가 늘어도 기존 노드에 다 들어가면 노드는 한 대도 안 늘어난다. 채점이 "노드 2대 이상"을 기대한다면 그건 **리소스 요청량으로 설계된 결과**이지 Pod 개수로 정해지는 게 아니다.
...
HPA는 **약 15초 주기**로 메트릭을 다시 읽고 판단한다(컨트롤 플레인 설정). 이것이 감지 지연의 하한이다.
...
`minReplicaCount ≥ 1`이면 0↔1 전환 자체가 없으므로 **`pollingInterval`은 아무 일도 하지 않는다.** KEDA 2.x는 이 조합에 "설정했지만 관련 없다"는 경고까지 낸다. 반대로 `minReplicaCount: 0`이면 활성화 감지 지연을 이 값이 그대로 결정하므로 **반드시 설정해야 한다.**
...
`spec.disruption.consolidationPolicy`가 언제 노드를 정리할지 정한다. `WhenEmpty`는 완전히 빈 노드만, `WhenEmptyOrUnderutilized`는 놀고 있는 노드의 Pod를 다른 노드로 옮겨서까지 정리한다. `consolidateAfter`는 그 판단까지의 대기 시간이다.
...
컨트롤러 노드가 1대로 고정된 set-07에서는 Karpenter 복제본을 1로 낮춰야 설치가 끝난다(기본 2인데 두 번째가 앉을 노드가 없다). set-08은 Fargate라 그 문제 자체가 없다. **같은 소프트웨어인데 배치가 달라 함정이 갈리는** 사례다.

상세 채점 항목은 [모듈 10 이론](/part-4/10-scaling-logging-streaming/theory/)에 있다.
...
`requests.cpu`가 500m인 Pod 5개면 합계 2.5 vCPU다. 2 vCPU짜리 인스턴스의 할당 가능 용량은 시스템 예약분을 빼면 약 1.9 vCPU이므로 다 들어가지 못하고, 못 들어간 Pod가 Pending이 되어 두 번째 노드를 부른다. **`requests`를 낮추면 전부 한 대에 들어가 노드가 안 늘어난다.**
...
| 증상 | 원인 | 확인 |
|---|---|---|
| ScaledObject는 READY인데 Pod가 안 늘어난다 | KEDA가 큐를 못 읽는다(권한). 큐를 읽는 주체는 앱이 아니라 오퍼레이터 | `kubectl describe hpa` 의 메트릭 값, KEDA 오퍼레이터 로그 |
...
<LinkButton href="/part-4/10-scaling-logging-streaming/" icon="right-arrow">모듈 10 — 오토스케일링 + 로깅 + 스트리밍</LinkButton>
<LinkButton href="/start/serverless-basics/" variant="minimal">서버리스·이벤트 기초</LinkButton>
