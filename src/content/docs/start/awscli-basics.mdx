...
**선수 학습 7/18.** [iam-basics](/start/iam-basics/)를 먼저 읽는다 — CLI 가 다루는 것이 결국 신원과 권한이다.
...
```mermaid
flowchart LR
    CLI@{ icon: "aws:cli", form: "square", label: "aws ec2 describe-vpcs", pos: "b", h: 46, w: 46 }
    CLI --> Q["--query 'Vpcs[0].VpcId'<br/>CLI 내장 JMESPath"]
    CLI --> J["| jq -r '.Vpcs[0].VpcId'<br/>외부 도구, 문법 다름"]
    Q --> V["vpc-0abc123"]
    J --> V
```
...
같은 `aws ec2 describe-vpcs` 응답에서 VPC ID 하나를 뽑는 두 갈래다. 위쪽은 CLI 가 응답을 받은 뒤 자체 JMESPath 엔진으로 거르는 `--query` 이고, 아래쪽은 응답 JSON 전체를 표준 출력으로 흘려 `jq` 에 넘기는 파이프다. 결과값 `vpc-0abc123` 은 같지만 문법이 서로 다르므로 한쪽 표현을 다른 쪽에 그대로 옮겨 쓸 수 없다.
...
기본 VPC가 있는 계정에서 ([iam-basics](/start/iam-basics/)의 사용자로):
...
둘은 실행 주체부터 다르다. `--query` 는 AWS CLI 안에 들어 있는 JMESPath 엔진이고, jq 는 파이프 뒤에서 도는 별개 프로그램이다.

| | `--query` | jq |
|---|---|---|
| 소속 | AWS CLI 내장 | 외부 도구 |
| 문법 | JMESPath | jq 문법 |
| 같은 표현 | `Vpcs[0].VpcId` | `.Vpcs[0].VpcId` |
| raw 값 얻기 | `--output text` | `-r` |

채점 스크립트(mark.sh)는 주로 jq 를 쓴다. 그래서 두 문법을 다 읽을 수 있어야 "채점이 어떤 필드를 보는지"를 역추적할 수 있다.
</Quiz>
...
AWS 를 부르는 법까지 끝났다. 다음은 정답지를 읽는 데 필요한 Terraform 문법이다. 채점 역추적 훈련은 [PART 6 — Battle Drills](/part-6/23-mutation-drill/)에서 이어진다.

<LinkButton href="/start/hcl-basics/" icon="right-arrow">다음 선수 학습 — HCL 기초</LinkButton>
