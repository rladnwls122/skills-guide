...
**선수 학습 9/18.** 앞선 문서를 전제하지 않는다. 관계형 데이터베이스를 써 봤다면 순서가 반대라는 점부터 본다.
...
이름이 하는 일을 그대로 담고 있다. 파티션 키는 DynamoDB 가 항목을 **어느 물리 파티션에 둘지** 정하는 데 쓴다. 정렬 키는 같은 파티션 안에서 **어떤 순서로 늘어놓을지** 를 정한다. 정렬 키가 있어야 "최근 것부터" 같은 범위 조회가 성립한다.
