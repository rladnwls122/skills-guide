...
노드당 파드 상한은 EKS 최적화 AMI 기준 `(ENI 수 × (ENI당 IP 수 − 1)) + 2` 로 정해진다. `+ 2` 는 파드 IP 를 쓰지 않는 VPC CNI 와 `kube-proxy` 몫이다. `t3.medium` 은 ENI 3개 × IP 6개라 `3 × 5 + 2 = 17` 개이고, 이 상한은 CPU·메모리가 남아 있어도 적용된다.
