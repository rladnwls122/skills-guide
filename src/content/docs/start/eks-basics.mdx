...
**선수 학습 12/18.** [k8s-basics](/start/k8s-basics/) · [iam-basics](/start/iam-basics/) · [vpc-basics](/start/vpc-basics/) 셋 위에 선다. IRSA 가 그 셋이 만나는 자리다.
...
EKS 가 어디까지 관리하고 사용자 VPC 에 무엇이 남는지를 가른다. API 서버·etcd·스케줄러는 AWS 관리 영역에 있어 사용자가 접속할 수는 있어도 손댈 수 없다. 워커 노드는 사용자 VPC 의 프라이빗 서브넷에 뜨는 EC2 다. 노드의 kubelet 이 API 서버 지시를 받아 파드를 띄우고 상태를 되보고한다. 파드가 받는 IP 는 가상 대역이 아니라 그 서브넷의 VPC IP 다 — 서브넷 CIDR 이 좁으면 파드 수가 먼저 막힌다.
...
IRSA 로 파드가 AWS 자격 증명을 얻는 순서다. 파일로 마운트된 프로젝티드 토큰을 읽어 `AssumeRoleWithWebIdentity` 를 부른다. 그러면 STS 가 역할의 신뢰 정책을 보고 OIDC 발급자와 `sub`(네임스페이스·서비스 계정 이름)가 맞는지 확인한다. 통과하면 임시 자격 증명이 나오고 그것으로 AWS API 를 부른다. 액세스 키를 파드에 넣지 않는 이유가 이 흐름이다.
...
셋의 의존 순서가 곧 장애 진단 순서다. **CNI(IP) → kube-proxy(라우팅) → CoreDNS(이름)**. CoreDNS 파드가 IP 를 못 받으면 클러스터 전체의 이름 해석이 죽는다. 증상은 "이미지 pull 실패"·"서비스 연결 안 됨"처럼 엉뚱하게 나타난다. 이름 해석이 의심되면 `kubectl -n kube-system get pods -l k8s-app=kube-dns` 를 먼저 본다.
...
AWS Load Balancer Controller 는 클러스터 안에서 Ingress·Service 리소스를 감시한다. 그러다 **실제 ALB·NLB 를 AWS API 로 만든다.** 그래서 이 컨트롤러에는 ELB를 만들 IAM 권한이 필요하고, 그 권한을 IRSA로 준다.

ALB 자체의 구조는 [로드밸런서 기초](/start/loadbalancer-basics/)에 있다. 리스너·규칙·타깃그룹·헬스체크가 거기 있다. 타깃 유형 `instance` 와 `ip` 의 차이, Ingress 와 TargetGroupBinding 중 무엇을 쓸지도 같은 문서에 있다.
