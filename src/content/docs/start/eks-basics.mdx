...
- 이 내용은 **PART-2 모듈 04(eksctl 클러스터)** 와 **모듈 05(k8s 워크로드·ALB)** 의 기반이다.
...
노드당 파드 상한은 대략 `(ENI 수 × ENI당 IP 수 - 1)` 이다. `t3.medium` 은 ENI 3개 × IP 6개로 17개 남짓이고, 이 상한은 CPU·메모리가 남아 있어도 적용된다.
...
Ingress가 `ADDRESS` 를 받지 못하고 비어 있으면 순서대로 본다 — ① 컨트롤러 파드가 `Running` 인가, ② 컨트롤러의 IRSA 역할에 ELB 권한이 있는가, ③ 서브넷 태그(`kubernetes.io/role/elb`)가 붙어 있는가.
...
- [ ] 파드가 VPC IP 를 받았는지
  > 파드 IP 는 대상 그룹을 채우는 단계에서 필요하다. ALB 자체가 만들어지지 않아 주소가 비는 것과는 층위가 다르다.
...
모듈 04에서 `eksctl` 로 클러스터를 세우고, 모듈 05에서 워크로드와 ALB를 올린다.
...
- [AWS Load Balancer Controller](https://kubernetes-sigs.github.io/aws-load-balancer-controller/latest/) — Ingress 애노테이션과 대상 그룹 모드
