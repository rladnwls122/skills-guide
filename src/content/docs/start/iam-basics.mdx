...
:::note[대회 계정 지급 방식 — 지도교사 협의회(2026-07-31)]
대회장에서는 **PowerUser 이상 권한의 IAM 계정**이 지급되고 root 계정은 주지 않는다. 두 가지를 대비한다.

- 관리형 정책 `PowerUserAccess`는 `iam:CreateRole` 같은 IAM 쓰기를 막는다. 과제는 Lambda 실행 역할·IRSA 역할 등 IAM 역할 생성을 요구하므로 실제 지급 권한은 그보다 넓어야 한다 — 시작 직후 `aws sts get-caller-identity`로 신원을 확인하고, 첫 terraform apply가 IAM 역할 생성 단계에서 AccessDenied를 내는지 가장 먼저 본다.
- **기존에 생성돼 있던 리소스를 삭제하지 못하게 하는 정책이 걸릴 수 있다.** 지급 리소스 삭제가 필요한 과제는 없다 — 삭제 계열 AccessDenied가 나면 내가 만든 리소스인지부터 확인하고, 지급 리소스면 그대로 둔다.
:::
