...
:::note[대회 계정 지급 방식 — 지도교사 협의회 회의록]
대회장에서는 **`PowerUserAccess` 수준 권한의 IAM 사용자**가 지급되고 root 계정은 주지 않는다([대회 운영 규칙](/reference/competition-rules/)). 두 가지를 대비한다.

- 관리형 정책 `PowerUserAccess`는 `iam:CreateRole` 같은 IAM 쓰기를 막는다. 과제는 Lambda 실행 역할·IRSA 역할 등 IAM 역할 생성을 요구하므로 실제 지급 권한은 그보다 넓어야 앞뒤가 맞는다 — 시작 직후 `aws sts get-caller-identity`로 신원을 확인하고, 첫 terraform apply가 IAM 역할 생성 단계에서 AccessDenied를 내는지 가장 먼저 본다.
- **사용자 정책에 명시적 Deny가 들어갈 수 있다.** 사전 제공 리소스의 삭제와 대회 운영 방해를 막는 목적이고, 특정 EC2 인스턴스 삭제 권한처럼 PowerUserAccess 범위의 일부가 막힐 수 있다. 지급 리소스 삭제가 필요한 과제는 없다 — 삭제 계열 AccessDenied가 나면 내가 만든 리소스인지부터 확인하고, 지급 리소스면 그대로 둔다.
:::
...
상한을 정하는 층(SCP·권한 경계·세션 정책)은 **권한을 주지 않는다**. 허용은 신원 기반 정책과 리소스 기반 정책에서만 나오고, 통과하려면 모든 층을 다 통과해야 한다. "`AdministratorAccess` 를 붙였는데 거부된다"의 원인이 대개 SCP나 권한 경계인 이유다. 단, 교차 계정 요청은 예외가 하나 있다 — 계정이 다르면 호출자 계정의 신원 기반 정책과 대상 계정의 리소스 기반 정책이 **둘 다** Allow 해야 하고, 같은 계정 안에서는 리소스 기반 정책 하나만으로도 성립한다.
</Quiz>
...
홉 제한 1은 파드에서 IMDS로 노드 역할의 자격 증명을 훔쳐 쓰는 것을 막는 설정이다. 증상만 보고 홉 제한을 늘려 넘어가지 말고 IRSA로 전환하는 것이 정석이고, 이 내용이 모듈 08의 주제다. 참고로 IMDSv2는 `PUT` 으로 받은 토큰을 헤더에 실어야 응답하며, 이는 SSRF로 자격 증명이 새는 경로를 막기 위한 설계이자 신규 인스턴스의 기본값이다.
</Quiz>
...
<LinkButton href="/part-3/08-private-eks-iam/" variant="minimal">PART 3 — Hard Mode</LinkButton>
