...
**선수 학습 2/18.** 앞선 문서를 전제하지 않는다. 이 문서의 정책 문법이 뒤의 S3·KMS·EKS 에서 계속 나온다.
...
`arn:aws:iam::<계정ID>:root` 는 뜻이 따로 굳은 관용 표기다. root 사용자 한 명이 아니라 **그 계정 전체**를 가리킨다. 이 값을 신뢰 정책에 쓰는 것은 "누가 이 역할을 맡을지는 그 계정의 IAM 이 정하라"고 판단을 넘기는 뜻이다. 실제 허용 여부는 그 계정의 신원 기반 정책이 정한다.
...
`Principal` 은 **리소스 기반 정책에만 있는 요소**다. 신원 기반 정책은 이미 특정 사용자나 역할에 붙어 있어 대상이 정해져 있다. 리소스 기반 정책은 리소스 쪽에 붙으므로 "누구에게"를 정책 안에 따로 적어야 한다. 역할의 신뢰 정책과 KMS 키 정책이 리소스 기반 정책이고, 그래서 둘 다 `Principal` 을 가진다.
...
양쪽이 다 있어야 성립한다. 계정 B 의 신뢰 정책에 `:root` 를 쓰는 것은 "계정 A 의 IAM 관리자에게 위임한다"는 뜻이다. 실제로 누가 맡을지는 계정 A 의 신원 기반 정책이 정한다.

제3자에게 역할을 열어 줄 때는 `sts:ExternalId` 조건을 건다. 이 조건이 막는 것이 **대리인 혼동**(confused deputy)이다. 같은 서비스를 쓰는 다른 고객이 역할 ARN 만 알아내 그 역할을 맡아 버리는 문제다. 양쪽만 아는 External ID 를 함께 요구하면 ARN 을 알아도 맡지 못한다.
