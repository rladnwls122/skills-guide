...
클러스터의 두 층이다. 위층 컨트롤 플레인에서 `kubectl` 요청은 kube-apiserver 로만 들어가고, scheduler 가 Pod 을 어느 노드에 놓을지 정하며, etcd 가 그 결과 상태를 보관한다. 아래층 노드에는 실제 컨테이너가 든 Pod 이 뜬다. Service 는 노드와 무관하게 Pod 을 라벨로 골라 하나의 가상 IP 뒤에 묶는다 — Pod 이 죽고 새로 떠 IP 가 바뀌어도 호출하는 쪽 주소가 그대로인 이유다.
...
두 값 중 대회에서 더 중요한 것은 `requests` 다. 스케줄러가 자리를 고르는 기준이자, 오토스케일링이 부하율을 계산하는 분모이기 때문이다. HPA 가 "CPU 사용률 50%" 를 판정할 때 나누는 값은 `limits` 가 아니라 `requests` 이고, 노드를 늘릴지 정하는 판단도 노드에 남은 `requests` 합계를 본다. 계산 방식은 [오토스케일링 기초](/start/autoscaling-basics/)에서 다룬다.
