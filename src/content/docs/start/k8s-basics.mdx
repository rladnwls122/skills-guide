...
**선수 학습 10/18.** [docker-basics](/start/docker-basics/)를 먼저 읽는다 — 컨테이너를 모르면 파드가 무엇을 담는지 잡히지 않는다.
...
flowchart TB
    subgraph CP["<span class='icon--logos icon--logos--aws-eks'></span> 컨트롤 플레인 (EKS가 관리)"]
        API@{ icon: "k8s:control-plane", form: "square", label: "kube-apiserver<br/>(kubectl이 여기로 요청)", pos: "b", h: 46, w: 46 }
        SCHED@{ icon: "k8s:control-plane", form: "square", label: "scheduler<br/>(Pod을 어느 노드에 놓을지)", pos: "b", h: 46, w: 46 }
        ETCD@{ icon: "k8s:etcd", form: "square", label: "etcd<br/>(상태 저장소)", pos: "b", h: 46, w: 46 }
    end
    subgraph N1["<span class='icon--logos icon--logos--aws-ec2'></span> 노드 1 (EC2)"]
        P1@{ icon: "k8s:pod", form: "square", label: "Pod<br/>app 컨테이너", pos: "b", h: 46, w: 46 }
        P2@{ icon: "k8s:pod", form: "square", label: "Pod<br/>app 컨테이너", pos: "b", h: 46, w: 46 }
    end
    subgraph N2["<span class='icon--logos icon--logos--aws-ec2'></span> 노드 2 (EC2)"]
        P3@{ icon: "k8s:pod", form: "square", label: "Pod<br/>app 컨테이너", pos: "b", h: 46, w: 46 }
    end
    SVC@{ icon: "k8s:svc", form: "square", label: "Service (ClusterIP)<br/>안정적인 가상 IP + DNS", pos: "b", h: 46, w: 46 }
...
클러스터의 두 층이다. 위층 컨트롤 플레인에서 `kubectl` 요청은 kube-apiserver 로만 들어간다. scheduler 가 Pod 을 어느 노드에 놓을지 정하고, etcd 가 그 결과 상태를 보관한다. 아래층 노드에는 실제 컨테이너가 든 Pod 이 뜬다. Service 는 노드와 무관하게 Pod 을 라벨로 골라 하나의 가상 IP 뒤에 묶는다 — Pod 이 죽고 새로 떠 IP 가 바뀌어도 호출하는 쪽 주소가 그대로인 이유다.
...
### 2-6. 리소스 요청과 상한 — requests / limits

컨테이너마다 CPU·메모리를 두 값으로 적는다. 매니페스트의 `spec.containers[].resources` 아래에 들어간다.

- **`requests`** — 스케줄러가 이 Pod 을 놓을 노드를 고를 때 **확보해 두는 양**이다. 노드에 남은 용량이 이 값보다 작으면 그 노드에는 배치되지 않는다. 컨테이너가 실제로 얼마나 쓰는지와 무관하게 예약된 몫으로 잡힌다.
- **`limits`** — 컨테이너가 **넘지 못하는 상한**이다. CPU 는 상한에 닿으면 속도가 깎이고, 메모리는 넘는 순간 컨테이너가 강제 종료된다. 종료된 컨테이너의 상태는 `OOMKilled` 로 찍힌다.
...
CPU 의 `500m` 은 **밀리코어** 표기다. `1000m` 이 vCPU 하나이므로 `500m` 은 0.5개다. 소수 대신 정수로 적으려고 쓰는 단위다. 메모리의 `Mi` 는 1024 기반 메비바이트다.

두 값 중 대회에서 더 중요한 것은 `requests` 다. 스케줄러가 자리를 고르는 기준이자, 오토스케일링이 부하율을 계산하는 분모이기 때문이다. HPA 가 "CPU 사용률 50%" 를 판정할 때 나누는 값은 `limits` 가 아니라 `requests` 다. 노드를 늘릴지 정하는 판단도 노드에 남은 `requests` 합계를 본다. 계산 방식은 [오토스케일링 기초](/start/autoscaling-basics/)에서 다룬다.
...
flowchart TB
    ...
    subgraph CLUSTER["<span class='icon--logos icon--logos--aws-eks'></span> EKS 클러스터"]
        ...
        subgraph NGAPP["<span class='icon--logos icon--logos--aws-ec2'></span> app 노드그룹 (label: node-type=app)"]
            ...
            subgraph NSAPP["<span class='icon--k8s icon--k8s--ns'></span> Namespace: app (채점이 ns 검사)"]
                DEP@{ icon: "k8s:deploy", form: "square", label: "Deployment<br/>replicas 2 · probe 3종<br/>topologySpread(AZ)", pos: "b", h: 46, w: 46 }
                POD1@{ icon: "k8s:pod", form: "square", label: "Pod", pos: "b", h: 46, w: 46 }
                POD2@{ icon: "k8s:pod", form: "square", label: "Pod", pos: "b", h: 46, w: 46 }
                PDB["PDB"]
                CM@{ icon: "k8s:cm", form: "square", label: "ConfigMap", pos: "b", h: 46, w: 46 }
                SEC@{ icon: "k8s:secret", form: "square", label: "Secret (KMS 암호화)", pos: "b", h: 46, w: 46 }
                SA@{ icon: "k8s:sa", form: "square", label: "ServiceAccount", pos: "b", h: 46, w: 46 }
                SVC@{ icon: "k8s:svc", form: "square", label: "Service (ClusterIP)", pos: "b", h: 46, w: 46 }
                TGB["TargetGroupBinding"]
            end
        end
        NSMON@{ icon: "k8s:ns", form: "square", label: "Namespace: monitoring<br/>(kube-prometheus-stack 리소스)", pos: "b", h: 46, w: 46 }
    end
...
대회 스택 한 벌이 클러스터 안에서 어떻게 이어지는지 전부 그린 그림이다. 트래픽은 CloudFront → ALB → 타깃그룹 → Pod 으로 내려가고, 그 마지막 연결을 TargetGroupBinding 이 만든다 — LBC 가 TargetGroupBinding 을 읽어 Pod IP 를 타깃그룹에 등록한다. 클러스터 안은 노드그룹 둘로 갈린다. addon 노드그룹에는 LBC·kube-prometheus-stack 같은 시스템 릴리스가 뜬다. app 노드그룹에는 앱 Deployment 가 nodeSelector·toleration 으로 배치된다. 점선은 트래픽이 아니라 설정·제어 관계다 — PDB 는 동시 중단 수를 제한하고, ServiceAccount 는 IRSA 로 IAM 역할과 이어진다.
...
| ALB → CloudFront | Terraform으로 생성 — 타깃그룹 이름 채점 때문에 Ingress를 안 씀 |
...
표에는 아직 정의하지 않은 용어가 섞여 있다. 각각 정의처가 따로 있다.

- ALB·타깃그룹·헬스체크 — [로드밸런서 기초](/start/loadbalancer-basics/)
- 노드그룹·IRSA·Pod Identity — [EKS 기초](/start/eks-basics/)
- taint 와 toleration, 안티어피니티 — [오토스케일링 기초](/start/autoscaling-basics/)
- probe 3종·topologySpreadConstraints·PDB — [PART-2 모듈 10 이론](/part-2/10-k8s-workloads-alb/theory/)

지금은 이름과 그림에서의 자리만 익히고 넘어간다.

## ④ 대회에서 어떻게 쓰이나

- **Deployment가 채점의 중심**: replicas, liveness/readiness/startup probe, nodeSelector·toleration(전용 노드그룹 배치), topologySpreadConstraints(AZ 분산), PDB, preStop 훅까지 요구된다 — 전부 **PART-2 모듈 10**에서 심화.
- **Service는 ClusterIP만**: 외부 노출은 Ingress가 아니라 **Terraform으로 만든 ALB + TargetGroupBinding**(Pod IP를 타깃그룹에 직접 등록). Ingress로 만들면 ALB 이름은 애노테이션으로 맞출 수 있어도 **타깃그룹 이름을 지정할 수 없어서** "타깃그룹 이름 채점"을 통과 못 한다.
- **ConfigMap**으로 앱 설정(DB 호스트 등)을 주입하고, **Secret**은 KMS envelope 암호화(모듈 13)의 대상이 된다.
- **ServiceAccount**는 IRSA/Pod Identity로 AWS IAM 권한과 연결된다 — "Pod이 S3에 접근하는 방법". 개념만 잡고 PART-2 모듈 09에서 심화.
- **Helm**은 AWS Load Balancer Controller, kube-prometheus-stack 설치에 쓴다. **차트 버전 고정**이 원칙(버전을 고정하지 않으면 다음에 깔 때 다른 버전이 들어온다).
...
막혔을 때 치는 여섯 명령이다. `apply -f k8s/` 는 디렉터리 안 파일을 이름 순으로 적용하므로 번호 접두어가 곧 적용 순서다. 원인 추적은 `describe pod` 의 Events 부터 본다 — 이미지 pull 실패·스케줄 불가·프로브 실패가 전부 거기 찍힌다. `logs --previous` 는 재시작하기 직전 컨테이너의 로그라 크래시 원인을 볼 때 쓴다. ConfigMap 만 바꾸면 Pod 은 그대로이므로 `rollout restart` 로 순차 교체한다.
...
로컬이 부담스러우면 생략 가능 — **PART-2 모듈 09~12에서 EKS로 동일 내용을 실습**한다.
...
- [x] Ingress가 만드는 타깃그룹은 이름을 지정할 수 없어 "타깃그룹 이름 채점"을 통과할 수 없다. 대신 Terraform으로 ALB와 타깃그룹을 만들고 TargetGroupBinding으로 Pod IP를 직접 등록한다
- [ ] Ingress를 쓰면 AWS Load Balancer Controller를 추가로 설치해야 하는데 그 시간이 아깝기 때문
  > LBC는 **어느 쪽이든 필요하다**. TargetGroupBinding을 읽어 Pod IP를 타깃그룹에 등록해 주는 것도 LBC다. 설치 여부가 아니라 **타깃그룹 이름을 누가 정하느냐**가 갈림길이다.
...
- [ ] Ingress로는 ALB 이름도 타깃그룹 이름도 지정할 수 없기 때문

채점이 **타깃그룹의 이름**을 보기 때문이다. ALB 이름은 `alb.ingress.kubernetes.io/load-balancer-name` 애노테이션으로 맞출 수 있지만, 타깃그룹 이름을 정하는 애노테이션은 없다 — 컨트롤러가 `k8s-` 로 시작하는 이름을 자동으로 짓는다. 그래서 ALB와 타깃그룹은 **Terraform으로 이름까지 정해서** 만들고, 그 타깃그룹에 `TargetGroupBinding`으로 Service 뒤의 Pod IP를 직접 등록한다. 흐름: CloudFront → ALB(Terraform) → TargetGroup ← TargetGroupBinding ← Service(ClusterIP) → Pod.
</Quiz>
...
`describe` 의 **Events 섹션이 디버깅 1순위**다. ImagePullBackOff, Insufficient cpu, Readiness probe failed 가 전부 여기 찍힌다. 그다음이 `logs --previous` — 앱이 왜 스스로 종료했는지는 죽기 직전 로그에만 남는다.
</Quiz>
...
base64는 누구나 되돌릴 수 있는 **인코딩**이지 암호화가 아니다. EKS 클러스터의 Secret 암호화 설정으로 **KMS envelope 암호화**를 켜면, etcd에 저장되는 Secret이 KMS 키로 감싼 데이터 키로 암호화된다. PART-3 모듈 13에서 실습하며, 이때 쓰는 KMS 키 정책이 대회 유형의 **root-less** 정책이다 — root로 리소스를 만들어 두면 여기서 전부 막힌다.
</Quiz>
...
모듈 09(EKS·IRSA)와 모듈 10(k8s 워크로드 심화)에서 이 내용을 EKS 위에서 실전 수준으로 확장한다.

<LinkButton href="/start/loadbalancer-basics/" icon="right-arrow">다음 선수 학습 — 로드밸런서 기초</LinkButton>
