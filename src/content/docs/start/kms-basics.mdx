...
관리자 구문의 `Principal` 이 `"*"` 인 것도 같은 이유로 오해를 부른다. 전 세계 공개가 아니다 — 바로 아래 `kms:CallerAccount` 조건이 **이 계정에서 온 호출**로 범위를 좁히고, 계정 안에서 실제로 누가 쓸 수 있는지는 각 신원의 IAM 정책이 정한다. `arn:aws:iam::123456789012:root` 라고 쓰면 같은 뜻이지만, 채점이 정책 원문에서 `:root` 문자열을 찾아내므로 조건식으로 우회한 것이다. 배포자 ARN 을 `aws_iam_session_context` 로 조회해 직접 박는 방식도 같은 목적을 달성한다.

두 방식 모두 `kms:Put*` 이 관리 구문에 들어 있어야 Lockout Safety Check 를 통과한다.
