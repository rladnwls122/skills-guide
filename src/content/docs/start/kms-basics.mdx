...
SSE-KMS 로 암호화된 객체를 내려받을 때 실제로 일어나는 순서다. S3 는 암호화된 객체와 함께 저장해 둔 암호화된 DEK 를 꺼내 KMS 에 `kms:Decrypt` 를 요청하고, 돌려받은 평문 DEK 로 객체를 풀어 사용자에게 준다. 사용자가 KMS 를 직접 부르지 않지만 **호출자의 권한으로** 그 Decrypt 가 평가된다 — 버킷 정책이 허용해도 키 정책이 막으면 다운로드가 실패하는 이유다.
...
KMS 키 정책이 특정 S3 버킷이나 CloudFront 배포판의 ARN을 참조하고, 그 S3 버킷·CloudFront가 다시 KMS 키 ARN을 참조하면 고리가 닫힌다. **Terraform 종속성 순환 참조(Cycle Error)** 가 발생한다.
...
키 정책은 **KMS 키 하나에 붙어 있는 문서**다. 그 문서가 통제하는 대상은 붙어 있는 그 키뿐이므로 `Resource` 칸에 쓸 값이 애초에 하나밖에 없고, `"*"` 는 **이 정책이 붙은 키 자신**을 가리키는 관용 표기다. 계정의 다른 KMS 키에는 아무 영향이 없다.
