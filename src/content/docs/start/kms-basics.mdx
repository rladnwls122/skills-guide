...
**선수 학습 4/18.** [iam-basics](/start/iam-basics/)와 [s3-basics](/start/s3-basics/)를 먼저 읽는다 — 키 정책이 IAM 문법이고 예제가 버킷 암호화다.
...
모든 데이터를 마스터 키 하나로 직접 암호화하면 두 가지가 걸린다. 파일이 클수록 느려지고, 그 키가 그만큼 자주 쓰여 노출된다. 이를 해결하기 위해 AWS KMS는 **봉투 암호화(Envelope Encryption)** 구조를 사용한다.
...
버킷에 KMS 키를 미리 지정해 두면 S3 는 객체를 저장할 때마다 그 키로 암호화한다. 이 설정을 **SSE-KMS**, 풀어서 서버 측 암호화(Server-Side Encryption)의 KMS 방식이라고 부른다. 버킷 쪽에서 이 설정을 켜는 방법과 채점이 무엇을 보는지는 [S3 기초](/start/s3-basics/)에 있다.
...
봉투 암호화의 구조다. 실제 데이터를 암호화하는 것은 CMK 가 아니라 `GenerateDataKey` 로 받은 데이터 키(DEK)다. 서비스는 평문 DEK 로 데이터를 암호화한 뒤 그 DEK 를 CMK 로 암호화한 형태로 데이터 옆에 함께 저장한다. 읽을 때는 암호화된 DEK 를 CMK 로 `Decrypt` 해 되살린다. 그래서 큰 데이터가 KMS 를 오갈 일이 없고, CMK 접근을 막으면 데이터 전체가 함께 잠긴다.
...
SSE-KMS 로 암호화된 객체를 내려받을 때 실제로 일어나는 순서다. S3 는 객체 옆에 저장해 둔 암호화된 DEK 를 꺼내 KMS 에 `kms:Decrypt` 를 요청한다. 돌려받은 평문 DEK 로 객체를 풀어 사용자에게 준다. 사용자가 KMS 를 직접 부르지 않지만 **호출자의 권한으로** 그 Decrypt 가 평가된다 — 버킷 정책이 허용해도 키 정책이 막으면 다운로드가 실패하는 이유다.
...
KMS 키 정책은 IAM 정책과 다르게 동작한다. **키 정책이 명시적으로 허용하지 않으면 IAM 관리자도 root 사용자도 그 키를 쓰지 못한다.** 키 정책 설계가 잘못되면 누구도 키를 관리하거나 삭제할 수 없는 **락아웃(Lockout)** 상태가 된다.
:::
...
일부 과제는 **키 정책에 root principal 과 `kms:*` 와일드카드를 넣지 말 것**을 요구한다. 1과제 set-03 이 그렇다. 이를 **root-less 키 정책**이라 부른다.

*   **취지**: 계정 소유자(`root`)조차 키 정책에 명시되지 않으면 키를 쓰지 못하도록 고립시키는 설계다.
*   **구현 방법**:
    *   관리자 `Principal` 을 `"*"` 로 두고, `kms:CallerAccount` 조건으로 **내 계정에서 온 호출**만 통과시킨다. `arn:aws:iam::<계정ID>:root` 와 뜻은 같으면서 정책 원문에 `:root` 문자열이 남지 않는다. set-03 정답지가 다섯 키 전부에 쓰는 방식이고, 실물 JSON 은 ④-1 에 있다.
    *   `kms:*` 대신 관리·운영에 필요한 개별 API 액션을 전부 나열한다.
*   **Lockout Safety Check**: AWS KMS는 키 정책을 적용할 때 "이 정책이 **호출한 신원 본인**에게 이후의 `PutKeyPolicy` 를 허용하는가"를 검사한다. 검사 대상은 `kms:PutKeyPolicy` 하나이며(`kms:Create*` 는 무관하다 — 키 정책에서 그 와일드카드는 `CreateAlias`·`CreateGrant` 만 덮는다), 다른 관리자 ARN 에 그 권한을 줘도 통과하지 못한다. 실패하면 적용(apply) 단계에서 `MalformedPolicyDocumentException` 이 발생하고 생성이 거부된다. 그래서 관리 구문에 `kms:Put*`(`kms:PutKeyPolicy` 를 덮는다)를 반드시 넣고, 그 구문의 `Principal` 이 apply 를 실행하는 신원까지 포함하게 해야 한다 — 계정 위임 방식은 `kms:CallerAccount` 조건이 배포자를 이미 덮으므로 이 조건이 자동으로 만족된다.
...
키 정책이 S3 버킷이나 CloudFront 배포의 ARN 을 참조한다. 그 버킷과 배포가 다시 키의 ARN 을 참조하면 고리가 닫힌다. **Terraform 종속성 순환 참조(Cycle Error)** 가 발생한다.

*   **해결책**: 키 정책의 `Condition` 블록에서 구체적인 리소스 대신 계정 ID(`aws_caller_identity` 로 조회)나 선수 번호 변수를 조합해 동적으로 평가되도록 우회한다.

set-03 이 CloudFront 에 복호화를 허용하는 구문이 이 우회의 실물이다. CloudFront 배포 ARN 을 `Condition` 에 박으면 키 → 배포 → 버킷 → 키로 고리가 닫히므로, 배포 대신 **계정 ID** 로 범위를 좁힌다.

```hcl title="kms.tf — 순환 참조를 피하는 Condition"
statement {
  ...
  # 배포 ARN 을 쓰면 순환 참조. 계정 ID 는 배포보다 먼저 확정되므로 고리가 닫히지 않는다
  condition {
    test     = "StringEquals"
...
CloudFront 서비스가 이 키로 객체를 복호화하도록 허용하되, 요청이 **내 계정의** 배포에서 왔을 때만 통과시킨다. `aws:SourceAccount` 는 요청을 유발한 리소스의 소유 계정을 담는 조건 키다. 배포 ARN 을 쓰면 정확도는 올라가지만 Terraform 의 의존 그래프에 고리가 생겨 `Cycle` 오류로 apply 자체가 멈춘다. 계정 ID 는 `aws_caller_identity` 로 배포 전에 이미 알 수 있는 값이라 고리를 만들지 않는다.
...
### 1. root-less 키 정책 실물 — 적용된 JSON

set-03 정답지가 S3 버킷용 키에 실제로 붙이는 키 정책이다. 액션 목록만 줄여서 옮겼다.

```json title="wsc2026-bucket-kms 의 키 정책"
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
구문(Statement) 하나가 "누구에게 무엇을 허용하는가" 한 줄이고, 이 정책에는 그런 줄이 둘이다. 칸별로 읽는다.

| 칸 | 이 정책에서의 값 | 뜻 |
|---|---|---|
| `Sid` | `KeyAdministration` | 구문 이름. 사람이 읽기 위한 꼬리표이고 평가에는 쓰이지 않는다 |
| `Effect` | `Allow` | 허용인지 거부인지 |
| `Principal` | `{ "AWS": "*" }` · `{ "Service": "cloudfront.amazonaws.com" }` | 이 구문이 대상으로 삼는 신원 |
| `Action` | `kms:Put*` 등 | 허용하는 KMS API |
| `Resource` | `"*"` | 대상 리소스 |
| `Condition` | `kms:CallerAccount` · `aws:SourceAccount` | 추가로 만족해야 하는 조건 |

:::danger[`"Resource": "*"` 는 "모든 리소스"가 아니다]
키 정책은 **KMS 키 하나에 붙어 있는 문서**다. 통제하는 대상이 붙어 있는 그 키 하나뿐이라 `Resource` 칸에 쓸 값도 하나뿐이다. `"*"` 는 **이 정책이 붙은 키 자신**을 가리키는 관용 표기다. 계정의 다른 KMS 키에는 아무 영향이 없다.

사용자·역할에 붙는 신원 기반 IAM 정책의 `"Resource": "*"` 는 글자만 같고 뜻이 다르다. 그쪽은 진짜로 계정의 모든 키를 가리킨다. 같은 표기가 붙는 자리에 따라 뜻이 갈리는 지점이라 처음 보면 반드시 걸린다.
:::

관리자 구문의 `Principal` 이 `"*"` 인 것도 같은 이유로 오해를 부른다. 전 세계 공개가 아니다 — 바로 아래 `kms:CallerAccount` 조건이 **이 계정에서 온 호출**로 범위를 좁히고, 계정 안에서 실제로 누가 쓸 수 있는지는 각 신원의 IAM 정책이 정한다. `arn:aws:iam::123456789012:root` 라고 쓰면 같은 뜻이지만, 채점이 정책 원문에서 `:root` 문자열을 찾아내므로 조건식으로 우회한 것이다.

배포자 한 사람의 ARN 만 관리자로 박는 방법도 `:root` 를 피하기는 한다. 다만 그러면 그 신원이 아닌 사람은 키를 조회조차 못 한다 — 채점자가 지급 계정 root 로 접속해 `get-key-policy` 를 부르는 순간 전부 실패한다. set-03 정답지가 계정 위임을 고른 이유이고, 그래서 이 사이트도 계정 위임을 기준으로 설명한다.

어느 방식이든 `kms:Put*` 이 관리 구문에 들어 있어야 Lockout Safety Check 를 통과한다.

### 2. root-less 키 정책을 위한 Admin 액션 목록 정의
...
키 정책의 관리자 구문에 넣을 액션 목록이다. `kms:*` 를 쓰지 않고 필요한 접두어만 나열한 것이 요점이다 — 와일드카드 하나로 뭉뚱그리면 최소권한 판정에 걸리고, 반대로 너무 좁히면 키를 다룰 수 없게 잠긴다(lockout). `ScheduleKeyDeletion`·`CancelKeyDeletion` 처럼 되돌릴 수단이 함께 들어 있어야 실수해도 복구가 된다.

</details>

### 3. CLI를 이용한 KMS 키 상태 자가 검증
...
키 상태를 두 갈래로 확인한다. `describe-key` 는 `KeyState` 가 `Enabled` 인지와 `KeyManager` 가 `CUSTOMER`(직접 만든 CMK)인지 `AWS`(관리형 키)인지를 알려 준다. `get-key-policy` 는 접근 통제의 실체인 키 정책 본문을 그대로 낸다 — 별칭으로 조회하므로 키 ID 를 몰라도 된다.
...
# cipher.b64 는 base64 텍스트다. fileb:// 로 그대로 넘기면 실패하므로 디코드해서 넘긴다
...
암호화 컨텍스트를 붙여 한 바퀴 돌린다. 암호화할 때 준 키·값 쌍이 복호화 때 **하나라도 다르면** `InvalidCiphertextException` 이 난다 — 컨텍스트가 암호문에 묶이기 때문이다. 중간의 `base64 -d` 가 필요한 이유는 CLI 가 암호문을 base64 텍스트로 내주는데 `fileb://` 는 바이너리를 기대하기 때문이다.
...
데이터 키 캐싱은 **보안과 비용의 교환**이다. AWS Encryption SDK 의 캐싱 머티리얼 매니저는 상한을 셋 지정하게 한다 — 캐시 수명, 메시지 수, 바이트 수다. 그래서 무엇을 얼마나 바꿨는지가 눈에 보인다. 상한 없이 무기한 재사용하면 키 하나가 새는 순간의 피해 범위가 커진다.
...
| 증상 | 원인 | 확인·대응 |
|---|---|---|
| `MalformedPolicyDocumentException` | root-less 키 정책에서 관리 액션 누락, lockout 검사 실패 | 배포자 신원 statement 에 `kms:PutKeyPolicy`(`kms:Put*`) 가 있는지 확인 |
...
AWS KMS는 키 정책을 적용하기 전에 **그 요청을 보낸 신원이 이후에도 `kms:PutKeyPolicy` 를 호출할 수 있는지** 확인해 락아웃을 방지한다. "유효한 관리자가 아무나 남으면 된다"가 아니다 — 다른 ARN 에만 권한을 줬다면 검사를 통과하지 못한다. 실패하면 `MalformedPolicyDocumentException` 과 함께 생성이 거부된다.
</Quiz>
...
암호화 컨텍스트는 암호문에 묶여 저장된다. 복호화할 때 키와 값이 완전히 같아야 성공한다. 컨텍스트 자체는 암호화되지 않고 평문으로 로그에 남으니 비밀 값을 넣지 않는다. 대신 CloudTrail에 남는다는 성질을 이용해 추적하거나, 키 정책의 `Condition` 에서 `kms:EncryptionContext:<키>` 로 검사해 특정 버킷을 위한 요청만 복호화를 허용할 수 있다.
</Quiz>
...
암호화가 정리됐으면 남은 기초는 컨테이너 쪽이다. 모듈 07에서 KMS 키와 버킷·CDN 연동을 Terraform 으로 묶는다.

<LinkButton href="/start/docker-basics/" icon="right-arrow">다음 선수 학습 — Docker 기초</LinkButton>
<LinkButton href="/part-1/07-kms-s3-cloudfront/" variant="minimal">PART 1 — KMS + S3 + CloudFront 실습</LinkButton>
