...
**등록 직후 바로 트래픽이 가지 않는다.** 새로 등록된 타깃은 헬스체크를 1회 통과해야 `healthy` 가 되므로 기본 간격 30초만큼 기다린다. `unhealthy` 로 떨어진 타깃이 되돌아올 때는 `HealthyThresholdCount` 만큼 연속으로 성공해야 하므로 기본값이면 150초다. 대회처럼 시간이 점수인 상황에서는 간격과 임계를 줄인다.
