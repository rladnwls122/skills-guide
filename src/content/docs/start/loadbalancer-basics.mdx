...
**선수 학습 11/18.** ALB 를 한 번도 안 만들어 본 상태를 전제로 쓴다. ② 까지는 Terraform·쿠버네티스를 몰라도 읽힌다. ③ 부터는 서브넷과 보안 그룹을 전제하므로 [vpc-basics](/start/vpc-basics/) 를 먼저 본다.
...
로드밸런서가 하는 최소한의 일이다. 사용자는 주소 하나만 알고, 그 뒤에 서버가 몇 대인지·어느 대가 살아 있는지는 모른다. 서버를 늘리거나 죽은 서버를 빼도 사용자가 쓰는 주소는 그대로다.
...
flowchart TD
    ...
    T2@{ icon: "logos:aws-ec2", form: "square", label: "타깃<br/>서버 · IP · Lambda", pos: "b", h: 46, w: 46 }
...
ALB 안의 네 계층이다. 리스너가 포트(:80)를 받고, 규칙이 priority 낮은 번호부터 조건을 맞춰 보고, 맞은 규칙이 지정한 타깃그룹으로 넘긴다. 아무 규칙도 걸리지 않으면 기본 액션이 처리한다. 타깃그룹이 실제 서버·IP·Lambda 를 담는 자리이므로, 헬스체크도 ALB 가 아니라 타깃그룹에 붙는다.
...
| `HealthyThresholdCount` | `unhealthy` 인 타깃이 몇 번 연속 성공해야 되살아나나 | 5 |
| `UnhealthyThresholdCount` | `healthy` 인 타깃이 몇 번 연속 실패해야 죽었다고 보나 | 2 |
| `Matcher` | 어떤 상태 코드를 성공으로 보나 | `200` |

기본값은 `instance`·`ip` 타깃 기준이다. `lambda` 타깃그룹은 간격 35초, 타임아웃 30초다.

**등록 직후 바로 트래픽이 가지 않는다.** 판정이 두 갈래로 나뉘기 때문이다.

- **새로 등록한 타깃**은 `initial` 상태로 들어가고, 헬스체크를 **한 번** 통과하면 `healthy` 가 된다. 기본 간격이 30초이므로 그만큼 기다린다.
- **`unhealthy` 로 떨어졌던 타깃**이 되살아날 때만 `HealthyThresholdCount` 가 적용된다. 기본값 5 에 간격 30초를 곱해 150초가 걸린다.

표의 기본값 5 를 신규 등록에도 적용된다고 읽으면 대기 시간을 다섯 배로 잘못 계산한다. 대회처럼 시간이 점수인 상황에서는 간격과 임계를 함께 줄인다.

`aws elbv2 describe-target-health` 출력의 상태값은 여섯 가지다.

| 상태 | 뜻 | 흔한 원인 |
|---|---|---|
| `initial` | 등록됐고 첫 헬스체크가 아직 진행 중 | 방금 등록함 — 기다리면 넘어간다 |
| `healthy` | 정상. 트래픽을 받는다 | — |
...
쿠버네티스에서 `instance` 를 쓰면 그 "지정 포트"가 Service 의 NodePort 다.
...
ALB 가 Lambda 를 타깃으로 부를 때 Lambda 가 돌려줘야 하는 응답 형태다. 이 네 키가 없으면 ALB 가 502 를 낸다 — 함수가 성공해도 형식이 안 맞으면 실패로 처리된다. `isBase64Encoded` 는 본문이 이미지 같은 바이너리인지 알리는 표시고, 텍스트면 `false` 다.
...
ALB 를 CloudFront 뒤에만 두는 방법이다. CloudFront 는 오리진으로 요청을 보낼 때 약속된 커스텀 헤더를 심고, ALB 는 그 헤더가 있는 요청만 규칙으로 통과시킨다. 헤더가 없는 직접 접근은 어느 규칙에도 걸리지 않아 기본 액션인 403 으로 끝난다. ALB 주소를 알아도 우회가 안 되는 구조다.

</details>

CloudFront 가 오리진으로 보낼 때 커스텀 헤더를 심고, ALB 규칙이 그 헤더 값을 조건으로 검사한다. 헤더가 없으면 어떤 규칙에도 안 걸려 기본 액션 `fixed-response 403` 으로 떨어진다.

set-02 실측은 규칙이 **정확히 2개**다. 채점(mark 7-2)이 `describe-rules` 출력의 `HttpHeaderConfig.Values[]` 줄 수를 세므로 규칙을 더하거나 빼면 그 항목이 어긋난다. 상세는 [k8s 워크로드·ALB 이론](/part-2/10-k8s-workloads-alb/theory/) 에 있다.
...
:::caution[과금 주의]
ALB 는 시간당 과금이다. 서울 리전 기준 시간당 약 $0.0225 에 처리량(LCU)이 더해진다. 30분이면 센트 단위지만 **삭제하지 않으면 계속 붙는다.** 7단계까지 반드시 끝낸다.
:::
...
1. **Lambda 함수를 만든다**
   ...
   위 응답 규격을 그대로 돌려주는 최소 함수다. Python 이라 `False` 의 대소문자만 JSON 과 다르고 키 구성은 같다. 이 함수 하나로 타깃그룹이 healthy 가 되므로 EC2 없이 ALB 동작을 확인할 수 있다.
...
5. **CLI 로 읽는다**
   ...
   채점이 보는 네 가지를 CLI 로 같은 순서로 확인한다. ALB 는 상태(`active`)와 스킴(internet-facing/internal), 타깃그룹은 타깃 유형과 헬스체크 경로, 규칙은 priority 와 조건 필드, 마지막이 타깃 상태다. 규칙·타깃 조회는 ARN 이 필요하므로 `describe-*` 결과를 변수로 받아 이어 붙였다 — 이름은 알아도 ARN 은 계정마다 다르기 때문이다.
...
6. **unhealthy 를 재현한다**
   ...
   기대: 약 20초 뒤 `State` 가 `unhealthy` 로, `Reason` 이 `Target.ResponseCodeMismatch` 로 바뀐다. 상태 코드를 200 으로 되돌리면 다시 `healthy` 가 된다.
...
7. **지운다**
   ...
   실습 리소스를 지우고 정말 지워졌는지까지 본다. 삭제는 ALB → 타깃그룹 → Lambda 순이고, 확인 명령이 오류 메시지를 내면 그것이 정상이다 — 리소스를 못 찾았다는 뜻이다. ALB 는 시간당 과금이라 이 단계를 건너뛰면 그대로 청구된다.

   </details>

   `delete-target-group` 이 `ResourceInUse` 로 실패하면 리스너 삭제가 아직 전파되지 않은 것이다. 30초쯤 뒤 다시 실행한다.

   ALB 를 지워도 남는 것이 있다. 실습용으로 새로 만든 보안 그룹, 콘솔이 Lambda 함수와 함께 만든 실행 역할, 로그 그룹 `/aws/lambda/lb-lab-fn` 셋을 확인해 지운다.
...
대회에서 ALB 와 쿠버네티스를 잇는 방식이다. Terraform 이 ALB 와 타깃그룹을 이름 고정으로 만들고(채점이 그 이름을 조회한다), TargetGroupBinding 이 "이 Service 의 파드를 저 타깃그룹에 넣어라"를 선언한다. Ingress 로 ALB 를 만들게 두지 않는 이유가 이름이다 — 컨트롤러가 만들면 이름이 자동 생성된다.
...
TargetGroupBinding 한 장의 필수 필드다. `serviceRef` 가 어느 Service 의 어느 포트인지, `targetGroupARN` 이 어느 타깃그룹인지를 가리킨다. `targetType: ip` 는 노드가 아니라 파드 IP 를 직접 등록한다는 뜻이고, 타깃그룹 쪽 타깃 유형과 값이 어긋나면 등록이 거부된다.

</details>

### 5-4. 왜 대회는 길 B 인가

채점이 `wskorea26-book-tg` 같은 **타깃그룹 이름**으로 리소스를 역조회한다(③-2). 길 A 에서는 그 이름을 컨트롤러가 짓는다. ALB 이름만은 애노테이션으로 맞출 수 있지만 타깃그룹에는 같은 애노테이션이 없어 거기서 막힌다.

길 B 는 그 이름을 Terraform 이 직접 정한다. 그래서 이름 채점이 있는 후보 세트는 전부 길 B 다.
...
모듈 10 에서 이 구조를 실제로 세운다. Terraform 으로 ALB 와 타깃그룹을 만들고, `TargetGroupBinding` 으로 파드를 등록하고, CloudFront 헤더를 검사하는 리스너 규칙을 붙인다.

<LinkButton href="/start/eks-basics/" icon="right-arrow">다음 선수 학습 — EKS 기초</LinkButton>
<LinkButton href="/part-2/10-k8s-workloads-alb/" variant="minimal">PART 2 — k8s 워크로드·ALB</LinkButton>
