...
5. **CLI 로 읽는다**
   ...
   채점이 보는 네 가지를 CLI 로 같은 순서로 확인한다. ALB 는 상태(`active`)와 스킴을 본다. 타깃그룹은 타깃 유형과 헬스체크 경로, 규칙은 priority 와 조건 필드, 마지막이 타깃 상태다. 규칙·타깃 조회는 ARN 이 필요하므로 `describe-*` 결과를 변수로 받아 이어 붙였다 — 이름은 알아도 ARN 은 계정마다 다르기 때문이다.
...
대회에서 ALB 와 쿠버네티스를 잇는 방식이다. Terraform 이 ALB 와 타깃그룹을 이름 고정으로 만든다 — 채점이 그 이름을 조회하기 때문이다. 그다음 TargetGroupBinding 이 "이 Service 의 파드를 저 타깃그룹에 넣어라"를 선언한다. Ingress 로 ALB 를 만들게 두지 않는 이유가 이름이다 — 컨트롤러가 만들면 이름이 자동 생성된다.
