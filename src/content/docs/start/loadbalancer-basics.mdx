...
set-02 실측은 규칙이 **정확히 2개**다. 채점(mark 7-2)이 `describe-rules` 출력의 `HttpHeaderConfig.Values[]` 줄 수를 세므로 규칙을 더하거나 빼면 그 항목이 어긋난다. 상세는 [k8s 워크로드·ALB 이론](/part-2/12-k8s-workloads-alb/theory/) 에 있다.
...
모듈 12 에서 이 구조를 실제로 세운다. Terraform 으로 ALB 와 타깃그룹을 만들고, `TargetGroupBinding` 으로 파드를 등록하고, CloudFront 헤더를 검사하는 리스너 규칙을 붙인다.

<LinkButton href="/start/eks-basics/" icon="right-arrow">다음 선수 학습 — EKS 기초</LinkButton>
<LinkButton href="/part-2/12-k8s-workloads-alb/" variant="minimal">PART 2 — k8s 워크로드·ALB</LinkButton>
