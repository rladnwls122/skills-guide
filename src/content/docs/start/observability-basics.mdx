...
**선수 학습 13/18.** [k8s-basics](/start/k8s-basics/)를 먼저 읽는다 — 무엇을 수집하는지가 파드·서비스 단위로 나온다.
...
### SLI · SLO · SLA — 목표를 숫자로 정한다 :badge[강의]{variant=tip}

"느리다"는 조치로 이어지지 않는다. 무엇을 재고 어디까지 허용할지 정해야 다음 행동이 갈린다. 세 단어가 그 자리를 나눠 맡는다.

| 말 | 무엇인가 | 정하는 쪽 |
|---|---|---|
| SLI(service level indicator) | 실제로 재는 값 | 엔지니어 |
| SLO(service level objective) | 그 값의 목표치 | 팀 내부 |
| SLA(service level agreement) | 고객과 맺은 약속 | 사업·법무 |

SLI 는 측정값이다. 성공 응답 비율, p99 응답시간이 여기 들어간다. SLO 는 그 측정값의 목표다 — "성공률 99.9% 유지"처럼 쓴다. SLA 는 계약이라 어기면 환불 같은 대가가 따른다. 그래서 SLA 는 SLO 보다 느슨하게 잡는다.

SLO 를 정하면 **허용 실패량**이 따라 정해진다. 성공률 목표가 99.9% 면 1000건 중 1건은 실패해도 목표 안이다. 이 여유분을 오류 예산(error budget)이라고 부른다. 예산이 남아 있으면 배포를 계속하고, 다 쓰면 안정화 작업으로 돌린다.

부하 속에서 운영하는 3과제가 이 감각을 쓰는 자리다. 오류 한 건마다 손을 대면 다른 축을 잃는다. 목표 대비 얼마나 남았는지가 조치 여부를 가른다.

### 알림은 증상에 건다 :badge[강의]{variant=tip}

CPU 사용률 같은 원인 지표에 알림을 걸면 대부분 조치가 필요 없는 알림이 된다. 사용률이 높아도 응답이 멀쩡하면 아무 일도 일어나지 않는다. 그래서 알림은 사용자가 겪는 증상에 건다 — 오류율, 응답시간, 성공률이다.

기준은 하나다. 알림을 받은 사람이 **지금 무엇을 할지** 바로 알 수 있어야 한다. 그 답이 없는 알림은 지운다. 조치 없는 알림이 쌓이면 진짜 장애 알림도 같이 묻힌다.

감시 체계 자체가 죽으면 알림도 오지 않는다. 그래서 정상일 때 주기적으로 오는 신호를 하나 두고, 그 신호가 끊기면 경보를 낸다. 이 장치를 dead man's switch 라고 부른다.
...
PART-2 모듈 11 에서 이 파이프라인을 실제로 세운다. 남은 기초는 2과제 쪽이다.

<LinkButton href="/start/serverless-basics/" icon="right-arrow">다음 선수 학습 — 서버리스·이벤트 기초</LinkButton>
<LinkButton href="/part-2/11-observability/" variant="minimal">PART 2 — 관측성 모듈</LinkButton>
