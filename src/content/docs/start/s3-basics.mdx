...
**선수 학습 3/18.** [iam-basics](/start/iam-basics/)를 먼저 읽는다 — 버킷 정책이 IAM 정책과 같은 문법이다.
...
S3 요청 하나가 허용되기까지의 관문 순서다. 퍼블릭 액세스 차단(BPA)이 가장 앞이라, 버킷 정책이 아무리 열려 있어도 퍼블릭으로 판정되면 그 자리에서 끝난다. 그다음이 명시적 Deny, 마지막이 IAM 정책 또는 버킷 정책의 Allow 다. BPA 를 켠 채 정적 웹을 서비스하려면 퍼블릭이 아닌 경로 — CloudFront 와 OAC — 가 필요한 이유가 이 순서다.

</details>

#### BPA 를 이루는 4개 항목

퍼블릭 액세스 차단은 스위치 하나가 아니라 독립된 항목 네 개다. 콘솔의 "모든 퍼블릭 액세스 차단"은 이 넷을 한 번에 켜는 버튼이고, API 로는 `PutPublicAccessBlock` 에 넷을 각각 넘긴다. 앞의 둘이 ACL 을 보고 뒤의 둘이 버킷 정책을 본다.

| 항목 | 보는 대상 | 작동 시점 | 하는 일 |
|---|---|---|---|
| `BlockPublicAcls` | ACL | 쓰기 | 퍼블릭 ACL 을 새로 넣는 요청을 거부한다 |
| `IgnorePublicAcls` | ACL | 요청 | 이미 붙어 있는 퍼블릭 ACL 을 무시한다 |
| `BlockPublicPolicy` | 버킷 정책 | 쓰기 | 퍼블릭을 허용하는 버킷 정책의 저장을 거부한다 |
| `RestrictPublicBuckets` | 버킷 정책 | 요청 | 퍼블릭으로 판정된 버킷에 대한 익명·교차 계정 접근을 막는다 |

같은 대상을 보는 두 항목이 시점으로 갈린다. `Block*` 은 퍼블릭 설정이 **들어오는 것**을 막고, `Ignore*`·`Restrict*` 는 이미 들어와 있는 설정이 **효력을 내는 것**을 막는다. 채점은 넷이 모두 `true` 인지를 본다.

#### 켠 순서에 따라 증상이 갈린다

요청 흐름 도식이 보여준 것은 **요청 시점**의 판정이다. BPA는 그보다 앞선 **쓰기 시점**에도 작동한다. `BlockPublicPolicy` 가 켜져 있으면 퍼블릭을 허용하는 버킷 정책은 `PutBucketPolicy` 호출 자체가 거부된다 — 조건 없는 `"Principal": "*"` 가 AWS가 드는 대표 예다. 순서에 따라 증상이 갈린다. BPA 를 켠 채로 퍼블릭 정책을 넣으면 저장 단계에서 `AccessDenied` 로 막힌다. 정책을 먼저 붙인 뒤 BPA 를 켜면 정책은 그대로 남고 `RestrictPublicBuckets` 가 요청 시점에 판정을 무효화한다. 증상이 다르므로 어느 순서로 켰는지부터 확인한다.

### 객체 소유권과 ACL

객체 소유권을 **버킷 소유자 적용**(Bucket owner enforced)으로 두면 ACL 이 통째로 비활성화된다. 2023년 이후 새 버킷의 기본값이 이것이다. ACL로 권한을 주는 예제 코드가 동작하지 않는 대부분의 이유가 여기에 있다. 대회 환경에서는 ACL을 쓰지 않고 버킷 정책으로만 통제한다 — 통제 지점이 하나여야 채점도 디버깅도 단순해진다.
...
- **정적 웹사이트 원본**: 버킷은 비공개로 두고 CloudFront만 읽게 한다. 버킷의 정적 웹사이트 호스팅 기능을 켜는 방식이 아니라 **OAC + 버킷 정책** 조합이 대회 유형이다. OAC(Origin Access Control)는 CloudFront가 S3 원본을 읽을 때 `cloudfront.amazonaws.com` 서비스 주체로 **요청에 서명**하게 하는 장치다 — 버킷은 그 주체만 허용하면 되므로 퍼블릭 액세스는 계속 차단해 둔다.
- **SSE-KMS 필수**: 버킷 기본 암호화를 지정 KMS 키로 건다. KMS 키 정책과 버킷 정책 양쪽이 맞아야 업로드가 성공한다.
- 이 내용은 **PART-1 모듈 07(KMS + S3 + CloudFront)** 의 직접적인 기반이다.

## ④ 핵심 코드 패턴

③에서 말한 OAC를 버킷 쪽에서 받아 주는 정책이다. 서명해서 오는 CloudFront 서비스 주체에게만 읽기를 열어, 원본을 비공개로 둔 채 특정 배포판만 읽게 한다. 구조와 OAI 와의 차이는 ⑤에서 이어 다룬다.
...
OAC 를 쓰는 버킷 정책이다. `Principal` 이 사용자나 계정이 아니라 CloudFront 서비스이고, `Condition` 의 `AWS:SourceArn` 이 그 배포 하나로 범위를 좁힌다. 이 조건이 없으면 다른 사람의 CloudFront 배포도 이 버킷을 오리진으로 삼을 수 있다.
...
채점이 보는 버킷 속성 셋을 각각 조회한다. `get-bucket-encryption` 은 기본 암호화가 SSE-KMS 인지와 어느 키인지, `get-public-access-block` 은 차단 4개 항목이 모두 `true` 인지, `get-bucket-policy` 는 정책 원문을 낸다. 세 가지가 서로 다른 API 라 하나만 맞춰 두고 넘어가면 나머지에서 감점된다.
...
④에서 쓴 정책을 요청 흐름으로 펼치면 이렇다. CloudFront는 원본 요청에 **SigV4 서명**을 실어 보내고, 버킷 정책은 그 서비스 주체와 `SourceArn` 을 대조해 통과시킨다. 퍼블릭 액세스는 내내 차단 상태다.
...
BPA 를 켠 비공개 버킷을 CloudFront 로 공개하는 경로다. CloudFront 가 OAC 로 SigV4 서명을 붙여 `GetObject` 를 보내고, 버킷 정책이 그 서비스 주체와 배포 ARN 을 확인해 통과시킨다. 객체가 SSE-KMS 로 암호화돼 있으면 S3 가 이어서 KMS 에 복호화를 요청한다. 그래서 키 정책에도 CloudFront 경로가 허용돼야 200 이 나온다.
...
<Quiz title="Standard-IA 로 내렸는데 더 비싸졌다">
1KB 짜리 로그 객체 수백만 개를 비용 절감 목적으로 Standard-IA 로 즉시 전환했다. 결과는?

- [x] 128KB 미만 객체도 128KB 로 과금되고 전환 요청 비용까지 붙어 오히려 비싸진다
- [ ] 최소 보관 30일이 지나기 전에는 전환 자체가 거부된다
  > 전환은 된다. 대신 30일을 채우기 전에 지우거나 다시 옮기면 남은 기간만큼 요금이 청구된다.
- [ ] Standard-IA 는 꺼내는 데 시간이 걸려 조회가 느려진다
  > 즉시 꺼낸다. 수 시간이 걸리는 쪽은 Glacier Deep Archive 다.
- [ ] Intelligent-Tiering 과 마찬가지로 최소 보관 기간이 없어 손해가 없다
  > 최소 보관이 없는 쪽이 Intelligent-Tiering 이다. Standard-IA 는 30일이다.

스토리지 클래스는 **최소 보관 기간**(Standard-IA 30일, Glacier Instant Retrieval 90일, Deep Archive 180일)과 **최소 과금 크기**(Standard-IA 128KB) 두 축을 함께 본다. 작은 객체를 무턱대고 내리면 두 축 모두에 걸린다. 접근 패턴을 모르면 최소 보관도 최소 크기도 없는 Intelligent-Tiering 이 안전한 기본값이다.
</Quiz>

<Quiz title="버전 관리 버킷의 수명 주기 규칙">
버전 관리를 켠 버킷에서 이전 버전이 쌓여 비용이 계속 는다. 수명 주기 규칙으로 옳은 것을 **모두** 고르시오.

- [x] 이전 버전(noncurrent)에 별도 만료 규칙을 건다
- [x] `AbortIncompleteMultipartUpload` 를 함께 걸어 미완료 조각을 정리한다
- [ ] 규칙 하나로 최신 버전과 이전 버전이 함께 정리된다
  > 둘은 서로 다른 규칙 대상이다. 최신 버전 만료만 걸면 이전 버전은 그대로 남아 계속 과금된다.
- [ ] 삭제 마커는 객체가 아니므로 수명 주기가 손대지 못한다
  > 만료된 삭제 마커를 지우는 규칙이 따로 있다. 방치하면 목록 조회만 느려진다.

버전 관리 버킷에서는 **최신 버전과 이전 버전에 각각 규칙을 건다**. 이전 버전 30일 만료 + 미완료 멀티파트 조각 7일 중단이 흔한 조합이다. 멀티파트 조각은 객체 목록에 보이지 않으면서 과금되므로 이 규칙이 없으면 원인을 찾기 어렵다.
</Quiz>

<Quiz title="이벤트 알림이 두 번 왔다">
S3 이벤트 알림으로 Lambda 를 호출해 DynamoDB 카운터를 올린다. 업로드는 1건인데 카운터가 2 오른 적이 있다. 원인과 대응은?

- [x] 이벤트 전달이 최소 한 번(at-least-once) 보장이라 중복될 수 있다 — 수신 측을 멱등하게 짠다
- [ ] S3 가 업로드를 두 번 기록했다
  > `PutObject` 한 번은 객체 하나다. 중복되는 것은 객체가 아니라 알림 전달이다.
- [ ] suffix 필터가 없어 다른 객체까지 같은 함수를 호출했다
  > 필터 누락은 재귀 루프의 원인이지, 같은 객체가 두 번 세어지는 원인은 아니다.
- [ ] DynamoDB 조건부 쓰기를 쓰지 않아 값이 겹쳐 쓰였다
  > 겹쳐 쓰기는 값이 덮이는 문제다. 카운터가 하나 더 오르는 것과는 방향이 다르다.

S3 이벤트는 **정확히 한 번이 아니라 최소 한 번** 전달된다. 같은 이벤트가 두 번 올 수 있다는 전제로 수신 측을 짜야 한다 — 객체 키나 `versionId` 를 멱등 키로 삼아 조건부 쓰기를 걸거나, 처리 완료를 별도로 기록해 두 번째 호출을 건너뛴다.
</Quiz>

<Quiz title="복제 규칙을 걸었는데 대상 버킷이 비어 있다">
객체 1만 개가 이미 들어 있는 버킷에 CRR 규칙을 걸었다. 한참 기다려도 대상 버킷에 아무것도 오지 않는다. 이유는?

- [x] 규칙을 건 **이후에 올라온 객체만** 복제된다 — 기존 객체는 배치 복제를 따로 돌려야 한다
- [ ] 복제는 동기 동작이라 완료까지 기다려야 한다
  > 비동기다. 지연은 있어도 "아무것도 오지 않는" 상태로 멈추지는 않는다.
- [ ] 원본만 버전 관리를 켜면 되는데 대상까지 켜서 충돌했다
  > 양쪽 다 버전 관리가 켜져 있어야 복제가 성립한다. 켜서 생기는 문제가 아니다.
- [ ] 리전이 같으면 SRR 이라 CRR 규칙이 무시된다
  > 같은 리전이면 SRR 이라 부를 뿐, 규칙이 무시되지는 않는다.

복제는 **규칙 생성 이후의 객체**를 대상으로 한다. 기존 객체는 S3 배치 복제로 따로 밀어 넣는다. 함께 걸리는 조건 둘 — 원본과 대상 **양쪽 버전 관리**가 켜져 있어야 하고, SSE-KMS 객체라면 복제 역할에 **대상 리전 KMS 키** 권한까지 줘야 한다. 이 셋 중 하나라도 빠지면 조용히 복제되지 않는다.
</Quiz>
...
SSE-KMS 가 실제로 어떤 키를 어떻게 쓰는지는 다음 문서에서 이어진다. 모듈 07에서 KMS·S3·CloudFront를 Terraform으로 한 스택에 묶는다.
