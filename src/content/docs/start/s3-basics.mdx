...
요청 흐름 도식이 보여준 것은 **요청 시점**의 판정이다. BPA는 그보다 앞선 **쓰기 시점**에도 작동한다. `BlockPublicPolicy` 가 켜져 있으면 퍼블릭을 허용하는 버킷 정책은 `PutBucketPolicy` 호출 자체가 거부된다 — 조건 없는 `"Principal": "*"` 가 AWS가 드는 대표 예다. 즉 BPA를 켠 채로 퍼블릭 정책을 넣으면 저장 단계에서 `AccessDenied` 로 막히고, 정책을 먼저 붙인 뒤 BPA를 켜면 정책은 그대로 남은 채 `RestrictPublicBuckets` 가 요청 시점에 판정을 무효화한다. 증상이 다르므로 어느 순서로 켰는지부터 확인한다.
...
BPA 를 켠 비공개 버킷을 CloudFront 로 공개하는 경로다. CloudFront 가 OAC 로 SigV4 서명을 붙여 `GetObject` 를 보내고, 버킷 정책이 그 서비스 주체와 배포 ARN 을 확인해 통과시킨다. 객체가 SSE-KMS 로 암호화돼 있으면 S3 가 이어서 KMS 에 복호화를 요청하므로, 키 정책에도 CloudFront 경로가 허용돼야 200 이 나온다.
