...
OAC는 `cloudfront.amazonaws.com` 서비스 주체로 **SigV4 서명**한 요청을 보내므로 KMS 복호화 경로까지 성립한다. 반면 OAI는 SigV2 기반의 특수 IAM 신원이라 SSE-KMS 원본을 지원하지 않는다. 대회 유형이 KMS 암호화를 요구하는 이상 선택지는 OAC 하나뿐이고, `403` 을 정책 오타로 오해해 시간을 버리기 쉬운 지점이다.
</Quiz>
