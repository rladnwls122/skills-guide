...
2과제 모듈의 절반이 이 다섯 서비스 위에 서 있다. [모듈 14](/part-4/14-serverless-event/)·[모듈 15](/part-4/15-eks-scaling/)의 이론은 "이미 안다"를 전제로 채점 함정만 압축해 두었으므로, 그 문서에서 EventBridge 룰이나 가시성 제한 시간이 낯설게 느껴지면 여기서 먼저 채운다.
