...
**선수 학습 6/18.** [PART 0 의 리눅스 실습](/part-0/04-linux-shell/)과 짝이다. 그쪽이 손, 여기가 눈이다.
...
이름이 바뀌는 변동 과제를 손으로 처리하는 순서다. `grep -rl` 로 그 문자열이 든 파일 목록을 먼저 확인하고, `sed -i` 로 파일을 제자리에서 치환한다. 그다음 `terraform plan` 으로 바뀐 코드가 무엇을 만들지 보고, 결과 JSON 에서 필요한 값만 `jq -r` 로 뽑는다. 찾기와 바꾸기를 나누는 것이 핵심이다 — 어디를 고칠지 모르는 채로 치환하면 의도치 않은 파일까지 바뀐다.
...
heredoc 으로 여러 줄짜리 파일을 한 번에 만든다. `<<'EOF'` 의 따옴표가 핵심이다 — 이것이 없으면 셸이 안쪽의 `$` 와 백틱을 먼저 해석해 Terraform 문법이 깨진다.
...
- 5.1 에서 `curl`은 Invoke-WebRequest 별칭 → 진짜 curl은 **`curl.exe`** (7 부터는 별칭이 없어졌지만 그대로 `curl.exe` 로 쓴다)
...
- **30% 변동 드릴(PART-6 모듈 23)이 전부 grep/sed다**: 과제지가 리소스 이름·CIDR·태그를 바꿔서 나오면, 받은 코드에서 `grep -rl`로 위치를 찾고 `sed -i`로 일괄 치환하는 속도가 곧 시간 점수다.
...
1. **샘플 tf 파일 만들고 이름 일괄 치환**
   ...
   grep -n "contest" vpc.tf            # 2곳 바뀌었는지 확인
   ```

   <details class="diagram-note">
   <summary>이 블록이 하는 일</summary>

   치환 드릴 한 판이다. 파일을 만들고 → `grep -rl` 로 대상 파일을 찾고 → `sed -i` 로 바꾸고 → `grep -n` 으로 몇 줄이 바뀌었는지 되짚는다. 마지막 확인이 있어야 치환이 리소스 이름과 태그 두 곳 모두에 걸렸는지 알 수 있다.
...
2. **`terraform output -json`을 가정한 jq 추출**
   ...
   `terraform output -json` 같은 응답에서 값만 꺼내는 세 가지 형태다. `jq` 는 따옴표를 포함한 JSON 문자열을 내고, `-r` 은 따옴표를 벗긴 raw 문자열을 낸다. 환경 변수에 넣을 값은 항상 `-r` 이다 — 따옴표가 그대로 들어가면 뒤따르는 CLI 호출이 이름을 못 찾는다. 배열은 `[0]` 으로 첨자를 준다.
...
Windows PowerShell 5.1에서 `curl http://...` 이 기대와 다르게 동작하는 이유와 해결책은?

- [x] `curl`이 `Invoke-WebRequest`의 별칭이기 때문. 진짜 curl은 `curl.exe`로 실행한다
- [ ] PowerShell 5.1의 CP949 인코딩 때문. PowerShell 7로 올리면 해결된다
  > CP949 인코딩 문제는 **실재하지만 다른 증상**이다(jq·terraform 출력 깨짐). 다만 `curl` 별칭은 PowerShell 7에서 실제로 사라졌으므로, 이 함정은 5.1 에서만 만난다.
...
**Windows PowerShell 5.1** 은 `curl`·`wget`을 각각 `Invoke-WebRequest`의 별칭으로 잡아 두었다. 문법도 출력도 다르므로 `-H`, `-d` 같은 curl 플래그가 통하지 않는다. **`curl.exe`** 로 명시하면 진짜 curl이 실행된다. (PowerShell 6.0 부터 이 별칭들이 제거돼 7에서는 `curl` 이 곧 `curl.exe` 다. 그래도 대회 PC 에 어느 버전이 뜰지 모르니 `curl.exe` 로 쓰는 습관이 안전하다.) 함께 기억할 PowerShell 함정 셋:

- `$` 가 든 문자열은 작은따옴표로 감싼다(`'pa$$word'`).
- PowerShell 7 이 필수다. 5.1은 CP949로 출력이 깨진다.
- `sed`·`grep` 은 없다. Git Bash나 CloudShell에서 실행한다.
</Quiz>
...
모듈 23(30% 변동 드릴)이 이 grep/sed 콤보를 시간 점수로 환산한다.

<LinkButton href="/start/awscli-basics/" icon="right-arrow">다음 선수 학습 — AWS CLI 기초</LinkButton>
<LinkButton href="/part-6/23-mutation-drill/" variant="minimal">PART 6 — Battle Drills</LinkButton>
