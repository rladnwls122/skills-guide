...
프라이빗 서브넷의 EC2 가 인터넷으로 나가는 경로다. 패킷은 프라이빗 RTB 의 `0.0.0.0/0 → NAT` 경로를 타고 퍼블릭 서브넷의 NAT GW 로 간다. NAT GW 는 출발지 주소를 자기 탄력적 IP 로 바꾸고, 그다음은 퍼블릭 RTB 의 `0.0.0.0/0 → IGW` 경로로 인터넷에 닿는다. 반대 방향 경로는 없다 — 인터넷에서 EC2 로 먼저 연결할 방법이 이 그림에 없는 것이 NAT GW 가 아웃바운드 전용이라는 뜻이다.
...
같은 EC2 가 AWS 서비스를 부를 때 엔드포인트 종류에 따라 경로가 갈린다. ① S3 는 게이트웨이 엔드포인트라 라우팅 테이블에 접두사 목록 경로가 생기고, 패킷이 그 경로를 타면 VPC 밖으로 나가지 않는다. ② ECR·STS 는 인터페이스 엔드포인트라 서브넷 안에 ENI 가 생기고, 사설 DNS 가 서비스 이름을 그 ENI 주소로 풀어 준다. ENI 에는 보안 그룹이 붙으므로 443 을 열지 않으면 여기서 막힌다.
