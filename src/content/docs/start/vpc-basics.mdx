...
| 기본값 | 인바운드 전부 거부, 아웃바운드 전부 허용 | 기본 NACL은 양방향 전부 허용 |
...
| `0.0.0.0/0` | NAT GW | |
| `10.0.0.0/16` | local | |
| `10.0.5.0/24` | Transit GW | **선택** — 접두사가 가장 길다 |

VPC CIDR에 대한 `local` 경로는 자동 생성되며 **지우거나 덮어쓸 수 없다**. VPC 내부 통신이 항상 VPC 안에서 끝나는 이유다.
...
양쪽 다 **CIDR이 겹치면 연결할 수 없다**. VPC를 여러 개 설계할 때 CIDR을 미리 갈라 두는 이유가 이것이다.
